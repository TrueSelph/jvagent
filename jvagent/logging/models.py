...
@compound_index([("context.app_id", 1), ("context.logged_at", -1)], name="error_app_logged_at")
@compound_index([("context.app_id", 1), ("context.error_code", 1), ("context.logged_at", -1)], name="error_app_code_logged_at")
@compound_index([("context.agent_id", 1), ("context.logged_at", -1)], name="error_agent_logged_at")
@compound_index([("context.status_code", 1), ("context.logged_at", -1)], name="error_status_logged_at")
@compound_index([("context.app_id", 1), ("context.user_id", 1), ("context.logged_at", -1)], name="error_app_user_logged_at")
class ErrorLog(Object):
    """Log entry for API errors.

    Stores comprehensive error information for debugging and traceability.
    Each log entry is mapped by application ID and agent ID, and includes
    all error context and details.
    ...
    Attributes:
        ...
        path: Request path
        method: HTTP method
        logged_at: Timestamp when the error was logged
    ...
    user_id: str = attribute(
        indexed=True, default="", description="User ID (if available)"
    )
    session_id: str = attribute(
        default="", description="Session identifier (if available)"
    )
    interaction_id: str = attribute(
        default="", description="Interaction ID (if error occurred during interaction)"
    )
    status_code: int = attribute(
        indexed=True, default=500, description="HTTP status code"
    )
    error_code: str = attribute(
        indexed=True, default="", description="Machine-readable error code"
    )
    path: str = attribute(
        indexed=True, default="", description="Request path"
    )
    method: str = attribute(
        default="", description="HTTP method"
    )
    logged_at: datetime = attribute(
        indexed=True,
        index_direction=-1,
        default_factory=lambda: datetime.now(timezone.utc),
        description="Timestamp when the error was logged",
    )
    error_data: Dict[str, Any] = attribute(
        default_factory=dict,
        description="Complete error payload structure",
    )
