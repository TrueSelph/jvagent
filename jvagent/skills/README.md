...
`jvagent` skill bundles are Claude-compatible modular capabilities consumed by `CockpitInteractAction`.
...
1. `CockpitInteractAction` resolves skill bundles from configured sources.
...
5. The cockpit tool registry loads tools from that bundle and returns the SOP.
