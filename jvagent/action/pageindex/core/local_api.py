...
class LocalAPI:
    ...
    def __init__(
        self, storage_path: str, model: str, summary_model: str, retrieve_model: str
    ):
    ...
    def submit_document(
        ...
        pages = [
            {"page_index": i + 1, "markdown": text} for i, text in enumerate(page_texts)
        ]
    ...
    def _index_standard(
        ...
        page_list = [
            (text, litellm.token_counter(model=self._model, text=text))
    ...
    def _index_flash(
        ...
        result = page_index_flash(
            file_path, summary=True, summary_model=self._summary_model
        )
    ...
    def _load_tree_with_text(self, doc_id: str, error_prefix: str) -> list:
        ...
        structure = self._require_data(self._store.get_tree(doc_id), error_prefix)
    ...
    def get_tree(
        self, doc_id: str, node_summary: bool = False, include_text: bool = True
    ) -> dict[str, Any]:
    ...
    def get_ocr(self, doc_id: str, format: str = "page") -> dict[str, Any]:
        ...
        if format == "node":
            ...
            _walk(self._load_tree_with_text(doc_id, "Failed to get OCR result"), 1)
        else:
