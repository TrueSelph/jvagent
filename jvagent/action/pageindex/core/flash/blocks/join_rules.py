...
def should_join_line_to_block(
    ...
    both_edges_aligned = left_edges_aligned or (
        other_block.line_count() == 1
        and left_aligned(
            other_block, candidate_line, 8 * avg_char_width(other_block.line())
        )
    )
    ...
    page_body_font_delta = min(
        candidate_line.avg_font_size() - block_cluster_ctx.primary_slot.primary_slot,
        other_block.avg_font_size() - block_cluster_ctx.primary_slot.primary_slot,
    )

    doc_body_font_delta = min(
        candidate_line.avg_font_size() - block_cluster_ctx.tertiary_slot.primary_slot,
        other_block.avg_font_size() - block_cluster_ctx.tertiary_slot.primary_slot,
    )
    ...
    if (
        ...
        and not (
            style_key(block_last_span) == style_key(line_first_span)
            and block_last_span.char_count() > 1
            and line_first_span.char_count() > 1
        )
        and (
            font_size_delta > 2
            ...
            or (
                font_size_delta < -1
                and candidate_line.char_count() >= 20
                and not both_edges_aligned
            )
        )
    ):
        return False
    ...
    if font_mismatch or bold_mismatch:
        if bold_mismatch and width_ratio > 2:
            return False
        if (
            block_last_line.char_stats.secondary_slot == 1
            or block_last_line.char_stats.secondary_slot == 2
        ) and (candidate_line.char_stats.secondary_slot == 2 or width_ratio > 4):
            return False
        if (
            block_last_line.char_stats.tertiary_slot == 6
            or other_block.bbox_width() > 1.5 * block_last_line.bbox_width()
        ):
            return False

    if (
        other_block.bold_frac() > 0.9
        and candidate_line.bold_frac() < 0.8
        and width_ratio > 2
    ):
        return False
    ...
    if not centers_aligned:
        ...
        if (
            vertical_gap > -1 and horizontal_offset > 0.33 * other_block.bbox_width()
        ) or horizontal_offset > 0.98 * other_block.bbox_width():
            return False
    ...
    join_tolerance = (
        _max_nan_propagating(
            1.3
            * (other_block.top_edge() - other_block.bottom_edge())
            / other_block.line_count(),
            block_cluster_ctx.primary_slot.tertiary_slot,
        )
    ...
    case_signal_flip = (
        (
            (block_case_signal == 1 and line_case_signal == -1)
            or (line_case_signal == 1 and block_case_signal == -1)
        )
        and letter_count(candidate_line.char_stats) >= 3
        and (
            is_upper_dominant(other_block.char_stats)
            != is_upper_dominant(line_first_span.char_stats)
            or letter_count(line_first_span.char_stats) < 3
        )
        and (
            is_upper_dominant(block_last_span.char_stats)
            != is_upper_dominant(candidate_line.char_stats)
            or letter_count(block_last_span.char_stats) < 3
        )
    )

    if (
        not font_mismatch
        and not bold_mismatch
        and not case_signal_flip
        ...
        and (
            block_last_line.bbox_width() / other_block.bbox_width() > 0.9
            if other_block.bbox_width() != 0
            else block_last_line.bbox_width() > 0
        )
    ):
        join_tolerance *= 1.3
    if (
        page_body_font_delta > 0.5 * block_cluster_ctx.primary_slot.primary_slot
        and not case_signal_flip
    ):
        join_tolerance *= 2
    ...
    column_rect = (
        block_cluster_ctx.state_slot[candidate_line.measure_slot]
        if (0 <= candidate_line.measure_slot < len(block_cluster_ctx.state_slot))
        else None
    ) or EMPTY_RECT
    ...
    line_column_centered = (
        ...
        and (
            x_centers_close(block_cluster_ctx.auxiliary_slot, candidate_line)
            or (block_column_justified and centers_aligned)
        )
    )
    ...
    if (
        block_column_justified
        and line_column_centered
        and other_block.bbox_width() > 0.5 * candidate_line.bbox_width()
        and (
            previous_line is None
            or candidate_line.bottom_edge() - previous_line.bottom_edge()
            >= bottom_edge_gap
        )
        ...
        if previous_line is not None and (
            (
                other_block.bold_frac() > previous_line.bold_frac()
                and candidate_line.bold_frac() > previous_line.bold_frac()
            )
            or (
                other_block.avg_font_size() > previous_line.bbox_height() + 1
                and candidate_line.bbox_height() > previous_line.bbox_height() + 1
            )
        ):
            join_tolerance = max(
                join_tolerance, candidate_line.bottom_edge() - previous_line.top_edge()
            )
        ...
        if (
            other_block.line_count() <= 1
            and candidate_line.char_stats.secondary_slot == 3
        ):
            join_tolerance *= 1.1
        if (
            candidate_line.left_edge() > other_block.left_edge()
            and candidate_line.left_edge()
            <= other_block.left_edge() + 0.1 * other_block.bbox_width()
            and (
                other_block.line_count() <= 1
                or left_aligned(candidate_line, block_last_line, 1)
            )
        ):
            join_tolerance *= 1.2
    elif (
        candidate_line.bbox_width() < 0.9 * block_last_line.bbox_width()
        and center_aligned(other_block, candidate_line, 1)
    ):
        join_tolerance *= 1.1

    if (
        left_edges_aligned
        and candidate_line.bbox_width() < 0.5 * other_block.bbox_width()
        and other_block.char_stats.tertiary_slot != 6
        and candidate_line.char_stats.tertiary_slot == 6
    ):
        join_tolerance *= 1.3
    ...
    block_has_numbering = (
        ...
        and first_span_of(other_block).bbox_height()
        >= 0.8 * other_block.avg_font_size()
    )
    ...
    line_has_numbering = (
        ...
        and candidate_line.primary_slot[0].bbox_height()
        >= 0.8 * candidate_line.avg_font_size()
    )

    line_starts_with_digit = line_has_numbering and line_numbering_kind == 1

    if (
        block_starts_with_digit
        and not line_starts_with_digit
        and font_size_delta <= -0.5
    ):
        join_tolerance /= 2
    elif (block_starts_with_digit and (bold_mismatch or font_size_delta <= -0.5)) or (
        line_starts_with_digit and (bold_mismatch or font_size_delta >= 0.5)
    ):
        join_tolerance /= 1.5
    elif (
        block_starts_with_digit
        and candidate_line.left_edge() >= other_block.left_edge()
        and 0.9 * candidate_line.bbox_width() > other_block.bbox_width()
    ):
        join_tolerance /= 1.5
    elif (
        block_has_numbering
        ...
        block_starts_with_digit
        and candidate_line.char_stats.secondary_slot != 3
        or line_starts_with_digit
    ) and font_mismatch:
        join_tolerance /= 1.3
    elif (
        block_starts_with_digit
        and left_edges_aligned
        and candidate_line.char_stats.secondary_slot == 2
    ):
        join_tolerance /= 1.3
    elif (
        block_has_numbering
        and (
            font_mismatch
            or bold_mismatch
            or font_size_delta <= -0.5
            or (left_edges_aligned and candidate_line.char_stats.secondary_slot == 2)
        )
    ) or (
        line_has_numbering
        and (font_mismatch or bold_mismatch or font_size_delta >= 0.5)
    ):
        join_tolerance /= 1.1
    ...
    if case_signal_flip:
        ...
        if (is_upper_dominant(other_block.char_stats) and block_has_numbering) or (
            is_upper_dominant(candidate_line.char_stats) and line_has_numbering
        ):
            join_tolerance /= 2
        if font_mismatch or bold_mismatch:
            join_tolerance /= 1.5

    if other_block is not first_candidate_block and bottom_edge_gap > 1.1 * (
        first_candidate_block.bottom_edge() - candidate_line.bottom_edge()
    ):
        join_tolerance /= 2
