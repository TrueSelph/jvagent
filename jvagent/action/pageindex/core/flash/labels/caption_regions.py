...
class CaptionedRegion(Bounded):
    ...
    def __init__(
        ...
        area_pct = (
            100.0 * self.area() / self.page.bounds.area()
            if self.page.bounds.area() > 0
            else 0.0
        )
        if area_pct <= 0:
            score = 0.0
            ...
            if self.type == 4:
                ...
                for block in self.output_slot:
                    ...
                    inner += block.area()
                score = (
                    area_pct * max(0.1, 1 - inner / self.area())
                    if self.area() > 0
                    else 0.0
                )
            else:
...
def dedupe_caption_entries(caption_context: "CaptionContext") -> list["CaptionEntry"]:
    ...
    out.sort(
        key=lambda caption_sort_key: (
            caption_sort_key.page_index,
            caption_sort_key.group_slot.reading_order_index,
        )
    )
    return out
...
def extend_caption_region(
    ...
    while 0 <= index < len(sorted_value):
        ...
        if (
            direction < 0
            and caption_column < column_index_of(entry.group_slot)
            and caption.bottom_edge() < anchor
        ):
            col_idx = caption_column - 1
            column_rect = (
                page.tertiary_slot[col_idx]
                if 0 <= col_idx < len(page.tertiary_slot)
                else None
            )
        ...
        if (
            caption.type != 0
            or caption.reading_order_index in caption_context.secondary_slot
        ):
            break
        if page_set is not None and index in page_set:
            break
        size = min(
            caption_context.primary_slot.secondary_slot.primary_slot,
            entry.group_slot.avg_font_size(),
        )
        if caption.is_body_paragraph and caption.avg_font_size() > min(
            0.9 * size, size - 1.5
        ):
            break
        ...
        if (
            ...
            and caption.line_count() <= 4
            and caption.char_stats.secondary_slot != 3
            and gap > line_gap
            and (
                previous is entry.group_slot
                or gap
                > min(3 * line_gap, caption.bottom_edge() - next_block.top_edge())
            )
        ):
            next_item = (
                sorted_value[index + 2] if index + 2 < len(sorted_value) else None
            )
            if heading_score(caption) >= heading_score(previous) + 0.5 and (
                next_block.is_body_paragraph
                or (next_item is not None and next_item.is_body_paragraph)
            ):
                break
    ...
    if direction < 0:
        ...
        while fwd_idx < len(sorted_value):
            ...
            if (
                center_x < bbox.left_edge()
                or center_x > bbox.right_edge()
                or center_y < bbox.bottom_edge()
                or center_y > bbox.top_edge()
            ):
                break
            blocks.append(block)
    ...
    for prior in prior_regions:
        overlap_area = max(
            0.0,
            min(bbox.right, prior.secondary_slot.right)
            - max(bbox.left, prior.secondary_slot.left),
        ) * max(
            0.0,
            min(bbox.top, prior.secondary_slot.top)
            - max(bbox.primary_slot, prior.secondary_slot.primary_slot),
        )
    ...
    return CaptionedRegion(
        primary_item=caption_context.primary_slot,
        secondary_item=page,
        candidate_item=entry.group_slot,
        bbox=bbox,
        blocks=blocks,
...
def build_caption_regions(caption_context: "CaptionContext") -> list[CaptionedRegion]:
    ...
    for entry in entries:
        ...
        back = extend_caption_region(
            caption_context, entry, prior_regions, page_set, -1
        )
        forward = extend_caption_region(
            caption_context, entry, prior_regions, page_set, 1
        )
        winner = (
            back
            if (back is not None and (forward is None or back.score > forward.score))
            else forward
        )
...
class CaptionEntry:
    ...
    def __init__(
        self, label: str, type_: int, page: int, block: Block, remainder: TokenView
    ):
...
def detect_captions(caption_context: CaptionContext) -> None:
    ...
    for entry in iter_page_blocks(caption_context.primary_slot):
        ...
        caption_context.auxiliary_slot.append(
            CaptionEntry(label, type_value, page.page_index, block, remainder)
        )
