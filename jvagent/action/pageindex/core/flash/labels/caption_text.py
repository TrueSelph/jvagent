...
def extract_structural_number(
    tokens: TokenView, other_flag: bool = True
) -> Optional[TokenView]:
...
REFERENCE_PHRASE_TRIE = build_trie(
    ["lists the", "presents", "show the", "showed the", "shows"],
    set_case_fold(TrieConfig(), False),
)
...
def is_uppercase_dominant(tokens: TokenView) -> bool:
    ...
    for reference_item in tokens:
        ...
        if reference_item.primary_slot == 2:
            ...
            if (
                len(reference_item.str) > 4
                and len(reference_item.str) >= 2
                and char_category(reference_item.str[1]) != 2
            ):
                return False
            candidate_item += 1
...
def caption_outranks(
    caption_entry: "CaptionEntry", other_caption_entry: "CaptionEntry"
) -> bool:
    ...
    other_is_uppercase = is_uppercase_dominant(
        tokenize_block(other_caption_entry.group_slot)
    )
    if caption != other_is_uppercase:
        return caption
    caption_first_token = (
        first_token(caption_entry.secondary_slot)
        if caption_entry.secondary_slot.length > 0
        else None
    )
    other_first_token = (
        first_token(other_caption_entry.secondary_slot)
        if other_caption_entry.secondary_slot.length > 0
        else None
    )
    ...
    return (
        caption_entry.group_slot.reading_order_index
        < other_caption_entry.group_slot.reading_order_index
    )
