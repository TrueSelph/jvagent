...
def assign_reading_order(primary_item: PageView, other_items: list) -> None:
    ...
    primary_item.secondary_slot.sort(
        key=lambda sort_block: (
            column_index_of(sort_block),
            -sort_block.top_edge(),
            -sort_block.bottom_edge(),
            sort_block.left_edge(),
            sort_block.right_edge(),
        )
    )
    ...
    for idx in range(len(primary_item.secondary_slot)):
        ...
        reference_item = (
            primary_item.secondary_slot[idx + 1]
            if idx + 1 < len(primary_item.secondary_slot)
            else None
        )
        ...
        if candidate_item.alignment_slot and x_centers_close(
            primary_item.bounds, candidate_item
        ):
            candidate_item.isolated_centered = (
                (not reference_item)
                or (reference_item.top_edge() > candidate_item.bottom_edge())
                or (
                    not left_aligned(candidate_item, reference_item, 1)
                    and not right_aligned(candidate_item, reference_item, 1)
                )
            )
        else:
            candidate_item.isolated_centered = bool(
                candidate_item.alignment_slot
                and reference_item
                and not left_aligned(candidate_item, reference_item, 1)
                and not right_aligned(candidate_item, reference_item, 1)
                and center_aligned(
                    candidate_item, reference_item, candidate_item.bbox_width() / 10
                )
                and x_centers_close(primary_item.bounds, reference_item)
            )
...
def process_page(spans: list[Span], page_num: int, page_bbox: Rect) -> PageView:
    ...
    column_context = ColumnDetectionContext(
        page_bbox, page.primary_slot, container.primary_slot
    )
