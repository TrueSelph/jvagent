...
def init_line_number_cluster(line: Line) -> LineNumberCluster:
    ...
    is_valid_integer = (
        line_number > 0
        and line_number < 1e4
        and not math.isnan(line_number)
...
def nearest_cluster(
    line: LineNumberCluster,
    other_line: Optional[LineNumberCluster],
    candidate_line: Optional[LineNumberCluster],
) -> Optional[LineNumberCluster]:
    ...
    candidate_distance = (
        (candidate_line.left - line.left) if candidate_line is not None else math.inf
    )
...
def validate_line_number_cluster(
    rect: Rect, other_lines: list[Line], candidate_line: LineNumberCluster
) -> bool:
    ...
    if min_gap != 0:
        ...
        gap_ratio = math.nan
    if empty_line_count < len(candidate_line.lines) / 2 and (
        not flag or gap_ratio > 1.3
    ):
        return False
    total = 0.0
    covered = 0.0
...
def strip_line_numbers(rect: Rect, other_lines: list[Line]) -> list[Line]:
    ...
    for line in other_lines:
        ...
        if match is not None:
            if match.is_valid_sequence:
                match.is_valid_sequence = (
                    candidate_cluster.primary_slot == match.primary_slot + 1
                )
    ...
    for cluster in cluster_tree:
        if cluster.is_valid_sequence and (
            best is None or len(cluster.lines) > len(best.lines)
        ):
            best = cluster
