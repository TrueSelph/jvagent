...
def aho_corasick_match(trie: BuiltTrie, tokens) -> Optional[dict]:
    ...
    for entry in enumerate_tokens(tokens):
        ...
        if kb_depth > 0 and (
            earliest_start < 0 or index - kb_depth + 1 <= earliest_start
        ):
...
def _trie_insert_entry(
    builder: TrieBuilder, entry: str, payload: Optional[Any] = None
) -> None:
...
def trie_bulk_insert(
    builder: TrieBuilder, entries, payload: Optional[Any] = None
) -> None:
    ...
    while queue:
        ...
        for child in node.children.values():
            ...
            while trie.failure_link is not None:
                child.failure_link = trie.failure_link.children.get(
                    trie.failure_link.normalize(child.str)
                )
...
def build_trie(
    strings: Iterable[str], other_trie: Optional[TrieConfig] = None
) -> BuiltTrie:
