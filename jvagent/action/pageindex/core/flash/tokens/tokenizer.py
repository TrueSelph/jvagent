...
class LineTokenizer:
    ...
    def _advance_char(self, other_text: str, candidate_number: int) -> None:
        ...
        if (
            not self.primary_slot
            and len(self.tertiary_slot) >= 2
            and reference_item == 3
        ):
    ...
    def add_line(self, other_line: Line) -> "LineTokenizer":
        ...
        if self.primary_slot:
            ...
            self._close_token(
                self.primary_slot != "-" or line is None or line.boundary_slot
            )
        ...
        for index in range(len(other_line.primary_slot)):
            ...
            if (
                ...
                and span.char_stats.secondary_slot == 5
            ):
                pending = span
                continue
            ...
            if self.secondary_slot is not None and self.primary_slot:
                ...
                if (
                    span.left_edge()
                    <= self.secondary_slot.right_edge()
                    + 0.1 * avg_char_width2(self.secondary_slot)
                    and (
                        abs(span.bottom_edge() - self.secondary_slot.bottom_edge())
                        < 0.1
                    ...
                    gap_tolerance = (
                        GAP_TOLERANCE_GRID[
                            self.secondary_slot.char_stats.tertiary_slot
                        ][span.char_stats.secondary_slot]
                        or 0.12
                    ) * avg_char_width(self.cache_slot)
                    close = (
                        self.previous_slot
                        or abs(self.secondary_slot.bottom_edge() - span.bottom_edge())
                        > 1
                        or span.left_edge() < self.secondary_slot.right_edge() - 1
                        or span.left_edge()
                        > self.secondary_slot.right_edge() + gap_tolerance
                    )
            ...
            for char_index in range(len(span.text)):
                ...
                if (
                    ...
                    combined = unicodedata.normalize(
                        "NFC", char_value + pending.state_slot[0]
                    )
...
def is_superscript_adjacent(token: Token, other_token: Token) -> bool:
    ...
    return (
        ...
        and reference_item.bottom_edge()
        > candidate_item.bottom_edge() + 0.1 * candidate_item.bbox_height()
    )
