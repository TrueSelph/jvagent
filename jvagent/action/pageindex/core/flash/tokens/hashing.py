...
def _little_endian_signed_word(byte_values: list, off: int) -> int:
    ...
    return (
        _sign_extend_byte(byte_values[off])
        + (_sign_extend_byte(byte_values[off + 1]) << 8)
        + (_sign_extend_byte(byte_values[off + 2]) << 16)
        + (_sign_extend_byte(byte_values[off + 3]) << 24)
    )
...
def _jenkins_mix(mix_state: list) -> int:
    ...
    secondary_item = _int32_xor(
        secondary_item - candidate_item - reference_item,
        _uint32_right_shift(reference_item, 13),
    )
    candidate_item = _int32_xor(
        candidate_item - reference_item - secondary_item,
        _int32_left_shift(secondary_item, 8),
    )
    reference_item = reference_item - secondary_item
    reference_item = _int32_xor(
        reference_item - candidate_item, _uint32_right_shift(candidate_item, 13)
    )
    ...
    candidate_item = _int32_xor(
        candidate_item - reference_item - secondary_item,
        _int32_left_shift(secondary_item, 16),
    )
    reference_item = reference_item - secondary_item
    reference_item = _int32_xor(
        reference_item - candidate_item, _uint32_right_shift(candidate_item, 5)
    )
    secondary_item = secondary_item - candidate_item
    secondary_item = secondary_item - reference_item
    ...
    candidate_item = _int32_xor(
        candidate_item - reference_item - secondary_item,
        _int32_left_shift(secondary_item, 10),
    )
    reference_item = reference_item - secondary_item
    reference_item = _int32_xor(
        reference_item - candidate_item, _uint32_right_shift(candidate_item, 15)
    )
    mix_state[0], mix_state[1], mix_state[2] = (
        secondary_item,
        candidate_item,
        reference_item,
    )
