...
def is_cover_like_page(doc, page) -> bool:
    ...
    return (
        page.page_index < early_limit
        and page.primary_slot.secondary_slot < 0.8 * threshold
    )
...
def score_title_candidate(zp_state, page, index: int) -> None:
        ...
        if (
            ...
            if (
                (
                    ...
                    and align_value in (1, 2)
                    and next_alignment in (1, 2)
                )
                or (
                    right_aligned(title_block, next_item, tolerance)
                    and align_value in (1, 4)
                    and next_alignment in (1, 4)
                )
                or (
                    center_aligned(title_block, next_item, tolerance)
                    and title_block.alignment_slot
                    and next_item.alignment_slot
                )
            ):
    ...
    for result_value in group:
        ...
        for entry in enumerate_tokens(title_tokens_view):
            ...
            if is_word_token(sample_item):
                ...
                if (
                    sample_item.str == "@"
                    and entry["index"] + 3 < title_tokens_view.length
                ):
    ...
    width_ratio_sq = (
        (max_width / page.bounds.bbox_width())
        if page.bounds.bbox_width()
        else (math.inf if max_width > 0 else math.nan)
    )
    ...
    page_pos = max(
        0.1, 1 - 2 * (page.page_index - 1) / max(1, len(doc_state.primary_slot))
    )
    ...
    page_density_ratio = page.primary_slot.secondary_slot / max(
        1e-6, doc_state.secondary_slot.secondary_slot
    )
    density_factor = max(0.5, 1 - page_density_ratio * page_density_ratio) * (
        1 + clamp_value((0.25 - page_density_ratio) / 0.15, 0, 1)
    )
    # Page top/height is positive in normal inputs. Degenerate pages take the
    # minimum top-position factor instead of raising.

    top = (
        max(0.1, group[0].top_edge() / page.bounds.top_edge())
        if page.bounds.top_edge()
        else 0.1
    )
    ...
    for block in group:
        ...
        for token in tokens:
            if (
                ...
                and len(token.str) <= 1
                and is_superscript_adjacent(previous, token)
            ):
                abbrev += 1
            previous = token
    ...
    recurrence_count = (
        doc_state.tertiary_slot.get(norm_text, 0)
        if hasattr(doc_state, "tertiary_slot")
        and isinstance(doc_state.tertiary_slot, dict)
        else 0
    )
    ratio = recurrence_count / max(1, len(doc_state.primary_slot))
    adj = total_tokens - 3
    recurrence_factor = 1 - 0.5 * clamp_value(ratio / 0.3, 0, 1) * (
        1 / max(1, adj * adj)
    )
    ...
    if previous_block is not None:
        ...
        if (
            ...
            and trie_prefix_match(TITLE_LABEL_TRIE, prev_tokens) is not None
        ):
            label = 3.0
    ...
    script = (
        1.0
        if dominant_script_family(script_acc) == doc_state.secondary_slot.tertiary_slot
        else 0.5
    )
