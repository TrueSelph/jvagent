    ...
    for page in doc.primary_slot:
        ...
        if (
            ...
            for idx, block in enumerate(page.secondary_slot):
                if (
                    is_title_candidate_block(block)
                    and id(block) not in state.primary_slot
                    and heading_score(block) > page.primary_slot.primary_slot - 0.1
                ):
                    score_title_candidate(state, page, idx)
            break

        if is_cover_like_page(doc, page) or (
            page.page_index <= 1
            and len(doc.primary_slot) >= 10
            and page.primary_slot.secondary_slot
            < 0.8 * doc.secondary_slot.secondary_slot
        ):
            # Cover / front-matter page
            for idx, block in enumerate(page.secondary_slot):
                if (
                    not is_title_candidate_block(block)
                    or id(block) in state.primary_slot
                ):
                    continue
                score = heading_score(block)
                if (
                    (
                        score > doc.secondary_slot.primary_slot + 0.1
                        and score > page.primary_slot.primary_slot + 0.1
                    )
                    or (
                        score > doc.secondary_slot.primary_slot + 2
                        and score > page.primary_slot.primary_slot - 0.1
                    )
                    or (
                        score > doc.secondary_slot.primary_slot - 0.1
                        and score > page.primary_slot.primary_slot - 0.1
                        and block.isolated_centered
                    )
                    or (
                        score > doc.secondary_slot.primary_slot - 0.1
                        and score > page.primary_slot.primary_slot - 0.1
            ...
            for idx, block in enumerate(page.secondary_slot):
                ...
                size_trigger = is_title_candidate_block(block) and (
                    (
                        score > doc.secondary_slot.primary_slot + 0.1
                        and score > page.primary_slot.primary_slot + 0.1
                    )
                    ...
                    or (
                        block.isolated_centered
                        and score > page.primary_slot.primary_slot - 0.1
                    )
                    or (
                        page.page_index == 1
                        and score > page.primary_slot.primary_slot + 2
                    )
                )
                if size_trigger:
                    score_title_candidate(state, page, idx)
                    ...
                    if not has_seen_da:
                        if (
                            block.bottom_edge() - page.bounds.bottom_edge()
                            < 2 * page.bounds.bbox_height() / 3
                        ):
                            has_seen_da = False
