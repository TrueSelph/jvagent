...
class Rect(RectLike):
    ...
    def bottom_edge(self) -> float:
        return self.primary_slot  # bottom

    def bbox_width(self) -> float:
        return _max_nan_propagating(0.0, self.right - self.left)  # width

    def bbox_height(self) -> float:
        return _max_nan_propagating(0.0, self.top - self.primary_slot)  # height

    def area(self) -> float:
        return self.bbox_width() * self.bbox_height()  # area

    def center_x(self) -> float:
        return (self.left + self.right) / 2  # x-center

    def center_y(self) -> float:
        return (self.top + self.primary_slot) / 2  # y-center
...
class Bounded(RectLike):
    ...
    def left_edge(self) -> float:
        return self.secondary_slot.left

    def right_edge(self) -> float:
        return self.secondary_slot.right

    def top_edge(self) -> float:
        return self.secondary_slot.top

    def bottom_edge(self) -> float:
        return self.secondary_slot.primary_slot

    def bbox_width(self) -> float:
        return self.secondary_slot.bbox_width()

    def bbox_height(self) -> float:
        return self.secondary_slot.bbox_height()

    def area(self) -> float:
        return self.secondary_slot.area()

    def center_x(self) -> float:
        return self.secondary_slot.center_x()

    def center_y(self) -> float:
        return self.secondary_slot.center_y()
...
def extend_top_to(rect: Rect, other_item: float) -> Rect:
    ...
    return Rect(
        rect.left,
        rect.right,
        _max_nan_propagating(rect.top, other_item),
        rect.primary_slot,
    )
...
def extend_bottom_to(rect: Rect, other_item: float) -> Rect:
    ...
    return Rect(
        ...
        rect.right,
        rect.top,
        _min_nan_propagating(rect.primary_slot, other_item),
    )
...
def left_edge_key(primary_item: Bounded) -> tuple:
    return (
        primary_item.left_edge(),
        primary_item.right_edge(),
        -primary_item.top_edge(),
        -primary_item.bottom_edge(),
    )
...
def same_x_extent(
    primary_item: Bounded, secondary_item: Bounded, candidate_item: float
) -> bool:
    """Return whether both horizontal edges are within the tolerance."""
    return (
        abs(primary_item.left_edge() - secondary_item.left_edge()) <= candidate_item
        and abs(primary_item.right_edge() - secondary_item.right_edge())
        <= candidate_item
    )
...
def same_y_extent(
    ...
    return (
        abs(primary_item.top_edge() - secondary_item.top_edge()) <= candidate_item
        and abs(primary_item.bottom_edge() - secondary_item.bottom_edge())
        <= candidate_item
    )


def intervals_overlap(
    value: float, other_item: float, candidate_item: float, reference_item: float
) -> bool:
    """Return whether the two closed ranges overlap by either endpoint."""
    return (value <= candidate_item and candidate_item <= other_item) or (
        candidate_item <= value and value <= reference_item
    )
...
def right_aligned(
    ...
    return (
        abs(primary_item.right_edge() - secondary_item.right_edge()) <= candidate_item
    )
...
def center_aligned(
    ...
    if sign(reference_item) != -sign(entry_item):
        return False
    return abs(primary_item.center_x() - secondary_item.center_x()) <= max(
        candidate_item, min(abs(reference_item), abs(entry_item)) / 2
    )
...
def x_aligned(
    ...
    return (
        left_aligned(primary_item, secondary_item, candidate_item)
        or right_aligned(primary_item, secondary_item, candidate_item)
        or center_aligned(primary_item, secondary_item, candidate_item)
    )
...
def x_centers_close(primary_item: Bounded, secondary_item: Bounded) -> bool:
    ...
    return abs(secondary_item.center_x() - primary_item.center_x()) <= max(
        1, secondary_item.bbox_width() / 10
    )
