...
_NUMBERING_PREFIX_RE = regex_module.compile(
    r"^(?:"
    r"([IVX]+|[1-9１-９]\p{Number}?)(?:[.．｡。):]|-\P{Number}|-$|["
    + _UNICODE_WHITESPACE_CLASS
    + r"]|$)"
    r"|(?:([A-Ha-h])|([ivx]))[.．｡。)]"
    r")"
...
def _detect_numbering(line: Line) -> None:
    ...
    if len(line.primary_slot) > 1:
        ...
        candidate_item = (
            line.primary_slot[2]
            if (line.primary_slot[1].char_count() <= 0 and len(line.primary_slot) > 2)
            else line.primary_slot[1]
        )
        if (
    ...
    if (
        measure_item
        and measure_item.group(1)
        and "1" <= measure_item.group(1)[0] <= "9"
    ):
        line.state_slot = 1
