...
def merge_char_stats(char_stats: CharStats, other_char_stats: CharStats) -> None:
    ...
    for candidate_item in range(12):
        char_stats.primary_slot[candidate_item] += other_char_stats.primary_slot[
            candidate_item
        ]
...
def letter_count(char_stats: CharStats) -> int:
    ...
    return (
        char_stats.primary_slot[3]
        + char_stats.primary_slot[2]
        + char_stats.primary_slot[4]
    )
...
def punct_count(char_stats: CharStats) -> int:
    ...
    return (
        char_stats.primary_slot[6]
        + char_stats.primary_slot[7]
        + char_stats.primary_slot[8]
    )
...
def info_weight(char_stats: CharStats) -> float:
    ...
    return (
        char_stats.primary_slot[3]
        + char_stats.primary_slot[2]
        + 2 * char_stats.primary_slot[4]
        + 0.5 * (char_stats.auxiliary_slot - letter_count(char_stats))
    )
...
def is_upper_dominant(char_stats: CharStats) -> bool:
    ...
    return secondary_item > max(
        candidate_item * 3 / 4, candidate_item - 4
    ) and secondary_item > max(3, char_stats.auxiliary_slot / 3)
