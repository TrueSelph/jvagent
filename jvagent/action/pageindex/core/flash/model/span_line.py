...
_italic_font_re = re.compile(
    r"(ital|it\Z|i[1-9][0-9]*\Z|obliq)", re.IGNORECASE | re.ASCII
)
...
def append_span(line: Line, other_span: Span) -> Line:
    ...
    if total_weight > 0:
        line.weighted_ratio_primary = (
            line.weighted_ratio_primary * span
            + (1 if other_span.primary_slot else 0) * added_weight
        ) / total_weight
        line.weighted_ratio_secondary = (
            line.weighted_ratio_secondary * span
            + (1 if other_span.measure_slot else 0) * added_weight
        ) / total_weight
        line.weighted_ratio_tertiary = (
            line.weighted_ratio_tertiary * span
            + other_span.previous_slot * added_weight
        ) / total_weight
        line.metric_slot = (
            line.metric_slot * span + other_span.font_size * added_weight
        ) / total_weight
    ...
    line.cache_slot = min(
        1.0, (line.cache_slot * span + other_span.area()) / max(1.0, line.area())
    )
...
def avg_char_width(primary_item: _HasCharCount) -> float:
    ...
    return (
        0.0
        if primary_item.char_count() <= 0
        else primary_item.bbox_width() / primary_item.char_count()
    )
