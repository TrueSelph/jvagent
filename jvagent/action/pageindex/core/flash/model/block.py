...
class Block(Bounded):
    ...
    def add_line(self, other_line) -> "Block":
        ...
        self.alignment_slot = self.alignment_slot and (
            len(self.primary_slot) <= 0 or center_aligned(self, other_line, 1)
        )
        ...
        if total_weight > 0:
            self.weighted_ratio_tertiary = (
                self.weighted_ratio_tertiary * line
                + other_line.bold_frac() * added_weight
            ) / total_weight
            self.previous_slot = (
                self.previous_slot * line
                + other_line.weighted_ratio_secondary * added_weight
            ) / total_weight
            self.weighted_skew = (
                self.weighted_skew * line + other_line.skew_frac() * added_weight
            ) / total_weight
            self.weighted_font_size = (
                self.weighted_font_size * line
                + other_line.avg_font_size() * added_weight
            ) / total_weight
            self.weighted_ratio_primary = (
                self.weighted_ratio_primary * line
                + other_line.cache_slot * added_weight
            ) / total_weight
        ...
        if added_weight > 0:
            self.weighted_ratio_secondary = (
                self.weighted_ratio_secondary * line
                + other_line.cache_slot * other_line.area()
            ) / added_weight
        for span in other_line:
            sty = style_key(span)
            self.style_char_counts[sty] = (
                self.style_char_counts.get(sty, 0) + span.char_count()
            )
            ...
            self.size_char_counts[size_key] = (
                self.size_char_counts.get(size_key, 0) + span.char_count()
            )
        # invalidate caches
        self.dominant_style_cache = self.dominant_size_cache = self.token_text_cache = (
            self.deaccented_text_cache
        ) = self.cache_slot = self.tokens_cache = None
...
def is_caps_heavy(primary_item) -> bool:
    ...
    return is_upper_dominant(
        primary_item.char_stats
    ) or primary_item.char_stats.primary_slot[2] >= max(
        2, primary_item.char_stats.auxiliary_slot
    )
...
def heading_score(heading) -> float:
    ...
    return (
        dominant_font_size(heading)
        + (2 if is_caps_heavy(heading) else 0)
        + (1 if heading.weighted_ratio_tertiary > 0.5 else 0)
    )
...
def case_signal(char_stats: CharStats) -> int:
    ...
    if (
        is_upper_dominant(char_stats)
        and not is_punct_category(char_stats.secondary_slot)
        and letter_count(char_stats) > 3 * char_stats.auxiliary_slot / 4
        and punct_count(char_stats) < 5
    ):
        return 1
