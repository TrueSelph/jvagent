...
def _xf_point(
    items: tuple, other_item: float, candidate_item: float
) -> tuple[float, float]:
    """Apply an (a,b,c,d,e,f) PDF matrix to a point (row-vector convention)."""
    return (
        items[0] * other_item + items[2] * candidate_item + items[4],
        items[1] * other_item + items[3] * candidate_item + items[5],
    )
...
def _compose_mtx(first_matrix: tuple, second_matrix: tuple) -> tuple:
    ...
    return (
        ...
        first_matrix[4] * second_matrix[0]
        + first_matrix[5] * second_matrix[2]
        + second_matrix[4],
        first_matrix[4] * second_matrix[1]
        + first_matrix[5] * second_matrix[3]
        + second_matrix[5],
    )
...
def _collect_text_objs(page, text_page) -> list[dict]:
    ...
    def iter_text_objs(parent, anc_mtx, depth):
        ...
        object_count = (
            pdfium_c.FPDFFormObj_CountObjects(parent)
            if parent is not None
            else pdfium_c.FPDFPage_CountObjects(page.raw)
        )
        for text in range(object_count):
            raw = (
                pdfium_c.FPDFFormObj_GetObject(parent, text)
                if parent is not None
            ...
            if typ == pdfium_c.FPDF_PAGEOBJ_TEXT:
                ...
                yield from iter_text_objs(
                    raw, _compose_mtx(font_matrix, anc_mtx), depth + 1
                )
    ...
    for raw_obj, anc_mtx in iter_text_objs(None, _IDENT_MTX, 0):
        ...
        if not pdfium_c.FPDFPageObj_GetBounds(
            raw_obj,
            ctypes.byref(bounds_left),
            ctypes.byref(value),
            ctypes.byref(bounds_right),
            ctypes.byref(bounds_top),
        ):
            continue
...
def _char_render_fs(text_page, char_idx: int) -> float:
    ...
    if pdfium_c.FPDFText_GetMatrix(text_page, char_idx, ctypes.byref(current_matrix)):
        return math.sqrt(
            current_matrix.c * current_matrix.c + current_matrix.d * current_matrix.d
        )
    return 0.0


def _find_obj_for_char(
    obj_index: dict[int, list[dict]],
    query_origin_x: float,
    query_origin_y: float,
    tol: float = 1.0,
    char_fs: float | None = None,
    text_page=None,
    ...
    for item_value in obj_index.get(int(round(query_origin_y)), ()):
        if (item_value["l"] - tol) <= query_origin_x <= (item_value["r"] + tol) and (
            item_value["b"] - tol
        ) <= query_origin_y <= (item_value["t"] + tol):
            if first is None:
                first = item_value
            elif cands is None:
    ...
    if cands is None:
        return first
    char_render = (
        _char_render_fs(text_page, char_idx)
        if text_page is not None and char_idx is not None
        else 0.0
    )
    if char_render > 0:
        ...
        return min(
            cands,
            key=lambda item_value: (
                abs(item_value["fs_eff"] - char_render),
                item_value["area"],
            ),
        )
    ...
    if char_fs is not None and char_fs > 0:
        ...
        return min(
            cands,
            key=lambda item_value: (
                abs(item_value["fs_raw"] - char_fs) / max(char_fs, 0.01),
                item_value["area"],
            ),
        )
