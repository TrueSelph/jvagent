...
_NORMALIZED_UNICODES: dict[str, str] = json.loads(
    (Path(__file__).parent.parent / "data" / "normalized_unicodes.json").read_text(
        encoding="utf-8"
    )
)
...
def _normalize_unicodes(text: str) -> str:
    ...
    if not any(candidate_item in unit_count for candidate_item in text):
        return text
    return "".join(
        unit_count.get(candidate_item, candidate_item) for candidate_item in text
    )
...
def _is_zero_width_diacritic(number: int) -> bool:
    ...
    return (
        number not in _WHITESPACE_CODEPOINTS
        and unicodedata.category(chr(number)) == "Mn"
    )
...
def _is_invisible_format_mark(number: int) -> bool:
    ...
    return (
        number not in _WHITESPACE_CODEPOINTS
        and unicodedata.category(chr(number)) == "Cf"
    )
...
def _apply_bidi_reordering(
    text: str, start_level: int = -1, vertical: bool = False
) -> str:
    ...
    for index_value in range(1, count_item - 1):
        if (
            types[index_value] == "ES"
            and types[index_value - 1] == "EN"
            and types[index_value + 1] == "EN"
        ):
            types[index_value] = "EN"
        if (
            types[index_value] == "CS"
            and types[index_value - 1] in ("EN", "AN")
