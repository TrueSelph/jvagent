        ...
        if byte_value in _PDF_WHITESPACE_BYTES:
            ...
            while index_value < count_item:
                ...
                if literal_byte == 0x5C:  # backslash escape
                    ...
                    if escape_byte in _PDF_STRING_ESCAPE_BYTES:
                        ...
                        val = 0
                        while (
                            token_end < count_item
                            and token_end - index_value <= 3
                            and 0x30 <= content_bytes[token_end] <= 0x37
                        ):
                        ...
                        if (
                            ...
                            and content_bytes[index_value] == 0x0A
                        ):
                            index_value += 1
                    else:  # \x -> x
            ...
            if index_value + 1 < count_item and content_bytes[index_value + 1] == 0x3C:
                ...
                push(
                    "str",
                    tuple(
                        (nib[key_value] << 4) | nib[key_value + 1]
                        for key_value in range(0, len(nib), 2)
                    ),
                )
            ...
            byte_value == 0x5B
        ):  # [ -- one array operand (text extraction parser builds an array)
            frames.append(("arr", []))
            index_value += 1
            ...
            if frames and frames[-1][0] == "arr":
                ...
                push(
                    ...
                    tuple(
                        codepoint
                        for kerning_delta, vertical_value in items
                        if kerning_delta == "str"
                        for codepoint in vertical_value
                    ),
            ...
            while (
                token_end < count_item
                and content_bytes[token_end] not in _PDF_WHITESPACE_BYTES
                and content_bytes[token_end] not in _PDF_DELIMITER_BYTES
            ):
                token_end += 1
            ...
            if (0x30 <= first_char <= 0x39) or first_char in b"+-.":
                ...
                while (
                    ...
                    if (
                        known
                        and cand not in _OP_OPERAND_COUNTS
                        and cand not in _OP_LEX_PREFIX
                    ):
                        break
                    token_end += 1
                    known = (
                        content_bytes[index_value:token_end] in _OP_OPERAND_COUNTS
            ...
            if (
                operator_token == b"ID"
            ):  # inline image data (text extraction inline-image parser)
                ...
                while key_value < count_item - 1:
                    if (
                        content_bytes[key_value] == 0x45
                        and content_bytes[key_value + 1] == 0x49
                        and (
                            key_value + 2 >= count_item
                            or content_bytes[key_value + 2] in b" \n\r"
                        )
                    ):
...
def _page_vertical_resource_names(pdf_doc, page_idx: int) -> set[bytes]:
    ...
    try:
        for rec in pdf_doc[page_idx].get_fonts(full=True):
            ...
            if page == "xref":
                width_type, width_value_local = pdf_doc.xref_get_key(
                    int(resource_names.split()[0]), "WMode"
                )
