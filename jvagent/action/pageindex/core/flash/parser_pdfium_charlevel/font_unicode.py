...
def _type1_builtin_encoding(font_file: bytes):
    ...
    while index_value < count_item:
        ...
        if candidate_item in _TYPE1_WHITESPACE_BYTES:
            ...
            candidate_item == 0x25
        ):  # % comment runs to EOL (PDF token reader's comment eater)
            ...
            while (
                state_item < count_item
                and head[state_item] not in _TYPE1_WHITESPACE_BYTES
                and head[state_item] not in _TYPE1_SPECIAL_BYTES
            ):
                state_item += 1
    ...
    while page_value < len(header_tokens):
        ...
        page_value += (
            2  # the name scanner advances past the slash unconditionally after a '/'
        )
        ...
        size = (
            0
            if array_size_float == float("inf")
            else ((int(array_size_float) + 2**31) % 2**32) - 2**31
        )
        ...
        for _ in range(size):
            ...
            try:
                value = (
                    _parse_int(token_value.decode("latin-1"), 10)
...
def _simple_font_to_unicode(
    ...
    encoding: dict[int, str] = {
        font: glyph_name_value for font, glyph_name_value in enumerate(default_enc)
    }
    ...
    for charcode in sorted(encoding):
        ...
        if glyph_prefix == "G":  # Gxx
            if len(glyph_name) == 3:
                ...
                code = (
                    int(parsed_integer) if parsed_integer == parsed_integer else 0
                )  # pi==pi: not NaN
...
def _font_unicode_map(pdf_doc, xref: int) -> tuple[int, dict[int, str]] | None:
    ...
    if subtype == "Type0":
        ...
        if pdf_value_type != "name" or pdf_value.lstrip("/") not in (
            "Identity-H",
            "Identity-V",
        ):
            return None
        ...
        if delta_top == "xref":
            delta_value = pdf_doc.xref_object(
                int(delta_value.split()[0]), compressed=True
            )
            delta_top = "array"
        if delta_top == "array":
        ...
        if desc_xref:
            right_type, right_value_local = _xref_key(
                desc_xref, "CIDSystemInfo/Registry"
            )
            other_type, other_value_local = _xref_key(
                desc_xref, "CIDSystemInfo/Ordering"
            )
            reg = (
                re.sub(r"[()\s]", "", right_value_local) if right_type != "null" else ""
            )
            ordering = (
                re.sub(r"[()\s]", "", other_value_local) if other_type != "null" else ""
            )
    ...
    if enc_obj is not None:
        ...
        if flags_match:
            ...
            for token_match in re.findall(
                r"/([^\s/\[\]<>()]+)|(\d+)", enc_obj[flags_match.end() : scan_index - 1]
            ):
                if token_match[1]:
    ...
    if pdf_value_type == "xref":
        try:
            included = _parse_tounicode_cmap(
                pdf_doc.xref_stream(int(pdf_value.split()[0]))
            )
        except Exception:
    ...
    if included:
        ...
        if (
            has_encoding
        ):  # predefined collection Unicode-map construction -> fallback Unicode map gap fill
            for font, glyph_name in _simple_font_to_unicode(
                default_enc, base_encoding_name, differences
            ).items():
    ...
    if (
        file_key is not None
        and file_key[0] == "FontFile"
        and subtype in ("Type1", "MMType1")
    ):
        try:
