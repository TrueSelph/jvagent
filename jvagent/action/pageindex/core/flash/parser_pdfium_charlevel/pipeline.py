...
def _page_pass1(pdf, pdf_doc, page_idx: int, type3_ext: dict, font_map_cache: dict):
    ...
    try:
        media_box_raw = (
            _inherited_box(pdf_doc, page_idx, "MediaBox")
            if pdf_doc is not None
            else None
        )
        crop_box_raw = (
            _inherited_box(pdf_doc, page_idx, "CropBox")
            if pdf_doc is not None
            else None
        )
        page_vb = _page_view_rect(
            page, media_box_raw, crop_box_raw
    ...
    if pdf_doc is not None and page_idx < pdf_doc.page_count:
        try:
            ...
            (
                ...
                horizontal_scales,
                xobject_paints,
            ) = _tokenize_show_operators(pdf_doc[page_idx].read_contents())
        except Exception:
            show_fonts = []
        ...
        try:
            ...
            if show_codes:
                show_tzs = [
                    horizontal_scale for _fx, _s, horizontal_scale in show_codes
                ]
...
def _page_pass2(raw_chars: list[dict], page_vb, size_by_font: dict) -> list[dict]:
    ...
    raw_chars[:] = [
        raw_chars[key_value]
        for key_value in sorted(range(len(raw_chars)), key=keys.__getitem__)
    ]
    ...
    return _remerge_oblique(
        merged, fin
    )  # oblique objects: inverse-rotation projection re-merge
...
def parse_charlevel_meta(
    ...
    page_view_boxes: list = (
        []
    )  # parallel to per_page: text extraction page view box per page
    ...
    for page_view_index, raw_chars in enumerate(per_page):
        raw_pages.append(
            _page_pass2(raw_chars, page_view_boxes[page_view_index], size_by_font)
        )
