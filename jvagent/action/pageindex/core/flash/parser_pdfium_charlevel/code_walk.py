...
def _resource_dict_xrefs(pdf_doc, owner_xref: int, sub: str) -> dict[bytes, int]:
    ...
    for measure_item in re.finditer(r"/([^\s/\[\]<>()]+)\s+(\d+)\s+\d+\s+R", body):
        out[_decode_pdf_name(measure_item.group(1).encode("latin-1"))] = int(
            measure_item.group(2)
        )
    ...
    try:
        ...
        for part in ("Resources", sub):
            ...
            node = (
                node["/" + part] if (node is not None and "/" + part in node) else None
            )
        if node is not None:
            if isinstance(node, PdfIndirectRef):
                node = node.get_object()
...
def _page_show_codes(
    ...
    def walk(
        stream: bytes,
        fonts_res: dict[bytes, int],
        xobjs_res: dict[bytes, int],
        cur_font: int | None,
        cur_tz: float,
        visited: frozenset,
        depth: int,
        ...
        if depth > 8:
            return
        flush_ids, fonts, show_text_units, horizontal_scales, xobject_paints = (
            _tokenize_show_operators(stream, cur_tz)
        )
        dict_index = 0
        for key_value in range(len(show_text_units) + 1):
            while (
                dict_index < len(xobject_paints)
                and xobject_paints[dict_index][0] == key_value
            ):
                ...
                if xobject_ref is None or xobject_ref in visited:
                    continue
                state_values, string_value = pdf_doc.xref_get_key(
                    xobject_ref, "Subtype"
                )
                if state_values != "name" or string_value.lstrip("/") != "Form":
                    continue
                sub_fonts = (
                    _resource_dict_xrefs(pdf_doc, xobject_ref, "Font") or fonts_res
                )
                sub_xobjs = (
                    _resource_dict_xrefs(pdf_doc, xobject_ref, "XObject") or xobjs_res
                )
                inherited = (
                    fonts_res.get(font_at_do) if font_at_do is not None else None
                )
                try:
            ...
            if key_value < len(show_text_units):
                ...
                resource_font_index = (
                    fonts_res.get(resource_font_name)
                    if resource_font_name is not None
                    else cur_font
                )
                out.append(
                    (
                        resource_font_index,
                        show_text_units[key_value],
                        horizontal_scales[key_value],
                    )
                )
    ...
    try:
        ...
        walk(
            ...
            frozenset(),
            0,
            out,  # the initial text state starts at scale 1
        )
        return out
    except Exception:
        return None
...
def _walk_codes(
    ...
    tuple[
        list[tuple[int, str]], list[int], list[tuple[int, int]], list[tuple[int, int]]
    ]
    | None
):
    ...
    for index_value, token_value in enumerate(targets):
        ...
        if text[pos : pos + target_len] == token_value:
            consumed.extend(
                (chars[query_value][0], index_value)
                for query_value in range(pos, pos + target_len)
            )
            pos += target_len
            shift_run = 0
            continue
        ...
        for normalized_text in (
            _normalize_unicodes(token_value),
            unicodedata.normalize("NFKC", token_value),
            unicodedata.normalize("NFKD", token_value),
            unicodedata.normalize("NFD", token_value),
        ):
            if (
                normalized_text != token_value
                and text[pos : pos + len(normalized_text)] == normalized_text
            ):
                if _normalize_unicodes(token_value) != normalized_text:
                    patches.append((chars[pos][0], token_value))
                    drops.extend(
                        chars[query_value][0]
                        for query_value in range(pos + 1, pos + len(normalized_text))
                    )
                consumed.extend(
                    (chars[query_value][0], index_value)
                    for query_value in range(pos, pos + len(normalized_text))
                )
        ...
        if allow_skips:
            ...
            for skip_len in range(1, len(targets) - index_value + 1):
                ...
                if not anchor or not all(
                    len(primary_item) == 1 for primary_item in anchor
                ):
                    break
            ...
            if skip_run_length:
                skips.extend(
                    (query_value, pos)
                    for query_value in range(index_value, index_value + skip_run_length)
                )
        ...
        if all(
            _is_whitespace(ord(unit_char)) for unit_char in token_value
        ...
        if (nxt is not None and text[pos : pos + len(nxt)] == nxt) or text[
            pos + 1 : pos + 1 + target_len
        ] == token_value:
            shift_run += 1
            if shift_run >= 2:
                return None
