...
def _extract_raw_chars(page, text_page) -> tuple[list[dict], list[dict]]:
    ...
    char_left_box = ctypes.c_double(0)
    char_right_box = ctypes.c_double(0)
    value = ctypes.c_double(0)
    char_top_box = ctypes.c_double(0)
    loose_box = pdfium_c.FS_RECTF(0, 0, 0, 0)
    u32 = ctypes.c_uint32(0)
    fs32 = ctypes.c_float(0)
    byref = ctypes.byref
    ox_ref = byref(char_origin_x)
    oy_ref = byref(char_origin_y)
    l_ref = byref(char_left_box)
    r_ref = byref(char_right_box)
    b_ref = byref(value)
    t_ref = byref(char_top_box)
    ...
    for index_value in range(count_item):
        ...
        if is_gen and not is_ws:
            continue
        char_origin_x.value = 0.0
        char_origin_y.value = 0.0
        get_char_origin(text_page, index_value, ox_ref, oy_ref)
        ox_v = char_origin_x.value
        oy_v = char_origin_y.value
        ...
        # containment.
        char_left_box.value = 0.0
        char_right_box.value = 0.0
        value.value = 0.0
        char_top_box.value = 0.0
        get_char_box(text_page, index_value, l_ref, r_ref, b_ref, t_ref)
        char_left, char_right, char_top, char_bottom = (
            char_left_box.value,
            char_right_box.value,
            char_top_box.value,
            value.value,
        )
        ...
        if (
            get_loose_box(text_page, index_value, loose_ref)
        ...
        if name > 1:
            ...
            if char_font_name is None:
                char_font_name = raw_name.decode("latin-1", errors="replace").rstrip(
                    "\x00"
                )
...
def _accumulate_type3_extents(raw_chars: list[dict], acc: dict) -> None:
    ...
    for candidate_item in raw_chars:
        ...
        if (
            item_value["fs_raw"] >= 1.5
            or item_value["scale_y"] >= 1.5
            or candidate_item["is_ws"]
        ):
            continue
...
def _finalize_chars(raw_chars: list[dict]) -> list[dict]:
    ...
    for key_value, candidate_item in enumerate(raw_chars):
        ...
        reference_item = {
            ...
            "ox": candidate_item["ox"],
            "oy": candidate_item["oy"],
            "glyph_w": glyph_w,
            "fs": obj["fs_eff"],
            "fs_x": (
                obj["fs_raw"] * obj["scale_x"] if obj["scale_x"] > 0 else obj["fs_eff"]
            ),
            ...
            "left": candidate_item["ox"],
            "right": candidate_item["right"],
            "top": candidate_item["top"],
            "bottom": candidate_item["bottom"],
        ...
        if obj.get("vertical"):
            ...
            reference_item["v_pen_x"] = (
                candidate_item["left"] + candidate_item["right"]
            ) / 2.0
            reference_item["v_pen_y"] = pen_y
            # pen y after this glyph's advance (text extraction previous glyph transform[5])
            reference_item["v_after"] = min(
                candidate_item["cell_top"], candidate_item["cell_bot"]
            )
        ...
        for _ in range(32):
            ...
            if token_value != "null":
                ...
                try:
                    ...
                    return None
            parent_key_type, position_value = pdf_doc.xref_get_key(
                xref_cursor, "Parent"
            )
...
def _page_view_rect(
    page, med_raw=None, crop_raw=None
    ...
    def norm(secondary_item):
        ...
        count_item = (
            min(box_x_min, box_x_max),
            min(box_y_min, box_y_max),
            max(box_x_min, box_x_max),
            max(box_y_min, box_y_max),
        )
        return (
            count_item
            if (count_item[2] - count_item[0] > 0 and count_item[3] - count_item[1] > 0)
            else None
        )
...
def _off_page(mapping: dict, view_box) -> bool:
    ...
    return (
        origin_offset_x < 0
        or origin_offset_x > view_box[2]
        or origin_offset_y < 0
        or origin_offset_y > view_box[3]
    )
