...
def _merge_text_items(chars: list[dict], view_box=None) -> list[dict]:
    ...
    def open_chunk(mapping: dict) -> None:
        ...
        chunk = {
            ...
            "flush_id": _object_merge_id(
                mapping
            ),  # per-object merge id (was q/Q flush scope)
            "left": mapping["left"],
            "right": mapping["right"],
            "top": mapping["top"],
    ...
    def extend_chunk(mapping: dict, leading_space: bool) -> None:
        ...
        last_ref = (chunk["prev_text_x"], chunk["prev_oy"])
        chunk["glyph_w"] = mapping.get(
            "glyph_w", 0.0
        )  # last glyph's advance (for _remerge_rotated)
        ...
        chunk["font_tally"][mapping["font_name"]] = (
            chunk["font_tally"].get(mapping["font_name"], 0) + 1
        )
        chunk["weight_tally"][mapping["weight"]] = (
            chunk["weight_tally"].get(mapping["weight"], 0) + 1
        )
    ...
    for text in chars:
        ...
        if text["is_mn"]:
            if chunk is not None and (
                ...
                or (
                    _object_merge_id(text) != chunk["flush_id"]
                    and not chunk["ws_pending"]
                )
            ):
                flush()
            if chunk is None:
        ...
        if (
            text["obj"] is chunk["obj"]
            and abs(line_delta_y) < 0.1 * height
        ...
        emit_fake_space(
            advance
        )  # standalone height-0 " ", width=abs(gap) (exact span merger)
        open_chunk(text)
        save_last_char(text["ch"])
        assert chunk is not None
        chunk["str"].append(text["ch"])
