...
def _apply_font_unicode(
    ...
    def targets_for(
        font_xref: int | None, other_numbers: tuple[int, ...]
    ) -> list[str] | None:
        ...
        return [
            measure_item.get(
                (other_numbers[key_value] << 8) | other_numbers[key_value + 1]
            )
            or chr((other_numbers[key_value] << 8) | other_numbers[key_value + 1])
    ...
    def apply(
        ...
        for index_value, token_value in patches:
            ...
            (
                candidate_item["is_ws"],
                candidate_item["is_mn"],
                candidate_item["is_cf"],
            ) = _char_category(token_value)
    ...
    if len(objects) == len(show_codes):
        ...
        for raw_char in raw_chars:
            if raw_char["is_gen"]:
                continue
            chars_by_obj.setdefault(id(raw_char["obj"]), []).append(
                (raw_char["i"], raw_char["ch"])
            )
        desynced: list[int] = []
        failed_windows: list[list[int]] = []
        synth_sites: list[dict] = []
        targets_by_object_index: dict[int, list[str] | None] = {}
        for object_index, (obj, (font_index, encoded_text, _tz)) in enumerate(
            zip(objects, show_codes)
        ):
        ...
        def _rewalk_window(window: list[int]) -> bool:
            char_value = sorted(
                (
                    ...
                    for pair in chars_by_obj.get(id(objects[state_item]), [])
                )
            )
            ...
            def _commit(res) -> bool:
                ...
                for char_index, text_index in res[2]:
                    ...
                    if (
                        ...
                        and candidate_item["obj"] is not objects[owner[text_index]]
                    ):
                ...
                for text_index, pos in res[3]:
                    synth_sites.append(
                        {
                            "t": text_transform[text_index],
                            "owner": objects[owner[text_index]],
                            "prev_i": char_value[pos - 1][0] if pos > 0 else None,
                            "next_i": (
                                char_value[pos][0] if pos < len(char_value) else None
                            ),
                        }
                    )
                return True
            ...
            def _displacement_repair() -> bool:
                ...
                for (
                    ...
                    char_start,
                    char_end,
                ) in state_map.get_opcodes():
                    if tag == "equal":
                        for reference_item in range(index_two - index_one):
                            char_to_tgt[char_start + reference_item] = (
                                index_one + reference_item
                            )
                    else:
                ...
                for char_index in loose_char_indexes:
                    ...
                    cands = [
                        target_index
                        for target_index in loose_target_indexes
                        if target_index not in used_targets
                        and tts[target_index] == chs[char_index]
                    ]
                    if not cands:
                    ...
                    if cdict is not None and len(cands) > 1:
                        ...
                        def _object_distance_sq(target_index: int) -> float:
                            ...
                            delta_x = max(
                                item_value["l"] - origin_x,
                                0.0,
                                origin_x - item_value["r"],
                            )
                            delta_y = max(
                                item_value["b"] - origin_y,
                                0.0,
                                origin_y - item_value["t"],
                            )
                ...
                if leftover:
                    ...
                    for target_index in leftover:
                        ...
                        point_value = (
                            mapped_tis[page_value - 1] if page_value > 0 else None
                        )
                        normalized_token = (
                            mapped_tis[page_value]
                            if page_value < len(mapped_tis)
                            else None
                        )
                        synth_sites.append(
                            {
                                "t": tts[target_index],
                                "owner": objects[owner[target_index]],
                                "prev_i": (
                                    char_value[tgt_to_char[point_value]][0]
                                    if point_value is not None
                                    else None
                                ),
                                "next_i": (
                                    char_value[tgt_to_char[normalized_token]][0]
                ...
                for char_index, target_index in char_to_tgt.items():
                    ...
                    if (
                        candidate_item is not None
                        and candidate_item["obj"] is not objects[owner[target_index]]
                    ):
        ...
        for object_index in desynced:
            if window:
                ...
                if len(gap) <= 2 and all(
                    targets_by_object_index.get(bridge_index) is not None
                    for bridge_index in gap
                ):
                    window.extend(gap)
...
def _synthesize_dropped_glyphs(
    ...
    for site in sites:
        if (
            groups
            and groups[-1][0]["prev_i"] == site["prev_i"]
    ...
    for group_value in groups:
        ...
        prev = (
            chars_by_index.get(group_value[0]["prev_i"])
            if group_value[0]["prev_i"] is not None
            else None
        )
        nxt = (
            chars_by_index.get(group_value[0]["next_i"])
            if group_value[0]["next_i"] is not None
            else None
        )
        text = "".join(
            site["t"] for site in group_value
        )  # one char per target codepoint
        count_item = len(text)
        if not count_item:
            continue
        if prev is not None:
