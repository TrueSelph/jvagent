...
def _new_oblique_span(
    glyph: dict, baseline_pos: float, cross_pos: float, glyph_width: float
) -> dict:
    ...
    return {
        ...
        "_oy0": glyph["oy"],
        "_u0": baseline_pos,
        "_uend": baseline_pos + glyph_width,
        "_pen": baseline_pos + glyph_width,
        "_vlast": cross_pos,
        "_lox": glyph["ox"],
        "_loy": glyph["oy"],
        "_lgw": glyph_width,
...
def _oblique_space(
    cur: dict, adv: float, baseline_unit_x: float, baseline_unit_y: float, scale: float
) -> dict:
...
def _merge_oblique_one(chs: list[dict]) -> list[dict]:
    ...
    baseline_unit_x, baseline_unit_y = (
        matrix_a / scale,
        matrix_b / scale,
    )  # baseline unit direction (page space)
    ...
    for glyph in ordered:
        ...
        if cur is None:
            ...
            continue
        baseline_gap = (
            baseline_pos - cur["_pen"]
        )  # along-baseline gap beyond prev advance
        cross_shift = cross_pos - cur["_vlast"]  # cross-axis shift
        if (
            baseline_gap < font_size * NEGATIVE_SPACE_FACTOR
            or abs(cross_shift) > font_size
        ):
        ...
        if baseline_gap <= font_size * TRACKING_SPACE_FACTOR:
            ...
            spans.append(
                _oblique_space(
                    cur, baseline_gap, baseline_unit_x, baseline_unit_y, scale
                )
            )  # standalone " "
            cur = _new_oblique_span(glyph, baseline_pos, cross_pos, glyph_width)
            continue
...
def _merge_vertical_one(group: list[dict]) -> list[dict]:
    ...
    for chunk in group:
        ...
        width = cur["fs"]
        if (
            vertical_gap < direction_sign * NEGATIVE_SPACE_FACTOR * font_size
            or abs(x_shift) > width
        ):
            ...
            direction_sign * SPACE_IN_FLOW_MIN_FACTOR * font_size
            <= vertical_gap
            <= direction_sign * SPACE_IN_FLOW_MAX_FACTOR * font_size
        ):
            cur["str"].append(" ")
            cur["v_height"] += vertical_gap + (chunk["v_pen_y"] - chunk["v_after"])
            _grow_vert_span(cur, chunk)
...
def _remerge_vertical(items: list[dict]) -> list[dict]:
    ...
    merged_for = {
        oid: _merge_vertical_one(group_value) for oid, group_value in groups.items()
    }
