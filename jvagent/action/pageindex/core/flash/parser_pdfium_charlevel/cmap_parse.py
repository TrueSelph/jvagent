...
def _ieee_div(value: float, other_item: float) -> float:
    ...
    return (
        math.inf
        if (value > 0.0) == (math.copysign(1.0, other_item) > 0.0)
        else -math.inf
    )
...
def _parse_tounicode_cmap(data: bytes) -> dict[int, str]:
    ...
    while index_value < count_item:
        ...
        if candidate_item in _PDF_WHITESPACE_BYTES:
            ...
            hex_values = "".join(
                chr(secondary_item)
                for secondary_item in data[index_value + 1 : state_item]
            ...
            index_value += (
                2
                if (index_value + 1 < count_item and data[index_value + 1] == 0x3E)
                else 1
            )
            ...
            while (
                ...
                and data[state_item] not in _PDF_WHITESPACE_BYTES
                and data[state_item] not in _PDF_DELIMITER_BYTES
            ):
                state_item += 1
            tokens.append(
                ("name", data[index_value + 1 : state_item].decode("latin-1"))
            )
            index_value = state_item
        elif (
            candidate_item == 0x28
        ):  # (string) -- literal-string lexer code units (dst values)
            depth = 0
            ...
            while index_value < count_item:
                ...
                if byte_value == 0x5C:
                    ...
                    if entry_item in _PDF_STRING_ESCAPE_BYTES:
                        ...
                        while (
                            state_item < count_item
                            and state_item - index_value <= 3
                            and 0x30 <= data[state_item] <= 0x37
                        ):
                        ...
                        if (
                            entry_item == 0x0D
                            and index_value < count_item
                            and data[index_value] == 0x0A
                        ):
                            index_value += 1
                    else:
    ...
    while key_value < len(tokens):
        ...
        if kind == "op" and val == "beginbfchar":
            ...
            key_value += 1
            while (
                key_value + 1 < len(tokens)
                and tokens[key_value][0] == "hex"
                ...
                if src_end - src_start > 0xFFFFFF:
                    ...
                    if key_value < len(tokens) and tokens[key_value] == ("delim", "["):
                        while key_value < len(tokens) and tokens[key_value] != (
                            "delim",
                            "]",
                        ):
                            key_value += 1
                        key_value += 1
                    elif key_value < len(tokens) and tokens[key_value][0] in (
                        "hex",
                        "num",
                    ):
                        key_value += 1
                ...
                if key_value < len(tokens) and tokens[key_value] == ("delim", "["):
                    ...
                    key_value < len(tokens)
                    and tokens[key_value][0] == "num"
                    and is_int(tokens[key_value][1])
                ):
            ...
            while (
                ...
                if not is_int(tokens[key_value + 1][1]):
                    ...
                    key_value += 2
                    break
                out[_cmap_str_to_int(tokens[key_value][1])] = codepoint_to_string(
                    tokens[key_value + 1][1]
                )
                key_value += 2
        elif kind == "op" and val == "begincidrange":
            key_value += 1
            while (
                key_value + 2 < len(tokens)
                and tokens[key_value][0] == "hex"
                and tokens[key_value + 1][0] == "hex"
                and tokens[key_value + 2][0] == "num"
            ):
                src_start = _cmap_str_to_int(tokens[key_value][1])
                src_end = _cmap_str_to_int(tokens[key_value + 1][1])
