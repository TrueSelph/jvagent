...
def extract_top_level_headings(
    headings: list[Block], levels: dict[int, int]
) -> list[Block]:
...
def assign_levels(headings: list[Block]) -> dict[int, int]:
    ...
    ranked = sorted(
        buckets.keys(), key=lambda key_value: (-key_value[1], not key_value[2])
    )
    style_level = {
        key_value: min(index_value + 1, 6)
        for index_value, key_value in enumerate(ranked)
    }
    out: dict[int, int] = {}
    for state_item in headings:
...
def build_tree(
    headings: list[Block], levels: dict[int, int], page_lookup, total_pages: int
) -> list[dict]:
    ...
    for index_value, count_item in enumerate(flat):
        next_start = (
            flat[index_value + 1]["start_index"]
            if index_value + 1 < len(flat)
            else total_pages
        )
        count_item["end_index"] = max(
            count_item["start_index"],
            (
                next_start - 1
                if next_start > count_item["start_index"]
                else count_item["start_index"]
            ),
        )
    if flat:
