...
def is_heading_candidate(block: Block, body_size: float, body_bold: bool) -> bool:
    ...
    if (
        block.line_count() >= 1
        and numbering_kind(block.line()) != 0
        and len(text) <= 120
        and (block.bold_frac() > 0.3 or size_gain >= 1.0)
    ):
        return True
...
_PSEUDO_CODE_PATTERNS = (
    ...
    re.compile(
        r"\bwhile\b|\bif\b|\belse\b|\bfor\b|\breturn\b|\bdo\b", re.IGNORECASE
    ),  # code keywords
...
def filter_by_clique(headings: list[Block]) -> list[Block]:
    ...
    if not anchor_groups:
        return headings
    winner_sig, winner_group = max(
        anchor_groups.items(), key=lambda item_pair: len(item_pair[1])
    )
