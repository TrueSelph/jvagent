...
_institution_thesis_words = list(_DICTS.get("institution_words", [])) + list(
    _DICTS.get("nk_thesis_words", [])
)
INSTITUTION_THESIS_TRIE = build_trie(
    _institution_thesis_words, set_case_fold(TrieConfig(), True)
)
    ...
    for line in block:
        ...
        if (
            not (
                line_number > 0
                and line_number < 1e6
                and line_number == math.ceil(line_number)
            )
            or line_number >= 1e4
            or last_number > line_number
        ):
            return NumberColumnCluster(
                block.center_x(),
                block.bbox_width(),
                column,
                last_number,
        ...
        sequence_length += 1
    return NumberColumnCluster(
        block.center_x(), block.bbox_width(), column, last_number, sequence_length, True
    )


def pick_nearer_cluster(
    cluster: NumberColumnCluster,
    other_cluster: Optional[NumberColumnCluster],
    other: Optional[NumberColumnCluster],
) -> Optional[NumberColumnCluster]:
    """Pick the closer neighbor cluster within the current cluster width."""
    distance = (
        (cluster.anchor_x - other_cluster.anchor_x)
        if other_cluster is not None
        else math.inf
    )
    candidate_distance = (
        (other.anchor_x - cluster.anchor_x) if other is not None else math.inf
    )
...
def detect_toc_range(doc, page, index) -> Optional[dict]:
    ...
    def _add_cluster(cluster: NumberColumnCluster) -> None:
        ...
        if (
            insert_index < len(clusters)
            and clusters[insert_index].anchor_x == cluster.anchor_x
        ):
            return
        clusters.insert(insert_index, cluster)

    def _next_number_column_cluster(
        cluster: NumberColumnCluster,
    ) -> Optional[NumberColumnCluster]:
    ...
    def _prev_number_column_cluster(
        cluster: NumberColumnCluster,
    ) -> Optional[NumberColumnCluster]:
    ...
    for codepoint, block in enumerate(blocks):
        ...
        if last_line is not None and DOT_LEADER_ROW_RE.search(text_of_line(last_line)):
            ...
            if last_toc >= 0:
                ...
                if (
                    is_last_page
                    and dot_leader_blocks >= 2
                    and weight >= 0.8 * page.primary_slot.secondary_slot
                ):
                    last_toc = codepoint
        # Body block tracking
        ...
        if (
            ...
            and trie_full_match(TOC_TITLES_TRIE, tokenize_block(block))
        ):
            contents = codepoint
        ...
        if cluster is not None:
            ...
            if picked is not None:
                ...
                new_value = NumberColumnCluster(
                    ...
                    picked.tertiary_slot
                    and cluster.tertiary_slot
                    and picked.primary_slot <= cluster.secondary_slot,
                )
            else:
                new_value = cluster
            _add_cluster(new_value)
            if (
                new_value.tertiary_slot
                and (new_value.length >= 10 or (new_value.length >= 5 and is_last_page))
                and new_value.primary_slot - new_value.secondary_slot
                > 0.01 * new_value.primary_slot
            ):
                last_toc = codepoint
    ...
    if last_toc < 0:
        return None
    return {
        "start_index": pre_contents + 1 if pre_contents >= 0 else 0,
        "end_index": last_toc,
    }
...
def mark_toc_and_boilerplate(doc) -> None:
    ...
    for page in doc.primary_slot:
        if (
            ...
            and page.primary_slot.secondary_slot
            >= 0.9 * doc.secondary_slot.secondary_slot
        ):
            continue
        ...
        if (
            ...
            and page.primary_slot.secondary_slot
            < max(200, min(0.75 * doc.secondary_slot.secondary_slot, 1000))
        ):
            total = 0.0
            body_paragraph = 0.0
            ...
            for line_or_block in page.output_slot:
                if line_or_block.type != 0 or line_or_block.skew_frac() >= 1:
                    continue
                width_value = info_weight(line_or_block.char_stats) * heading_score(
                    line_or_block
                )
                total += width_value
        ...
        for idx in range(start, len(blocks)):
            ...
            if (
                block.skew_frac() < 1
                and score > doc.secondary_slot.primary_slot + 4
                and score > page.primary_slot.primary_slot + 4
            ):
                break
            if centered_flag <= 1 and block.isolated_centered:
                break
            if (
                info_weight(block.char_stats) > 300
                and block.char_stats.primary_slot[6] > 2
                and block.weighted_ratio_secondary > 0.5
            ):
                break
            if width > 0:
                average = width_flag / width
                if (
                    walk_break_y - block.top_edge() > average
                    and block.skew_frac() < 1
                    and score > average + 1.5
                ):
                    break
            walk_break_y = min(walk_break_y, block.bottom_edge())
            block.type = 9
