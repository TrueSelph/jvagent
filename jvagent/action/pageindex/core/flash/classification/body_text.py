...
def is_body_paragraph(doc_stats: DocStats, page, block: Block) -> bool:
    ...
    dw_per_line = width / lines if lines != 0 else (math.inf if width > 0 else math.nan)
    if (
        dw_per_line < 15
    ...
    if lines >= 4:
        ...
        for state_item in block:
            if (
                state_item.bbox_width() < 0.75 * block_width
                and not state_item.primary_slot[0].state_slot.startswith("•")
            ):
                short += 1
    ...
    if (
        chars < 40
        or (lines >= 3 and alignment_code(block) == 3)
        or letter_count(block.char_stats) < 0.1 * chars
    ):
        return False
    ...
    min_value = min(
        doc_stats.primary_slot,
        max(page.bounds.bbox_height(), page.bounds.bbox_width()) / 60,
    )
    ...
    if (
        size < body_size - 2
        or size < min_value
        or trie_prefix_match(BOILERPLATE_TRIE, tokenize_block(block))
    ):
        return False
    ...
    if (chars >= 100 or block.char_stats.tertiary_slot == 6) and (
        size >= page.primary_slot.primary_slot - 0.5
        or size > doc_stats.primary_slot - 0.1
    ):
        return (
            first_span_of(block).font_name == page.primary_slot.state_slot
            or last_span(last_line_of(block)).font_name == page.primary_slot.state_slot
        )
    return False
...
def span_page_number(span) -> Optional[int]:
    ...
    if match:
        ...
        if (
            not math.isnan(page_number)
            and page_number > 0
            and page_number < 1e6
            and page_number == math.ceil(page_number)
        ):
