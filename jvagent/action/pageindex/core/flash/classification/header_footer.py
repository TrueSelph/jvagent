...
def is_header_positioned(
    ctx, other_block: Block, candidate_block: Optional[Block]
) -> bool:
    ...
    return (
        cond
        and other_block.line_count() == 1
        and info_weight(other_block.char_stats) >= 8
        and letter_count(other_block.char_stats) >= 5
        and other_block.char_stats.primary_slot[1] >= 1
    )


def has_adjacent_page_numbers(
    ctx, page: int, candidate_number: int, reference_flag: bool
) -> bool:
    ...
    adjacent_one = (
        page_index - 1 >= 0
        and (candidate_number - 1) in ctx.tertiary_slot[page_index - 1]
    ) or (
    ...
    adjacent_two = (
        page_index - 2 >= 0
        and (candidate_number - 2) in ctx.tertiary_slot[page_index - 2]
    ) or (
        page_index + 2 < page_count
        and (candidate_number + 2) in ctx.tertiary_slot[page_index + 2]
    )
    ...
    adjacent_four = (
        page_index - 4 >= 0
        and (candidate_number - 4) in ctx.tertiary_slot[page_index - 4]
    ) or (
        page_index + 4 < page_count
        and (candidate_number + 4) in ctx.tertiary_slot[page_index + 4]
    )
    if candidate_number > page / 2 - 30:
        return (
            adjacent_one
            or (not reference_flag and adjacent_two)
            or (adjacent_two and adjacent_four)
        )
...
def find_cross_page_match(
    ctx, page, block: Block, text_key: str, ref: Block
) -> Optional[Block]:
        ...
        if not (
            ...
            or (
                ...
                and (
                    ...
                    or (
                        entry_page_index == 1
                        and heading_score(entry_block) >= size + 0.5
                    )
                )
            )
        ):
    ...
    if is_header_positioned(ctx, block, ref):
        for key in longest_word_and_number(block):
            ...
            if map_value is None or len(map_value) < max(
                4, len(ctx.secondary_slot.primary_slot) / 4
            ):
                continue
            ...
            for nearby_page_index in range(page.page_index - 2, page.page_index + 3):
                ...
                if (
                    abs(target - heading_score(nearby_entry["block"])) > 1
                    or (page.page_index == 1 and target >= body_font_size + 0.5)
                    or (
                        nearby_page_index == 1
                        and heading_score(nearby_entry["block"]) >= body_font_size + 0.5
                    )
                ):
                    continue
                threshold = min(len(text_key), len(nearby_entry["text_key"])) / 5
                if (
                    bounded_edit_distance(text_key, nearby_entry["text_key"], threshold)
                    >= threshold
                ):
                    continue
...
class HeaderFooterContext:
    ...
    def __init__(self, doc, candidate_number: int):
        ...
        self.auxiliary_slot: dict[str, list[dict]] = (
            {}
        )  # normalized text key -> location/block records
        self.measure_slot: dict[str, dict[int, dict]] = (
            {}
        )  # word/number key -> page -> text/block record
...
def bounded_edit_distance(text: str, other_text: str, candidate_item: float) -> float:
    ...
    candidate_item = (
        max(len(text), len(other_text))
        if candidate_item <= 0
        else math.ceil(candidate_item)
    )
    if len(text) <= 0:
    ...
    for state_item in range(1, len(other_text) + 1):
        ...
        for line_value in range(reference_item, min(entry_item + 1, len(text)) + 1):
            if line_value == reference_item:
                ...
                group_value[line_value] = 1 + min(
                    group_value[line_value - 1], score_value[line_value - 1]
                )
                if line_value <= entry_item:
                    group_value[line_value] = min(
                        group_value[line_value], 1 + score_value[line_value]
                    )
...
def detect_header_footer(ctx: HeaderFooterContext) -> None:
    ...
    for page in ctx.secondary_slot.primary_slot:
        ...
        def walk_cb(block: Block) -> bool:
            ...
            relative = (
                (num / den)
                if den
                else (math.copysign(math.inf, num) if num else math.nan)
            )
            if (ctx.primary_slot == 1 and relative < 0.8) or (
                ctx.primary_slot == 2 and relative > 0.2
            ):
                pass_value = False
            else:
    ...
    for page in ctx.secondary_slot.primary_slot:
        ...
        for candidate_index in range(len(candidates)):
            ...
            # anywhere inside it.
            if candidate_tokens.length < 10 and trie_prefix_match(
                COPYRIGHT_TRIE, candidate_tokens
            ):
            ...
            if letter_count(candidate_block.char_stats) >= 5:
                ...
                match = find_cross_page_match(
                    ctx, page, candidate_block, pk_hash, first_substantive[0]
                )
                if match is not None:
                    ...
                    if len(pk_hash) >= 5:
                        ...
                        text_counts[pk_hash] = (
                            2 if (previous_count or other_unmarked) else 1
                        )
                    continue
            style_threshold = max(
                2.0, min(len(ctx.secondary_slot.primary_slot) / 3.0, 5.0)
            )
            chars = 0
            for candidate_line in candidate_block:
                for candidate_span in candidate_line:
                    ...
                    if page_number is not None and has_adjacent_page_numbers(
                        ctx, page.page_index, page_number, seen_page_number
                    ):
        ...
        for index in range(state.primary_slot):
            ...
            if (
                ctx.primary_slot == 1
                and block.bottom_edge() < first_block.bottom_edge()
            ):
                continue
            ...
            if (
                ...
                and heading_score(block) >= state.secondary_slot + 1
            ):
                continue
        ...
        for recurring_block in ctx.state_slot[page.page_index - 1]:
            ...
            if stripped is not None:
                ...
                if head_tok is not None and head_tok.type == 1:
                    ...
                    record_recurring_text(
                        ctx.secondary_slot, deaccented_text(recurring_block)
                    )
            ...
            if (
                page.page_index <= 1
                and heading_score(recurring_block)
                > ctx.secondary_slot.secondary_slot.primary_slot + 1
            ):
                continue
