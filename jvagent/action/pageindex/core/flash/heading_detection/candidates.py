...
class PageScanState:
    ...
    def __init__(self, doc, page):
        ...
        self.state_slot = (
            doc.primary_slot[page.page_index - 2] if page.page_index >= 2 else None
        )  # prev page
...
def make_heading_candidate(
    page_scan: PageScanState,
    type_: int,
    block: Block,
    item_list: list[int],
    tokens: Optional[TokenView],
    title_tokens: Optional[TokenView],
    has_numbering_flag: bool = False,
) -> HeadingCandidate:
    ...
    if (
        not has_numbering_flag
        and right_neighbor is not None
        and block.previous_slot > 0.9
        and title_tokens is not None
    ):
        ...
        if last_title_token is not None and is_trimmable_token(last_title_token):
            ...
            if mh_block.bbox_width() > 0.7 * right_neighbor.bbox_width() and abs(
                mh_block.right_edge() - right_neighbor.right_edge()
            ) < 2 * avg_char_width(mh_block):
                has_numbering_flag = True
    prominent_flag = type_ == 7 or (
        len(item_list) > 0
        and title_tokens is not None
        and matches_references(title_tokens)
    )
...
def make_plain_candidate(
    page_scan: PageScanState, type_: int, block: Block
) -> HeadingCandidate:
    """Build a type-only candidate using the full block text."""
    return make_heading_candidate(
        page_scan, type_, block, [], None, tokenize_block(block), False
    )


def make_body_heading_candidate(
    page_scan: PageScanState, type_: int, block: Block, tokens: TokenView
) -> HeadingCandidate:
    """Build a candidate from body-heading tokens."""
    return make_heading_candidate(
        page_scan, type_, block, [], None, trim_trailing_punct(tokens), True
    )
...
def make_numbered_candidate(
    page_scan: PageScanState,
    block: Block,
    item_list: list[int],
    tokens: TokenView,
    title_tokens: TokenView,
    ...
    if (
        title_tokens.length == 1
        and first_title_token is not None
        and first_title_token.primary_slot != 2
        and first_title_token.primary_slot != 4
        and first_title_token.secondary_slot != 2
    ...
    if (
        len(item_list) == 1
        and item_list[0] == 1
        and block.top_edge() < 0.3 * page_scan.primary_slot.bounds.bbox_height()
    ):
        ...
        if neighbor_right(page_scan.tertiary_slot, block) is None:
            last_title_token = last_token(title_tokens)
            if (
                last_title_token is not None
                and last_title_token.anchor_ranges
                and last_title_token.anchor_ranges[-1].line is last_line_of(block)
            ):
                return None
    ...
    if block.line_count() > 1:
        ...
        if first_number_token is not None and first_title_token is not None:
            ...
            if not (
                left < title_left and second_line.left_edge() > (left + title_left) / 2
            ):
                # Check trailing tokens for c+1 continuation
                trailing_tokens = tokenize_block(block)
                trailing_tokens = trailing_tokens.slice(
                    _di_count(trailing_tokens, block.line())
                )
    ...
    if reject:
        return None
    return make_heading_candidate(
        page_scan, 1, block, item_list, tokens, title_tokens, False
    )
