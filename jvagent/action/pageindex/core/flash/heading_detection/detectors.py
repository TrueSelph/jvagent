...
def detect_numbered_heading(
    page_scan: PageScanState, block: Block, tokens: TokenView
    ...
    for entry in enumerate_tokens(tokens):
        ...
        if token.type == 1:
            ...
            if (
                len(item_list) > 1
                or heading_score(block)
                > page_scan.primary_slot.primary_slot.primary_slot + 1
                or (
                    not block.measure_slot
                    and at_value is not None
                    and (
                        at_value.primary_slot in (2, 4)
                        or at_value.secondary_slot == 2
                        or at_value.type == 4
                        or at_value.str == "."
                        or at_value.str == "|"
                    )
        ...
        if token.primary_slot not in (2, 4):
            break
        if len(item_list) > 1 or len(token.str) >= 3 or tokens.length - index >= 3:
            return make_numbered_candidate(
                page_scan,
                block,
                item_list,
...
def detect_labeled_heading(
    ...
    if roman is not None and is_word_token(second) and second.str in ".．｡。:)":
        ...
        return make_heading_candidate(
            page_scan, 2, block, [roman], prefix, tokens.slice(prefix.length)
        )
    # CJK number path
    ...
    # Letter path
    if tokens.length <= 1 or (
        block.char_stats.secondary_slot == 3
        and (
            block.line_count() > 1 or is_punct_category(block.char_stats.tertiary_slot)
        )
    ):
        return None
    ...
    if second.str == "." or second.str == ")":
        ...
        if (
            ...
            or second_anchor.left_edge()
            < first_anchor.right_edge() + first_anchor.bbox_width()
            or heading_score(block)
            < page_scan.primary_slot.primary_slot.primary_slot + 1
...
def detect_chapter_appendix(
    ...
    if candidate_item <= page_scan.primary_slot.primary_slot.primary_slot + 0.1:
        return None
    flag = (
        other_block.isolated_centered
        or candidate_item > page_scan.secondary_slot.secondary_slot.primary_slot + 0.1
        and (
            other_block.bold_frac() > 0.9
            or is_upper_dominant(other_block.char_stats)
            or candidate_item
            > 1.5 * page_scan.secondary_slot.secondary_slot.primary_slot
        )
    )
    ...
    if flag and match is not None:
        ...
        return make_heading_candidate(
            page_scan, 8, other_block, [value], prefix, tokens.slice(prefix.length)
        )
    if flag:
        ...
        if match is not None:
            ...
            return make_heading_candidate(
                page_scan, 9, other_block, [], prefix, tokens.slice(prefix.length)
            )
    match = trie_prefix_match(APPENDIX_KEYWORDS_TRIE, tokens)
    if match is not None:
        next_item = tokens.token_at(match.length)
        val = token_to_number(next_item) or (
            letter_to_ordinal(next_item.str) if next_item is not None else None
        )
        if not flag and val is None:
            return None
        item_list = [val] if val is not None else []
        prefix = tokens.slice(
            0, skip_bracketed_word(tokens, match.length + (1 if val is not None else 0))
        )
        return make_heading_candidate(
            page_scan, 10, other_block, item_list, prefix, tokens.slice(prefix.length)
        )
    return None
...
def detect_box_heading(
    ...
    prefix = tokens.slice(0, skip_bracketed_word(tokens, match.length + 1))
    return make_heading_candidate(
        page_scan, 12, other_block, [int(val)], prefix, tokens.slice(prefix.length)
    )
...
def classify_heading(page_scan: PageScanState, other_block: Block) -> HeadingCandidate:
    ...
    if heading is not None:
        return heading
    type_code = (
        7 if matches_references(tokens) else (5 if matches_abstract(tokens) else 0)
    )
...
def is_acceptable_heading(
    page_scan: PageScanState, other_heading_candidate: HeadingCandidate
) -> bool:
    """. The big "is this an acceptable heading?" gate."""
    heading = other_heading_candidate.group_slot
    if (
        heading.bbox_height() >= 2 * heading.bbox_width()
        or info_weight(heading.char_stats) <= 3
        or heading.line_count() > 5
        or heading.char_count() >= 300
    ):
        return False
    ...
    if (
        score <= page_scan.primary_slot.primary_slot.primary_slot + 0.5
        and score <= doc_group + 0.5
        and not other_heading_candidate.is_prominent
    ):
        return False
    ...
    if (
        ...
        or (
            ...
            and page_scan.primary_slot.primary_slot.secondary_slot
            > min(1000, page_scan.secondary_slot.secondary_slot.secondary_slot)
        )
    ):
        return False
    col_bottom = (
        page_scan.primary_slot.tertiary_slot[safe_column_index(heading)]
        if 0 <= safe_column_index(heading) < len(page_scan.primary_slot.tertiary_slot)
        else None
    )
    if (
        heading.bbox_width() < 0.2 * width
        and col_bottom is not None
        and col_bottom.bbox_width() < 0.2 * width
        and col_bottom.bbox_height() > 1.5 * col_bottom.bbox_width()
    ):
        return False
    neighbor = neighbor_right(page_scan.tertiary_slot, heading)
    gap = (
        heading.bottom_edge() - neighbor.top_edge()
    ...
    above_gap = (
        above.bottom_edge() - heading.top_edge() if above is not None else math.inf
    )
    ...
    if (
        ...
        and (
            page_scan.state_slot.primary_slot.secondary_slot
            < clamp(page_scan.secondary_slot.secondary_slot.secondary_slot, 200, 500)
    ...
    previous_page_left_neighbor = (
        page_scan.state_slot.primary_slot.option_slot
        if page_scan.state_slot is not None
        else math.nan
    )
    previous_page_height = (
        page_scan.state_slot.bounds.bbox_height()
        if page_scan.state_slot is not None
        else math.nan
    )
    ...
    if (
        ...
        or heading.weighted_ratio_primary
    ...
    if (
        body_neighbor is not None
        and heading.bottom_edge() - body_neighbor.top_edge() > 2 * heading.bbox_height()
        and body_neighbor.marker_slot != 0
    ):
        return False
    previous_block = (
        page_scan.auxiliary_slot[heading.orig_index - 1]
        if 0 <= heading.orig_index - 1 < len(page_scan.auxiliary_slot)
        else None
    )
    next_block = (
        page_scan.auxiliary_slot[heading.orig_index + 1]
    ...
    return (
        (
            previous_page_left_neighbor
            > doc_right_neighbor + 0.1 * previous_page_height
            and (
                score > doc_group + 2
                or (
                    previous_page_left_neighbor
                    > doc_right_neighbor + 0.2 * previous_page_height
                    and body_neighbor is not None
        ...
        or (
            len(other_heading_candidate.numbering) == 1
            and other_heading_candidate.numbering[0] == 1
            and above is None
        )
    )
...
def is_too_wide_for_heading(page_scan: PageScanState, other_block: Block) -> bool:
    ...
    if (
        width > 0.7 * page_scan.primary_slot.bounds.bbox_width() / 2
        or width > 0.7 * page_scan.secondary_slot.secondary_slot.option_slot
    ):
        return True
    count = 0
    for heading in range(
        page_scan.primary_slot.page_index - 1, page_scan.primary_slot.page_index + 2
    ):
...
def passes_neighbor_check(page_scan: PageScanState, other_block: Block) -> bool:
    ...
    next_overlap = candidate_item is not None and y_overlaps(
        other_block, candidate_item
    )
    ...
    if candidate_item is not None and is_too_wide_for_heading(
        page_scan, candidate_item
    ):
        return False
    if (
        candidate_item is not None
        and not candidate_item.is_body_paragraph
        and candidate_item.line_count() > 3
        and candidate_item.bbox_height() > 0.8 * candidate_item.bbox_width()
    ):
        return True
    ...
    if (
        ...
        and candidate_item is not above_or_overlap
...
def has_competing_labeled_heading(
    page_scan: PageScanState,
    other_heading_candidate: HeadingCandidate,
    ...
    if (
        not similar_style(block, candidate_block)
        or abs(block.top_edge() - candidate_block.top_edge()) >= 5 * block.bbox_height()
    ):
        return False
    other_candidate = detect_labeled_heading(
        page_scan, candidate_block, tokenize_block(candidate_block)
    )
...
def is_bibliography_entry(block: Block, other_number: int = -1) -> bool:
    ...
    if word < 0.1 * tokens.length:
        return False
    return (
        digit >= 1.5 * other_number
        or year >= 0.5 * other_number
        or period_after_word >= 0.5 * other_number
    )
