...
def scan_page_headings(page_scan: PageScanState) -> list[HeadingCandidate]:
    ...
    for block in blocks:
        ...
        if block.char_count() <= 1 and block.char_stats.secondary_slot != 4:
            continue
        wn_entry = (
            page_scan.tertiary_slot.primary_slot[block.orig_index]
            if 0 <= block.orig_index < len(page_scan.tertiary_slot.primary_slot)
            else None
        )
        ...
        if (
            ...
            and (
                (0 < block.bold_frac() < 1)
                or style_key(first_span_of(block))
                != style_key(last_span(last_line_of(block)))
            )
        ):
        ...
        if (
            has_da_above
            and not block.measure_slot
            and rows >= 3
            and first_line.bbox_width()
            <= 0.2
            * min(
                block.primary_slot[1].bbox_width(), block.primary_slot[2].bbox_width()
            )
            and matches_abstract(first_line_tokens)
        ):
            push_candidate(
                page_scan,
                make_body_heading_candidate(page_scan, 5, block, first_line_tokens),
            )
            continue
        if block.char_count() >= 200:
            continue
        if (
            block.char_count() >= 100
            and rows > 1
            and block.char_stats.primary_slot[6] - first_line.char_stats.primary_slot[6]
            > 1
        ):
            continue
        size = block.avg_font_size()
        page_width = page_scan.primary_slot.bounds.bbox_width()
        lots_caps = block.char_stats.primary_slot[2] >= max(
            3, letter_count(block.char_stats) / 2
        )
        if rows > 4 or (
            rows >= 3
            and not (
                size >= 1.5 * page_scan.primary_slot.primary_slot.primary_slot
                or lots_caps
            )
        ):
            continue
        if (
            block.weighted_ratio_primary
            < 0.5 * page_scan.secondary_slot.secondary_slot.auxiliary_slot
        ):
            continue
        ...
        if (
            block.bold_frac() < 0.1
            and not lots_caps
            and size < page_scan.secondary_slot.secondary_slot.primary_slot - 2
        ):
            continue
        ...
        if passes_neighbor_check(page_scan, block):
            continue
        top_gap = (
            above.bottom_edge() - block.top_edge() if above is not None else math.inf
        )
        isolated = (
            not block.measure_slot
            and rows <= 2
            and (
                above is None
                or top_gap > 1.5 * block.avg_font_size()
        ...
        if not has_da_above and (
            not (
                heading_score(block)
                >= page_scan.primary_slot.primary_slot.primary_slot + 1.5
            )
            or (above is not None and above.type != 1)
            or (previous_block is not None and previous_block.type != 1)
            or (
                below is not None
                and not (below.top_edge() < block.bottom_edge() - size)
            )
        ):
            continue
        if (
            block.bold_frac() < 0.1
            and not lots_caps
            and first_span_of(block).font_name
            == page_scan.primary_slot.primary_slot.state_slot
        ...
        if top_gap < 0:
            continue
        predecessor_gap = (
            block.bottom_edge() - predecessor.top_edge()
        ...
        if predecessor_gap < -0.9 * block.bbox_height():
            continue
        line_gap = (
            page_scan.primary_slot.primary_slot.tertiary_slot
            - page_scan.primary_slot.primary_slot.primary_slot
        )
        if (
            top_gap < line_gap
            and size < page_scan.primary_slot.primary_slot.primary_slot - 1
        ):
            continue
        ...
        if heading_kind is not None:
            ...
            column_rect = (
                page_scan.primary_slot.tertiary_slot[col_idx]
                if 0 <= col_idx < len(page_scan.primary_slot.tertiary_slot)
                else None
            )
            # Narrow-column heading-vs-prev-numbering check
            if (
                size < page_scan.secondary_slot.secondary_slot.primary_slot - 1
                ...
                first_number = (
                    heading_kind.numbering[0] if heading_kind.numbering else 0
                )
                if above is not None:
                    ...
                    if (
                        first is not None
                        and first.type == 1
                        and token_numeric_value(first) != first_number
                    ):
                        continue
                if predecessor is not None:
                    predecessor_first_token = first_token(tokenize_block(predecessor))
                    if (
                        predecessor_first_token is not None
                        and predecessor_first_token.type == 1
                        and token_numeric_value(predecessor_first_token) != first_number
                    ):
                        continue
            ...
            if len(heading_kind.numbering) <= 1 and (
                ...
                or (
                    second_token_value is not None and is_word_token(second_token_value)
                )
            ):
                first_number = (
                    heading_kind.numbering[0] if heading_kind.numbering else 0
                )
                if above is not None and is_heading_continuation(
                    above, block, first_number
                ):
                    continue
                if (
                    right_neighbor_sib is not None
                    and above is not right_neighbor_sib
                    ...
                    continue
                if predecessor is not None and is_heading_continuation(
                    predecessor, block, first_number
                ):
                    continue
                if (
                    left_neighbor_sib is not None
                    and predecessor is not left_neighbor_sib
                    and not center_aligned(block, left_neighbor_sib, 1)
                    and (
                        predecessor.line_count() < 10 or predecessor.char_count() < 300
                    )
            ...
            if (
                block.top_edge() < page_scan.primary_slot.bounds.bbox_height() / 4
        ...
        if heading_signature is not None:
            if above is not None and has_competing_labeled_heading(
                page_scan, heading_signature, above
            ):
                continue
            if (
                right_neighbor_sib is not None
                and above is not right_neighbor_sib
                and has_competing_labeled_heading(
                    page_scan, heading_signature, right_neighbor_sib
                )
            ):
                continue
            if predecessor is not None and has_competing_labeled_heading(
                page_scan, heading_signature, predecessor
            ):
                continue
            if (
                left_neighbor_sib is not None
                and predecessor is not left_neighbor_sib
                and has_competing_labeled_heading(
                    page_scan, heading_signature, left_neighbor_sib
                )
            ):
                continue
        ...
        if isolated:
            caps_heavy = (
                size + 2 * block.bold_frac()
                >= page_scan.secondary_slot.secondary_slot.primary_slot + 4
                or is_upper_dominant(block.char_stats)
            )
            above_or_overlap = closest_body_neighbor_above(
                page_scan.tertiary_slot, block
            )
            page_width = page_scan.primary_slot.bounds.bbox_width()
            ...
            type5_cond = caps_heavy or (
                ...
                and (
                    block.bottom_edge() - above_or_overlap.top_edge()
                    < 3 * (block.bottom_edge() - predecessor.top_edge())
            ...
            type6_cond = (
                ...
                or (
                    above is not None
                    and x_aligned(block, above, 1)
                    and (
                        above.bbox_width() >= page_width / 6
                ...
                or (
                    ...
                    and x_aligned(block, predecessor, 1)
                    and (
                        predecessor.bbox_width() >= page_width / 6
                        or predecessor.bold_frac() > 0.9
                        or is_upper_dominant(predecessor.char_stats)
                        or (
                            block.previous_slot < 0.1
                            and predecessor.previous_slot > 0.9
                        )
                    )
                )
            )
...
def filter_page_candidates(
    doc_collector: DocCandidateCollector, page, page_candidates: list[HeadingCandidate]
) -> None:
    ...
    while doc_collector.option_slot < len(doc_collector.measure_slot):
        ...
        if current_candidate.type == 2:
            if not doc_collector.tertiary_slot:
                doc_collector.tertiary_slot = (
                    len(current_candidate.numbering) > 0
                    and current_candidate.numbering[0] == 1
                )
        elif current_candidate.type == 4:
            if not doc_collector.secondary_slot:
                doc_collector.secondary_slot = (
                    len(current_candidate.numbering) > 0
                    and current_candidate.numbering[0] == 1
                )
        elif current_candidate.type == 1 and len(current_candidate.numbering) > 0:
            doc_collector.primary_slot = max(
                doc_collector.primary_slot, current_candidate.numbering[0]
            )
    ...
    def _ih_key(heading_candidate: HeadingCandidate):
        first_line = (
            heading_candidate.group_slot.primary_slot[0]
            if heading_candidate.group_slot.primary_slot
            else None
        )
        column_index = first_line.measure_slot if first_line is not None else -1
        return (
            column_index,
            -heading_candidate.group_slot.top_edge(),
            -heading_candidate.group_slot.bottom_edge(),
            heading_candidate.group_slot.left_edge(),
            heading_candidate.group_slot.right_edge(),
        )
    ...
    for page_candidate in page_candidates:
        if total_bottom == math.inf and page_candidate.type == 5:
            total_bottom = (
                page_candidate.group_slot.top_edge()
                + page_candidate.group_slot.avg_font_size()
            )
    ...
    for index in range(count):
        ...
        if is_script_compatible(
            doc_collector.previous_slot.secondary_slot.tertiary_slot, active_candidate
        ):
            continue
        ...
        if not doc_collector.auxiliary_slot and active_candidate.type != 11:
            ...
            if (
                title is not None
                and bottom > title.top_edge()
                and intervals_overlap(
                    active_candidate.group_slot.left_edge(),
                    active_candidate.group_slot.right_edge(),
                    title.left_edge(),
                    title.right_edge(),
                )
            ):
        ...
        if (
            active_candidate.type == 0
            and next_item is not None
            and next_item.type == 5
            and active_candidate.group_slot.left_edge()
            <= next_item.group_slot.right_edge()
            and active_candidate.group_slot.right_edge()
            >= next_item.group_slot.left_edge()
            and active_candidate.group_slot.bottom_edge()
            - next_item.group_slot.top_edge()
            < 2 * active_candidate.group_slot.bbox_height()
        ...
        if active_candidate.type == 2:
            if (
                len(active_candidate.numbering) > 0
                and active_candidate.numbering[0] == 1
            ):
        ...
        if single_numbering_count >= 5:
            continue
        first_number = (
            active_candidate.numbering[0] if len(active_candidate.numbering) > 0 else 0
        )
        if (
            active_candidate.group_slot.bold_frac() < 0.9
            and not is_caps_heavy(active_candidate.group_slot)
            and active_candidate.group_slot.avg_font_size()
            < page.primary_slot.primary_slot + 1
        ):
            if (
                doc_collector.primary_slot <= 0
                and min_first_number > 1
                and len(active_candidate.numbering) <= 1
            ):
                continue
...
def build_doc_heading_candidates(
    doc, labeled: Optional[list] = None
        ...
        if not page_scan_state.primary_slot.auxiliary_slot:
            for block in page_scan_state.auxiliary_slot:
                if block.char_count() <= 0 or block.skew_frac() > 1 or block.type != 0:
                    continue
                if (
                    block.is_body_paragraph
                    or block.top_edge()
                    < 0.5 * page_scan_state.primary_slot.bounds.bbox_height()
                ):
                    break
                ...
                if block.line_count() > 2:
                    break
        if current_candidate is not None and not is_script_compatible(
            doc.secondary_slot.tertiary_slot, current_candidate
        ):
