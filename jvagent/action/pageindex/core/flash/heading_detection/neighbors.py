...
def compute_bucket_span(neighbor_map, block) -> dict:
    ...
    start_bucket = int(
        clamp(
            math.floor(block.left_edge() / neighbor_map.tertiary_slot),
            0,
            neighbor_map.secondary_slot - 1,
        )
    )
    end_bucket = int(
        clamp(
            math.ceil(block.right_edge() / neighbor_map.tertiary_slot),
            0,
            neighbor_map.secondary_slot - 1,
        )
    )
...
def neighbor_above(neighbor_map, other_block: Block) -> Optional[Block]:
    ...
    width_value = (
        neighbor_map.primary_slot[other_block.orig_index]
        if other_block.orig_index < len(neighbor_map.primary_slot)
        else None
    )
...
class PageNeighborMap:
    ...
    def __init__(self, page):
        ...
        self.secondary_slot = int(
            math.floor(page.bounds.bbox_width() / self.tertiary_slot)
        )  # bucket count
        self.primary_slot: list[Optional[BlockNeighborCache]] = [None] * (
            max(len(blocks), 1) + 1
        )
        ...
        for candidate_item in blocks:
            ...
            for index in range(spans["start_bucket"], spans["end_bucket"]):
                if 0 <= index < self.secondary_slot:
                    marked[index] = True
        recent_height: list[int] = [
            -1
        ] * self.secondary_slot  # most recent body block height at bucket
        ...
        recent_block_index: list[int] = [-1] * max(
            self.secondary_slot, 1
        )  # most-recent block V (j-direction)
        recent: list[Optional[Block]] = [
            None
        ] * self.secondary_slot  # most-recent block at bucket
        pending: list[list[int]] = [
            [] for _ in range(self.secondary_slot)
        ]  # pending V's per bucket
        ...
        for block_index, current_block in enumerate(blocks):
            ...
            adjacent_bucket_index = recent_block_index[
                (
                    ...
                    if (
                        left > 0
                        and current_block.left_edge()
                        < (left + 0.5) * self.tertiary_slot
                    )
            ...
            if value >= 0 or adjacent_bucket_index >= 0:
                ...
                adjacent_bucket_block = (
                    blocks[adjacent_bucket_index]
                    if 0 <= adjacent_bucket_index < len(blocks)
                    else None
                )
                if same_bucket_block is not None and (
                    adjacent_bucket_block is None
                    or same_bucket_block.bottom_edge()
                    < adjacent_bucket_block.bottom_edge()
                ):
                    picked_index = value
                else:
            ...
            for col in range(left, right):
                if 0 <= col < self.secondary_slot:
                    ...
                    if recent_block is not None and (
                        width.primary_slot is None
                        or recent_block.bottom_edge() < width.primary_slot.bottom_edge()
                    ):
                    ...
                    if previous_body_index >= 0 and previous_body_index < len(blocks):
                        ...
                        if (
                            width.tertiary_slot is None
                            or same_bucket_block.bottom_edge()
                            < width.tertiary_slot.bottom_edge()
                        ):
                            width.tertiary_slot = same_bucket_block
                        previous_cache = self.primary_slot[previous_body_index]
                        if previous_cache is not None and (
                            previous_cache.auxiliary_slot is None
                            or current_block.top_edge()
                            > previous_cache.auxiliary_slot.top_edge()
                        ):
                    ...
                    if current_block.is_body_paragraph:
                        for pending_index in pending[col]:
                            if 0 <= pending_index < len(self.primary_slot):
                                ...
                                if pending_neighbor is not None and (
                                    pending_neighbor.secondary_slot is None
                                    or current_block.top_edge()
                                    > pending_neighbor.secondary_slot.top_edge()
                                ):
