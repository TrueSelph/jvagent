...
def similar_style(block: Block, other_block: Block) -> bool:
    ...
    return (
        abs(block.bold_frac() - other_block.bold_frac()) < 0.5
        and abs(block.avg_font_size() - other_block.avg_font_size()) < 1
    )


def is_heading_continuation(
    block: Block, other_block: Block, candidate_number: int
) -> bool:
    """Return whether ``block`` is the next numbered heading continuation of ``other_block``."""
    if (
        block.type != 0
        or block.char_count() >= 500
        or not similar_style(other_block, block)
    ):
        return False
    ...
    if (
        ...
        and is_upper_dominant(block.char_stats)
        and not left_aligned(other_block, block, 1)
        and not right_aligned(other_block, block, 1)
        and center_aligned(other_block, block, 1)
    ):
        return False
        ...
        if tokens.length <= 15:
            ...
            for candidate_item in tokens:
                if is_word_token(candidate_item):
                    continue
                if (
                    candidate_item.type != 2
                    or len(normalized) + len(candidate_item.str) > 20
                ):
                    return False
...
def vertically_close(block: Optional[Block], other_block: Block) -> bool:
    ...
    if block is None:
        return False
    candidate_item = (
        block.bottom_edge() - other_block.top_edge()
        if block.top_edge() > other_block.top_edge()
        else other_block.bottom_edge() - block.top_edge()
    )
    return candidate_item < 2 * other_block.avg_font_size() or (
        x_aligned(block, other_block, 1)
        and candidate_item < 5 * other_block.avg_font_size()
    )
...
def is_equation_adjacent_line(line: Optional[Line], block: Block) -> bool:
    ...
    if (
        tokens.length >= 3
        and (first := first_token(tokens)) is not None
        and len(first.str) <= 1
        and first.type != 1
        and (last := last_token(tokens)) is not None
        and len(last.str) <= 1
        and last.type != 1
    ):
...
def has_substantive_content(
    block: Block, other_block: Optional[Block], candidate_block: Optional[Block]
) -> bool:
    ...
    for token in tokenize_block(block):
        ...
        flag = (
            anchor.top_edge() < line.bottom_edge() + 0.8 * size
            or anchor.bottom_edge() > line.top_edge() - 0.8 * size
        )
        if token.type == 1:
    ...
    if entry_item >= 5:
        return True
    return is_equation_adjacent_line(other_block, block) or is_equation_adjacent_line(
...
def is_cover_page(doc, page) -> bool:
    ...
    return (
        ...
        and (
            page.primary_slot.secondary_slot
            < clamp(0.5 * doc.secondary_slot.secondary_slot, 200, 1000)
