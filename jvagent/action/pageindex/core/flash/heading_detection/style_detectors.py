...
def detect_font_heading(
    page_scan: PageScanState, other_block: Block
    ...
    top_gap = (
        above.bottom_edge() - other_block.top_edge() if above is not None else math.inf
    )
    predecessor = neighbor_right(page_scan.tertiary_slot, other_block)
    predecessor_gap = (
        other_block.bottom_edge() - predecessor.top_edge()
    ...
    if not (
        vertically_close(keyword_match, other_block)
        or vertically_close(above_or_overlap, other_block)
        or (
            other_block.bottom_edge()
            >= 0.8 * page_scan.primary_slot.bounds.bbox_height()
            and other_block.avg_font_size()
            >= page_scan.secondary_slot.secondary_slot.primary_slot + 1
    ...
    if above is not None and above.state_slot != 0:
        inner_reject = (
            ...
            or (
                ...
                and predecessor.bbox_width()
                < page_scan.primary_slot.bounds.bbox_width() / 5
            )
            or (
                predecessor is not None
                and predecessor.char_count() < 0.5 * other_block.char_count()
            )
            or (predecessor is not None and predecessor.weighted_ratio_secondary < 0.33)
            or (
                predecessor is not None
                and predecessor.char_count() < 500
                and predecessor.weighted_ratio_secondary < 0.5
                and alignment_code(predecessor) != 1
            )
            or (
                predecessor is not None
                and predecessor.char_count() < 250
                and predecessor.weighted_ratio_secondary < 0.5
            )
        )
    if inner_reject or (
        predecessor is not None
        and (
            predecessor.weighted_ratio_primary
            < 0.67 * page_scan.secondary_slot.secondary_slot.auxiliary_slot
            or (
                other_block.char_count() < 30
                and predecessor.char_count() < 300
                and predecessor.weighted_ratio_primary
                < 0.8 * page_scan.secondary_slot.secondary_slot.auxiliary_slot
            )
        )
    ):
        return None
    ...
    if (
        predecessor_gap < math.inf
        and predecessor_gap > 0
        and other_block.style_slot
        >= page_scan.secondary_slot.secondary_slot.primary_slot + 2
        and other_block.avg_font_size()
        >= page_scan.secondary_slot.secondary_slot.primary_slot + 1.5
        and other_block.avg_font_size() >= page.primary_slot + 0.5
        and (
            above is None
            or (
                other_block.style_slot >= above.style_slot
                and other_block.avg_font_size() >= above.avg_font_size()
            )
        )
        and predecessor is not None
        and other_block.style_slot >= predecessor.style_slot
    ...
    if (
        dominant_style_of(other_block) in page_scan.primary_slot.style_slot
        and not caps_heavy
        and (
            page.auxiliary_slot == dominant_style_of(other_block)
            or (
                other_block.bold_frac() < 0.9
                and other_block.previous_slot < 0.9
                and alignment_code(other_block) != 3
                and not is_sentence_like(other_block)
            )
        )
    ) or (
        above is not None
    ...
    if (
        ...
        and predecessor_gap > 0
        and other_block.avg_font_size()
        >= page_scan.secondary_slot.secondary_slot.primary_slot + 0.5
        and predecessor is not None
        and other_block.style_slot >= predecessor.style_slot
        and other_block.avg_font_size() >= predecessor.avg_font_size()
        ...
        and predecessor.bbox_width()
        >= 0.25 * page_scan.primary_slot.bounds.bbox_width()
    ):
        return make_plain_candidate(page_scan, 0, other_block)
    ...
    if (
        predecessor_gap > line_height
        and predecessor_gap < 5 * line_height
        and (
            above is None or other_block.avg_font_size() >= above.avg_font_size() + 0.5
        )
        and predecessor is not None
    ...
    if other_block.char_stats.primary_slot[10] != 0:
        ratio = (
            letters + other_block.char_stats.primary_slot[8]
        ) / other_block.char_stats.primary_slot[10]
    else:
        # IEEE division edge case: positive numerator over zero behaves as +inf,
        # which keeps the low-density rejection active.
        ratio = (
            math.inf
            if (letters + other_block.char_stats.primary_slot[8]) > 0
            else math.nan
        )
    ...
    symbol_ratio = (
        symbol_count / letter_total
        if letter_total != 0
        else (math.inf if symbol_count > 0 else math.nan)
    )
    if (
        symbol_count >= 5
        and symbol_ratio > 0.2
    ...
    neighbor_heading_cue = (
        predecessor_gap < math.inf
        and predecessor_gap > neg
        and other_block.avg_font_size() >= page.primary_slot - 0.1
        and predecessor is not None
        and other_block.avg_font_size() >= predecessor.avg_font_size() - 0.1
        and other_block.avg_font_size()
        >= page_scan.secondary_slot.secondary_slot.primary_slot - 0.5
        and (
            (
                predecessor.is_body_paragraph
                and other_block.bbox_width() < 0.95 * predecessor.bbox_width()
                and x_aligned(
                    other_block, predecessor, max(1, other_block.bbox_width() / 10)
                )
        ...
        and (
            (
                other_block.bold_frac() > predecessor.bold_frac()
                and other_block.bold_frac() > 0.5
                and (
                    not first_span_of(predecessor).primary_slot
                    or (
                        above is not None
                        and other_block.bold_frac() > above.bold_frac()
                    )
                )
            )
            or caps_heavy
    ...
    difference_style = bool(
        ...
        and predecessor.avg_font_size() > page.primary_slot - 0.5
        and predecessor_gap > 0
        and predecessor_gap < 3 * other_block.bbox_height()
    ...
    style_change_cue = (
        ...
        and above.is_body_paragraph
        and top_gap > 0
        and top_gap < 3 * other_block.bbox_height()
        and centered
        and dominant_style_of(above) == dominant_style_of(predecessor)
        if predecessor is not None
        else False
    )
    ...
    branch_C1 = (
        topnum
        and centered
        and difference_style
        and predecessor_gap < other_block.bbox_height()
        and predecessor is not None
        and dominant_style_of(predecessor) == page.auxiliary_slot
    )
    branch_C2 = (
        topnum
        and centered
        and predecessor is not None
        and above_or_overlap is not None
        and predecessor is not above_or_overlap
        and predecessor.bottom_edge() - above_or_overlap.top_edge()
        < predecessor.avg_font_size()
        and dominant_style_of(above_or_overlap) == page.auxiliary_slot
        and dominant_style_of(other_block) != page.auxiliary_slot
        and (
            other_block.avg_font_size() >= predecessor.avg_font_size() + 0.5
    ...
    branch_C3 = (
        ...
        and (
            above.avg_font_size() >= other_block.avg_font_size() + 0.5
...
def detect_heading_with_body(
    ...
    if first_line.bold_frac() > 0 and first_line.bold_frac() < 1:
        for entry in enumerate_tokens(tokens):
            ...
            if (
                anchor_token.line() is not first_line
                or not first_anchor_span(anchor_token).primary_slot
            ):
                break
        ...
        for candidate_line in other_block:
            if (
                candidate_line is not first_line
                and candidate_line.primary_slot[0].font_name == font
            ):
                other_count += 1
        ...
        for entry in enumerate_tokens(tokens):
            ...
            if first_anchor_span(anchor_token).font_name != font or (
                line is not first_line and line is not second_line
            ):
                break
            if anchor_token.type == 2 and (
                len(anchor_token.str) > 1 or anchor_token.primary_slot == 4
            ):
                letter_count += 1
            split = index + 1
    ...
    if first_anchor is not None:
        if (
            not first_anchor.primary_slot
            and not first_anchor.measure_slot
            and other_block.previous_slot > 0.5
        ):
            return None
        if (
            not first_anchor.primary_slot
            and first_anchor.font_size < other_block.avg_font_size() + 1
        ):
            rest = tokens.slice(split)
            if rest.length <= 0 or (
                rest.token_at(0) is not None and rest.token_at(0).primary_slot == 3
            ):
                return None
    ...
    if heading_kind is not None and len(heading_kind.numbering) > 1:
        return make_heading_candidate(
            page_scan,
            heading_kind.type,
            heading_kind.group_slot,
            heading_kind.numbering,
            heading_kind.secondary_slot,
            heading_kind.primary_slot,
            True,
        )
    if heading_kind is not None and (
        trie_matches_all(INTRODUCTION_SECTION_TRIE, heading_kind.primary_slot)
        or (
            is_uppercase_dominant(heading_kind.primary_slot)
            and not is_bibliography_entry(other_block)
        )
    ):
        return make_heading_candidate(
            page_scan,
    ...
    if prefix.length > 3:
        ...
        if heading_signature is not None:
            return make_heading_candidate(
                page_scan,
                heading_signature.type,
                heading_signature.group_slot,
                heading_signature.numbering,
                heading_signature.secondary_slot,
                trim_trailing_punct(heading_signature.primary_slot),
                True,
            )
    ...
    if (
        not is_upper_dominant(other_block.char_stats)
        and other_block.is_body_paragraph
        and info_weight(other_block.char_stats) >= 100
    ):
        all_caps_vf = CharStats(prefix.to_string())
        if (
            is_upper_dominant(all_caps_vf)
            and all_caps_vf.primary_slot[2] <= other_block.char_stats.primary_slot[3]
        ):
    ...
    gap = (
        (above_neighbor.bottom_edge() - first_line.top_edge())
    ...
    if first_anchor is not None:
        cond_outer = (
            first_anchor.measure_slot and not first_anchor_span(next_token).measure_slot
        ...
        if (
            ...
            and not first_anchor_span(next_token).measure_slot
            and (
                gap > 1.1 * intersection
                or (
                    last_token(tokenize_block(above_neighbor)) is not None
                    and is_word_token(last_token(tokenize_block(above_neighbor)))
                )
                or last_line_of(above_neighbor).right_edge()
                < first_line.right_edge() - 8 * per_char
            )
            and (
                first_line.right_edge() > second_line.right_edge() - 4 * per_char
    ...
    if (
        ...
        and is_trimmable_token(token)
        and next_token is not None
        and next_token.primary_slot == 2
    ):
        if trie_matches_all(SECTION_KEYWORDS_TRIE, prefix):
            return make_body_heading_candidate(page_scan, 6, other_block, prefix)
        if letter_count > 1:
