...
def _skip_mark_only(span: Span, page_area: float) -> bool:
    ...
    return (
        span.char_count() - span.char_stats.primary_slot[5]
    ) > 1 and span.area() < page_area * 1e-6
...
def build_initial_lines(spans: list[Span], page_bbox) -> list[Line]:
    ...
    for span in spans:
        ...
        if pending_span is not None:
            ...
            if not (
                len(pending_line.primary_slot) <= 0
                or span_continues_line(pending_line, pending_span)
            ):
...
def _is_label_stack(line: Line, other_line: Line, body_ma: float) -> bool:
    ...
    if body_ma <= 0:
        return False
    overlap = min(line.right_edge(), other_line.right_edge()) - max(
        line.left_edge(), other_line.left_edge()
    )
    frac = overlap / max(1e-6, min(line.bbox_width(), other_line.bbox_width()))
    vertical_overlap = min(line.top_edge(), other_line.top_edge()) - max(
        line.bottom_edge(), other_line.bottom_edge()
    )
    vertical_overlap_fraction = vertical_overlap / max(
        1e-6, min(line.bbox_height(), other_line.bbox_height())
    )
    if not (frac > 0.5 and vertical_overlap_fraction < 0.5):
        return False
    upper, lower = (
        (line, other_line)
        if line.center_y() > other_line.center_y()
        else (other_line, line)
    )
    ...
    return (
        upper.avg_font_size() >= 2.0 * body_ma
        and upper.avg_font_size() >= 1.5 * lower.avg_font_size()
    )
...
def cluster_lines(
    lines_container: LinesContainer, other_item: float, candidate_items: list
) -> list[Line]:
    ...
    merged_accent_spans = sorted(
        span_value.font_size
        for line in lines_container.primary_slot
        for span_value in line.primary_slot
        if getattr(span_value, "font_size", 0) > 0
    )
    body_ma = (
        merged_accent_spans[len(merged_accent_spans) // 2]
        if merged_accent_spans
        else 0.0
    )
    ...
    for candidate_line in lines_container.primary_slot:
        ...
        neighbor_line = pick_closer_neighbor(
            line_item, successor_line, candidate_line, other_item
        )
        ...
        if (
            ...
            and neighbor_last_span.state_slot
            == candidate_line.primary_slot[0].state_slot
            and same_x_extent(
                neighbor_last_span,
                candidate_line,
                0.1 * neighbor_last_span.bbox_width(),
            )
            and same_y_extent(
                neighbor_last_span,
                candidate_line,
                0.1 * neighbor_last_span.bbox_height(),
            )
        ):
            if (
                abs(neighbor_last_span.left_edge() - candidate_line.left_edge()) < 0.01
                and abs(neighbor_last_span.top_edge() - candidate_line.top_edge())
                < 0.01
            ):
            ...
            if (
                _is_label_stack(neighbor_line, candidate_line, body_ma)
                and candidate_line.center_y() > neighbor_line.center_y()
            ):
                merged = Line()
