...
def span_continues_line(line: Line, other_span: Span) -> bool:
    ...
    if (
        line_center_y > other_span.top_edge()
        or line_center_y < other_span.bottom_edge()
    ) and (span_center_y > line.top_edge() or span_center_y < line.bottom_edge()):
        return False
    ...
    if line.char_stats.tertiary_slot == 5:
        wide_tolerance *= 2.0
    return (
        other_span.left_edge() > line.right_edge() - wide_tolerance
...
def pick_closer_neighbor(
    ...
    if line is None and other_line is None:
        return None
    entry_item = (
        vertical_distance_in_line_heights(line, candidate_line)
        if line is not None
        else float("inf")
    )
    second_candidate = (
        vertical_distance_in_line_heights(other_line, candidate_line)
...
def should_merge_lines(line: Line, other_line: Line, candidate_items: list) -> bool:
    ...
    if line.char_count() > 0 and other_line.char_count() > 0:
        ...
        if (
            magnitude_ratio(line.previous_slot, other_line.previous_slot) > 2
            and abs(line.previous_slot - other_line.previous_slot) > 10
        ):
            return False
    ...
    line_projection = (
        1.0 / avg_char_width(line) if avg_char_width(line) != 0 else float("inf")
    )
    other_projection = (
        1.0 / avg_char_width(other_line)
        if avg_char_width(other_line) != 0
        else float("inf")
    )
    ...
    if (
        line.char_stats.tertiary_slot == 4
        or line.char_stats.primary_slot[4] > line.char_count() / 2
    ):
        gap_factor /= 2.0
    ...
    if sent_end:
        ...
        all_digits = (
            other_line.char_stats.auxiliary_slot > 0
            and other_line.char_stats.auxiliary_slot
            == other_line.char_stats.primary_slot[1]
        )
    ...
    if (line.char_count() < 40 or inside_col) and (
        same_y_extent(line, other_line, 0.1)
        or same_y_extent(last_span(line), other_line, 0.1)
    ):
        gap_factor *= 1.5
    ...
    if 0 <= other_line.measure_slot < len(candidate_items):
        ...
        other_column = None
    if len(candidate_items) <= 0 or (
        abs(line.right_edge() - col_right) < 5
        and (
            line.measure_slot >= len(candidate_items) - 1
            or not other_column
            or abs(other_line.left_edge() - other_column.get("left", float("inf"))) < 5
        )
    ):
        gap_factor /= 2.0
    ...
    if (
        line.char_count() <= 8
        and line.bbox_width() <= 10 * line.avg_font_size()
        and numbering_kind(line) != 0
        and letter_count(other_line.char_stats) > 0
    ):
    ...
    if (
        inside_col
        and is_upper_dominant(line.char_stats)
        and is_upper_dominant(other_line.char_stats)
    ):
        gap_factor *= 1.5
