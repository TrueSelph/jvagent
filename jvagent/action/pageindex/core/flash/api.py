...
def _optimize(structure, page_texts, do_expand, model):
    ...
    lines = [
        [
            line_text.strip()
            for line_text in (page_text or "").splitlines()
            if line_text.strip()
        ]
...
def page_index_flash(
    pdf,
    summary=True,
    summary_model=None,
    optimize=False,
    optimize_expand=True,
    optimize_model=None,
    ...
    if optimize and structure:
        result["optimize"] = _optimize(
            structure,
            result.get("page_texts") or [],
            optimize_expand,
    ...
    if summary and structure:
        ...
        asyncio.run(
            _summarize(
                structure, page_list, summary_model, concurrency=summary_concurrency
            )
        )
    else:
