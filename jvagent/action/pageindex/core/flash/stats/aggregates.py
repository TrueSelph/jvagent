...
def _percentile_sample_cmp(
    values: tuple[float, float], other_values: tuple[float, float]
) -> float:
...
def compute_page_stats(page, other_lines: list[Line]) -> PageStats:
    ...
    for line in other_lines:
        ...
        for span in line:  # for each span t in line u
            ...
            span_weight = (
                span_weight * span_weight * span.bbox_height()
            )  # weight = tf^2 * height
        ...
        area_metric_samples.append(
            (line.cache_slot, line.area())
        )  # NB: this one is weighted by area
        ...
        bucket_right = (
            20 if right == float("inf") else min(20, math.ceil(right / bucket_size))
        )
        best_gap = float("inf")
        ...
        while idx < bucket_right:
            ...
            if prev_in_bucket is None:
                continue
            gap = (
                max(prev_in_bucket.bottom_edge(), line.top_edge()) - line.bottom_edge()
            )
            if gap < best_gap:
                best_gap = gap
                best_prev = prev_in_bucket
        if best_gap < float("inf") and best_prev is not None:
            overlap_gap_samples.append(
                (best_gap, info_weight(best_prev.char_stats) * line_weight)
            )
...
def compute_doc_stats(pages: list) -> DocStats:
    ...
    return DocStats(
        ...
        upper_width_percentile=weighted_percentile(
            upper_samples, 80
        ),  # NB: 80th percentile, not 50
