...
def find_keyword_clique(
    heading_candidates: list[HeadingCandidate],
) -> Optional[StyleCluster]:
...
def find_ancestor_next_sibling(primary_item: CliqueTreeNode):
    ...
    if primary_item.parent is primary_item:
        return None
    return primary_item.secondary_slot or find_ancestor_next_sibling(
        primary_item.parent
    )
...
def is_member_of_tree(
    doc, block, target_sig: str, sentence_like: bool, node: CliqueTreeNode
) -> bool:
    ...
    if (
        tree_parent_candidate is None
        or tree_parent_candidate.type == 5
        or tree_parent_candidate.is_prominent
    ):
        return False
    ...
    if target_sig != block_style_signature(parent_block) or (
        sentence_like and is_sentence_like(parent_block)
    ):
        return is_member_of_tree(doc, block, target_sig, sentence_like, node.parent)
    if info_weight(block.char_stats) >= max(
        100, 4 * info_weight(parent_block.char_stats)
    ):
        return is_member_of_tree(doc, block, target_sig, sentence_like, node.parent)
    return True
...
def can_share_heading_style(heading, other_heading, neighbor_map) -> bool:
    ...
    from ..model import dominant_style_of, y_overlaps

    if (
        other_heading is None
        or not y_overlaps(heading, other_heading)
        or dominant_style_of(heading) != dominant_style_of(other_heading)
    ):
        return False
    ...
    if (
        ...
        and heading_above.marker_slot != 0
        or other_above is not None
        and other_above.marker_slot != 0
        or heading_right is not None
        and heading_right.marker_slot != 0
        or other_right is not None
        and other_right.marker_slot != 0
    ):
        return True
    if (
        heading_right is not other_right
...
def detect_body_headings(filter_context: CliqueFilterContext) -> list[HeadingCandidate]:
    ...
    for page in filter_context.auxiliary_slot.primary_slot:
        ...
        for block in page.secondary_slot:
            ...
            while (
                filter_context.primary_slot.heading is not None
                and heading_precedes_line(
                    filter_context.primary_slot.heading, page, block
                )
            ):
                left_sib = filter_context.primary_slot.tertiary_slot
                filter_context.primary_slot = (
                    descend_to_deepest_last(left_sib)
                    if left_sib is not None
                    else filter_context.primary_slot.parent
                )
            ...
            anchor = (
                first_tok.anchor_ranges[-1].anchor_span
                if (first_tok is not None and first_tok.anchor_ranges)
                else None
            )
            if (
                block_style != style_key(first_span_of(block))
            ...
            if pred_neigh is not None and sig == block_style_signature(pred_neigh):
                continue
            previous_block = (
                all_blocks[block.orig_index - 1]
                if 0 <= block.orig_index - 1 < len(all_blocks)
                else None
            )
            next_block = (
                all_blocks[block.orig_index + 1]
            ...
            if not (
                is_member_of_tree(
                    ...
                    sig,
                    sentence_like,
                    filter_context.secondary_slot,
                )
                or is_member_of_tree(
                    filter_context,
                    block,
                    sig,
                    sentence_like,
...
def partition_candidates(
    heading_candidates: list[HeadingCandidate], other_outline_nodes: list[OutlineNode]
) -> dict:
...
def interleave_clusters(
    ...
    for labeled_outline_node in other_outline_nodes:
        ...
        while (
            index < len(heading_candidates)
            and compare_heading_order(heading_candidates[index], boundary_candidate) < 0
        ):
