...
class StyleCluster:
    ...
    def __init__(self):
        ...
        self.secondary_slot: SortedKeyList = SortedKeyList(
            key=lambda sort_node: (
                heading_score(sort_node.group_slot),
                heading_order_key(sort_node),
            )
        )
    ...
    def contains(self, other_heading_candidate: HeadingCandidate) -> bool:
        ...
        return idx < len(self.secondary_slot) and self.secondary_slot.key(
            self.secondary_slot[idx]
        ) == self.secondary_slot.key(other_heading_candidate)

    def add(self, other_heading_candidate: HeadingCandidate) -> None:
        self.state_slot[cached_signature(self, other_heading_candidate)] = (
        ...
        idx = self.secondary_slot.bisect_left(other_heading_candidate)
        if idx >= len(self.secondary_slot) or self.secondary_slot.key(
            self.secondary_slot[idx]
        ) != self.secondary_slot.key(other_heading_candidate):
            self.secondary_slot.add(other_heading_candidate)
        if (
            self.primary_slot is None
            or compare_heading_order(other_heading_candidate, self.primary_slot) < 0
        ):
            self.primary_slot = other_heading_candidate
        if (
            self.tertiary_slot is None
            or compare_heading_order(other_heading_candidate, self.tertiary_slot) > 0
        ):
    ...
    def has_nearby_duplicate(self, other_heading_candidate: HeadingCandidate) -> bool:
        ...
        return (
            ...
            and abs(other_heading_candidate.page.page_index - existing.page.page_index)
            < 20
        )
...
def pick_style_bucket(
    outline_context: "OutlineContext", other_heading_candidate: HeadingCandidate
) -> StyleCluster:
...
def has_conflict_in_context(
    outline_context: "OutlineContext", other_heading_candidate: HeadingCandidate
) -> bool:
    """Return True if a candidate conflicts with the existing outline context."""
    if other_heading_candidate.type != 10 and is_in_oo_range(
        outline_context.secondary_slot, other_heading_candidate
    ):
        return True
    if other_heading_candidate.type != 8 and is_in_oo_range(
        outline_context.primary_slot, other_heading_candidate
    ):
        return True
    if len(other_heading_candidate.numbering) <= 0 and is_in_oo_range(
        outline_context.auxiliary_slot, other_heading_candidate
    ):
        return True
    if (
        other_heading_candidate.type != 8
        and len(other_heading_candidate.numbering) > 0
        and outline_context.primary_slot.size() > 0
    ):
        return True
    return False


def is_compatible_with_context(
    outline_context: "OutlineContext", other_heading_candidate: HeadingCandidate
) -> bool:
    ...
    if text is not None:
        ...
        if previous_block.isolated_centered and not candidate_block.isolated_centered:
            return False
        if (
            not other_heading_candidate.is_prominent
            and heading_score(previous_block) > heading_score(candidate_block) + 0.5
        ):
            return False
        if (
            text.is_prominent
    ...
    if not style_cluster.has_nearby_duplicate(
        other_heading_candidate
    ) and has_style_neighbor(style_cluster, other_heading_candidate, 1.0):
        return True
    if len(other_heading_candidate.numbering) == 1 and has_style_neighbor(
        outline_context.tertiary_slot, other_heading_candidate, 1.0
    ):
        return True
    return False
...
class OutlineContext:
    ...
    def has_nearby_duplicate(self, other_heading_candidate: HeadingCandidate) -> bool:
        return pick_style_bucket(self, other_heading_candidate).has_nearby_duplicate(
            other_heading_candidate
        )
...
def insert_numbering(
    primary_item: NumberingTrie, other_heading_candidate: HeadingCandidate, index: int
) -> None:
    ...
    if index == len(other_heading_candidate.numbering):
        ...
        return
    reference_item = primary_item.primary_slot.get(
        other_heading_candidate.numbering[index]
    )
    if reference_item is None:
        reference_item = NumberingTrie()
        primary_item.primary_slot[other_heading_candidate.numbering[index]] = (
            reference_item
        )
    insert_numbering(reference_item, other_heading_candidate, index + 1)


def count_sibling_numberings(
    primary_item: NumberingTrie, other_heading_candidate: HeadingCandidate, index: int
) -> int:
        ...
        for cluster in clusters:
            ...
            for state_candidate in cluster_candidates:
                ...
                for style_cluster in item_list:
                    if not style_cluster.has_nearby_duplicate(
                        state_candidate
                    ) and has_style_neighbor(style_cluster, state_candidate, 1.0):
...
def _apply_heading_to_state(
            ...
            for anchor in token.anchor_ranges:
                ...
                if tree is None:
                    tree = SortedKeyList(
                        key=lambda heading: (
                            heading["size"],
                            heading_order_key(heading["heading"]),
                        )
                    )
    ...
    state.previous_slot = min(
        state.previous_slot, other_heading_candidate.page.page_index
    )
...
def compare_heading_depth(
    ...
    if (heading_candidate.type == 1 and other_heading_candidate.type == 1) or (
        heading_candidate.type == 4 and other_heading_candidate.type == 4
    ):
    ...
    if (
        heading_candidate.has_numbering
        and other_heading_candidate.has_numbering
        and abs(first_span_of(heading_block).font_size - first_span_of(block).font_size)
        < 0.9
    ):
        return 0
    ...
    if abs(score - other_score) > 1.9 or (
        abs(score - other_score) > 0.9 and (not heading_in_clique or not in_value)
    ):
    ...
    if (heading_in_clique and other_heading_candidate.type != 4) or (
        in_value and heading_candidate.type != 4
    ):
