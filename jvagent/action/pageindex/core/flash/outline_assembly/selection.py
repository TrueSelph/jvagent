...
def min_font_distance(
    state: OutlineState, other_heading_candidate: HeadingCandidate
) -> float:
    ...
    for token_list in (
        other_heading_candidate.secondary_slot,
        other_heading_candidate.primary_slot,
    ):
...
def should_reject_heading(
    ...
    if other_heading_candidate.type == 0:
        for previous in state.style_slot:
            ...
            if style_cluster is not None and is_in_oo_range(
                style_cluster, other_heading_candidate
            ):
                return True
    if (
        state.primary_slot is not None
        and state.primary_slot.is_prominent
        and other_heading_candidate.type == 0
    ):
        count = 0
        ...
        if count >= 3:
            return True
    if (
        other_heading_candidate.type == 1
        and len(other_heading_candidate.numbering) <= 1
        and (
            (
                0
                if (
                    other_heading_candidate.type != 1
                    or len(other_heading_candidate.numbering) <= 0
                )
                else count_sibling_numberings(
                    state.cache_slot, other_heading_candidate, 0
                )
            )
            <= 1
        )
    ...
    if other_heading_candidate.type in (1, 5, 9, 10, 7):
        ...
        if distance <= 0.9:
            ...
            reject = not (
                is_caps_heavy(other_heading_candidate.group_slot)
                and other_heading_candidate.tertiary_slot is not None
                and other_heading_candidate.group_slot.bottom_edge()
                - other_heading_candidate.tertiary_slot.top_edge()
                < 5 * other_heading_candidate.group_slot.bbox_height()
            )
    if reject:
        return True
    if other_heading_candidate.type == 1:
        first = other_heading_candidate.numbering[0]
        if (first < state.secondary_slot and first < state.tertiary_slot) or (
            state.secondary_slot > 0 and first > state.secondary_slot + 2
        ):
            return True
        if (
            len(other_heading_candidate.numbering) == 1
            and state.auxiliary_slot is not None
        ):
            ...
            candidate_style = (
                style_key(
                    first_anchor_span(first_token(other_heading_candidate.primary_slot))
                )
                if other_heading_candidate.primary_slot is not None
                and first_token(other_heading_candidate.primary_slot) is not None
                else ""
            )
            state_style = (
                style_key(
                    first_anchor_span(first_token(state.auxiliary_slot.primary_slot))
                )
                if state.auxiliary_slot.primary_slot is not None
                and first_token(state.auxiliary_slot.primary_slot) is not None
                else ""
            )
            if candidate_style != state_style:
                ...
                if abs(
                    dominant_font_size(other_heading_candidate.group_slot)
                    - dominant_font_size(existing)
                ) > 0.5 or _round_half_up_to_int(
                    other_heading_candidate.group_slot.bold_frac()
                ) != _round_half_up_to_int(
                    existing.bold_frac()
                ):
                    return True
    if (
        state.primary_slot is not None
        and other_heading_candidate.type == 4
        and state.primary_slot.type == 4
        and len(state.primary_slot.numbering) > 0
        and len(other_heading_candidate.numbering) > 0
        and (
            state.primary_slot.numbering[0] > other_heading_candidate.numbering[0]
            or (
                len(other_heading_candidate.numbering) == 1
                and state.primary_slot.numbering[0]
                == other_heading_candidate.numbering[0]
            )
        )
    ):
        return True
    if (
        ...
        if len(candidate_tokens) == len(tokens):
            ...
            for heading in range(len(candidate_tokens)):
                token = (
                    candidate_tokens[heading]
                    if heading < len(candidate_tokens)
                    else None
                )
                ...
                if token is None or state_token is None:
                    same = False
                    break
                if _strip_diacritics(token.str.lower()) != _strip_diacritics(
                    state_token.str.lower()
                ):
                    same = False
                    break
            if same:
...
def push_heading_to_state(
    ...
    if len(other_heading_candidate.numbering) > 0:
        ...
        state.style_slot[len(other_heading_candidate.numbering) - 1] = (
...
class HierarchyStack:
    ...
    def push(self, other_outline_node: OutlineNode) -> None:
        ...
        self.secondary_slot = (
            self.secondary_slot or other_outline_node.heading.type == 4
        )
        self.tertiary_slot = (
            self.tertiary_slot or other_outline_node.heading.is_prominent
        )


def find_parent_heading(
    stack: HierarchyStack, other_heading_candidate: HeadingCandidate
    ...
    while stack.primary_slot:
        ...
        if (
            other_heading_candidate.is_prominent
            and len(other_heading_candidate.numbering) <= 1
            and state_candidate.type != 8
        ):
            stack.pop()
        ...
        if state_candidate.is_prominent and other_heading_candidate.type == 5:
            ...
            heading = state_candidate
            continue
        cmp = compare_heading_depth(
            state_candidate, other_heading_candidate, stack.auxiliary_slot
        )
        if cmp != -1:
            if cmp == 1:
            ...
            if (
                state_candidate.type != other_heading_candidate.type
                and other_heading_candidate.type in (4, 2)
                and not stack.tertiary_slot
                and is_appendix_nesting_ok(stack, other_heading_candidate, heading)
            ):
                first_number = (
                    other_heading_candidate.numbering[0]
                    if other_heading_candidate.numbering
                    else 0
                )
                if heading is None:
                    if first_number == 1:
                    ...
                    if (
                        other_heading_candidate.type == heading.type
                        and other_heading_candidate.numbering
                        and heading.numbering
                        and first_number > heading.numbering[0]
                    ):
                        return stack_outline_node
        stack.pop()
        heading = state_candidate
...
def is_appendix_nesting_ok(
    stack: HierarchyStack,
    other_heading_candidate: HeadingCandidate,
    candidate_heading_candidate: Optional[HeadingCandidate],
) -> bool:
...
def extract_sub_headings(
    doc,
    state: OutlineState,
    parent_node: Optional[OutlineNode],
    cluster_candidates: list[HeadingCandidate],
) -> list[OutlineNode]:
    ...
    page_index = (
        (parent_node.heading.page.page_index - 1) if parent_node is not None else 0
    )
    acc = 0
    ...
    while page_index < end_pg:
        ...
        if getattr(heading_page, "state_slot", False):
            for block in heading_page.output_slot:
                if (
                    page_index >= first.page.page_index - 1
                    and block.reading_order_index
                    >= first.group_slot.reading_order_index
                ):
                    break
    ...
    parent_anchor = (
        parent_node
        if (parent_node is not None and parent_node.heading.type == 5)
        else None
    )
    ...
    while index < len(cluster_candidates):
        ...
        if not (
            ...
            or (
                cluster_candidate.type == 11
                and cluster_candidate.has_numbering
                and parent_anchor is not None
                and index <= 1
            )
        ):
            next_item = (
                cluster_candidates[index + 1]
                if index + 1 < len(cluster_candidates)
                else None
            )
            if (
                next_item
                and next_item.type == 5
                and next_item.page is cluster_candidate.page
                and next_item.tertiary_slot is cluster_candidate.tertiary_slot
            ):
                index += 1
                continue
            break
    ...
    if (
        ...
        or (
            len(out) == 2
            and out[0].heading.has_numbering
            and out[1].heading.has_numbering
        )
