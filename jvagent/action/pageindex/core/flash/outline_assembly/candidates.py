...
def _viewport_y_fraction(viewport_box, rot: int, user_x: float, user_y: float) -> float:
    ...
    viewport_y = (
        x_axis_scale * user_x
        + y_axis_scale * user_y
        + (viewport_offset - x_axis_scale * center_x - y_axis_scale * center_y)
    )
...
class HeadingCandidate:
    ...
    def __init__(
        ...
        if viewport_box_value is not None:
            self.auxiliary_slot = _viewport_y_fraction(
                viewport_box_value,
                getattr(page, "rot", 0) or 0,
                group_value.left_edge(),
                group_value.top_edge(),
            )
        else:
            page_height = page.bounds.bbox_height() or 1.0
            self.auxiliary_slot = (
                page.bounds.top_edge() - group_value.top_edge()
            ) / page_height
...
def compare_heading_order(
    heading_candidate: HeadingCandidate, other_heading_candidate: HeadingCandidate
) -> float:
    """Order by page, then block reading position."""
    if heading_candidate.page.page_index != other_heading_candidate.page.page_index:
        return (
            heading_candidate.page.page_index - other_heading_candidate.page.page_index
        )
    return _compare_block_order(
        heading_candidate.group_slot, other_heading_candidate.group_slot
    )
...
def heading_order_key(heading_candidate: HeadingCandidate) -> tuple:
    ...
    return (
        heading_candidate.page.page_index,
        _column_index(heading_candidate.group_slot),
...
def is_script_compatible(
    number: int, other_heading_candidate: HeadingCandidate
) -> bool:
...
def cached_signature(
    primary_item: "StyleCluster", other_heading_candidate: HeadingCandidate
) -> str:
...
def is_in_oo_range(
    ...
    if primary_item.primary_slot is None or primary_item.tertiary_slot is None:
        return False
    return (
        compare_heading_order(other_heading_candidate, primary_item.primary_slot) >= 0
        and compare_heading_order(other_heading_candidate, primary_item.tertiary_slot)
        <= 0
    )


def has_style_neighbor(
    style: "StyleCluster",
    ...
    def fcheck(item):
        ...
        if (
            abs(heading_score(measure_item) - heading_score(line_value))
            >= candidate_item
        ):
            state_item = False
        elif len(other_heading_candidate.numbering) > 0 and len(item.numbering) > 0:
            state_item = other_heading_candidate.type == item.type and len(
                other_heading_candidate.numbering
            ) == len(item.numbering)
        elif (
            len(other_heading_candidate.numbering) <= 0 and len(item.numbering) > 1
        ) or (len(item.numbering) <= 0 and len(other_heading_candidate.numbering) > 1):
            state_item = False
        else:
            ...
            if block or other_centered:
                ...
                if (
                    first_span_of(measure_item).font_style()
                    != first_span_of(line_value).font_style()
                ):
                    state_item = False
                else:
                    state_item = (
                        abs(
                            dominant_font_size(measure_item)
                            - dominant_font_size(line_value)
                        )
                        < candidate_item
                    )
        if state_item:
