...
def compute_max_heading_gap(
    outline_nodes: list[OutlineNode], other_number: int
) -> dict:
    ...
    for stack_outline_node in outline_nodes:
        page_pos = (
            stack_outline_node.heading.page.page_index
            + stack_outline_node.heading.auxiliary_slot
        )
...
def has_table_or_prominent(outline_nodes: list[OutlineNode]) -> bool:
    ...
    return any(
        secondary_item.heading.type == 5 or secondary_item.heading.is_prominent
        for secondary_item in outline_nodes
    )
...
def is_landscape_or_empty(doc) -> bool:
    ...
    for page in doc.primary_slot:
        if (
            page.bounds.bbox_width() > page.bounds.bbox_height()
            and page.primary_slot.secondary_slot < 1e3
        ):
            secondary_item += 1
            candidate_item += page.primary_slot.secondary_slot
    count_item = len(doc.primary_slot)
    return secondary_item >= 0.9 * count_item or (
        secondary_item >= 0.7 * count_item
        and candidate_item >= 0.5 * doc.secondary_slot.state_slot
    )
...
def build_heading_from_block(
    block: Block, page, anchor: Optional[Block] = None
) -> HeadingCandidate:
    ...
    if numbering_kind(block.line()) == 1:
        ...
        if num_str:
            try:
                parts = [
                    int(number_part)
                    for number_part in num_str.replace("．", ".").split(".")
    ...
    body_size_threshold = (
        page.primary_slot.primary_slot + 0.5 if page.primary_slot else 0
    )
    ja_flag = block.avg_font_size() > body_size_threshold + 1.5 or (
        block.bold_frac() > 0.5 and block.avg_font_size() >= body_size_threshold
    )
...
def assemble_outline(doc, labeled: list[OutlineNode]) -> list[OutlineNode]:
    ...
    if len(labeled) + len(general) > 0:
        ...
        filtered = detect_body_headings(
            CliqueFilterContext(
                doc,
                combined,
                lambda line, other_line: compare_heading_depth(
                    line, other_line, clique
                ),
            )
        )
    ...
    for cluster in clusters:
        ...
        sub_clique = (
            find_keyword_clique(cluster_candidates) if cluster_candidates else None
        )
...
def _heading_appears_at_page_top(heading: HeadingCandidate) -> bool:
    ...
    for block in page.secondary_slot or []:
        if (
            block is top_heading
            or getattr(block, "reading_order_index", 0) >= group_index
        ):
...
def outline_to_dict_tree(
    outline_node_list: list[OutlineNode], total_pages: int
) -> list[dict]:
    ...
    def _walk_nodes(items: list[OutlineNode]) -> list[dict]:
        ...
        for item in items:
            ...
            child = (
                _trim_unicode_ws(str(prefix_tokens))
    ...
    for line, outline_entry in enumerate(flat):
        if line + 1 < len(flat):
            ...
            # it.
            boundary = (
                nxt["start_index"] - 1 if nxt["_appear_start"] else nxt["start_index"]
            )
        else:
        ...
        outline_entry["end_index"] = max(
            ...
            (
                boundary
                if boundary > outline_entry["start_index"]
                else outline_entry["start_index"]
            ),
        )
    if flat:
