...
from .model import Block, Rect, block_text, deaccented_text, numbering_kind
...
REFERENCES_KEYWORDS = frozenset(
    _normalize_text_key(text_value)
    for text_value in _REFS_DICT_RAW.get("references", [])
    if text_value
)
...
def extract_toc(
    ...
    for index_value, spans in enumerate(parsed):
        ...
        viewport_width, viewport_height = abs(viewport_x1 - viewport_x0), abs(
            viewport_y1 - viewport_y0
        )
    ...
    if (
        doc.secondary_slot.state_slot <= 300
        or doc.secondary_slot.previous_slot <= 200
        or doc.secondary_slot.tertiary_slot in (0, 2, 10)
        or is_landscape_or_empty(doc)
    ):
        ...
        if use_embedded_toc:
            ...
            result["structure"], result["toc_source"] = apply_embedded_toc(
                ...
                page_texts=[
                    "\n".join(
                        block_text(block) for block in (page.secondary_slot or [])
                    )
                    for page in pages
                ],
            )
    ...
    for page in pages:
        for block in page.output_slot or []:
            if block.type == 0:
                block.is_body_paragraph = is_body_paragraph(
                    doc.secondary_slot, page, block
                )
    ...
    if is_outline_valid(doc, outline_nodes):
        ...
        if doc.secondary_slot.tertiary_slot == 7 or (
            page_count >= 3
            and compute_max_heading_gap(outline_nodes, 1)["max_gap"]
            > (0.65 if doc.secondary_slot.tertiary_slot == 4 else 0.85) * page_count
        ):
            outline_nodes = []
