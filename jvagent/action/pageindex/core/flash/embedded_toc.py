...
def _normalize_title(title: str) -> str:
    ...
    return "".join(
        _roman_to_arabic(token) if _is_roman(token) else token for token in tokens
    )
...
def _title_template(title: str) -> str:
    ...
    tokens = _TOKEN.findall(_LATEX_SPAN.sub("", title.lower()))
    return "".join(
        "#" if _is_roman(token) else _DIGIT_RUN.sub("#", token) for token in tokens
    )
...
def validate_bookmarks(entries: list[dict], n_pages: int) -> list[dict]:
    ...
    for entry in kept:
        ...
        out.append(
            {"title": entry["title"], "level": len(stack), "page": entry["page"]}
        )
    return out
...
def classify_bookmarks(entries: list[dict], n_pages: int) -> int:
    ...
    if (
        "#" in top_template
        and 2 * top_count >= len(entries)
        and (
            top_template.count("#") >= 2
...
def _same_heading(node: dict, chapter: dict) -> bool:
    ...
    if not node_norm or not chapter_norm:
        return False
    return (
        node_norm == chapter_norm
        or node_norm.endswith(chapter_norm)
...
def _title_in_text(entry: dict, page_norms: list[str]) -> bool:
    ...
    return any(
        norm in page_norms[page_pos] for page_pos in range(window_start, window_end)
    )
...
def apply_embedded_toc(
    ...
    if tier == SKELETON:
        return (
            merge_bookmark_skeleton(structure, entries, n_pages, page_texts=page_texts),
            "hybrid",
        )
