...
class ColumnDetectionContext:
    ...
    def __init__(self, primary_item, secondary_item, candidate_item):
        ...
        self.tertiary_slot = (
            math.floor(2 * math.log2(len(candidate_item))) if candidate_item else 0
        )
        self.state_slot = primary_item.bbox_width() / 6.0
        self.auxiliary_slot = _max_nan_propagating(
            0.5 * secondary_item.primary_slot,
            _min_nan_propagating(
                1.1 * (secondary_item.tertiary_slot - secondary_item.primary_slot),
                3.0 * secondary_item.primary_slot,
            ),
        )
    ...
    for size_value in range(len(events) - 1):
        ...
        if score is not None:
            ...
            out_candidates.append(
                SplitCandidate(key_value, score_value, direction, score)
            )
...
def _score_gutter_gap(
    ...
    secondary_item = reference_item = distance_accumulator = width_value = (
        wide_accumulator
    ) = 0.0
    min_edge = math.inf
    ...
    while sample_item >= 0:
        ...
        if next_number == 1:
            leading_edge, trailing_edge = (
                sweep_line.bottom_edge(),
                sweep_line.top_edge(),
            )
        else:
            leading_edge, trailing_edge = (
                sweep_line.left_edge(),
                sweep_line.right_edge(),
            )
    ...
    while count_item < len(reference_items):
        ...
        if event_position - score_value < 1:
            quantity += 1
            following_edge_max_width = max(
                following_edge_max_width, sweep_line.bbox_width()
            )
        if numbering_kind(sweep_line) == 1:
            following_numbering_count += 1
            numbering_score = _max_nan_propagating(
                numbering_score, numbering_value(sweep_line)
            )
    ...
    if next_number == 1:
        ...
        if (secondary_item <= 100 and after <= 100) and (
            key_value < other_rect.left + 0.2 * root_width
            or score_value > other_rect.left + 0.8 * root_width
        ):
            return None
    ...
    if next_number == 1:
        ...
        if (
            following_numbering_count >= 3
            and numbering_score >= 6
            and right_gap < threshold
        ):
            ...
            line_value *= 1 + factor
        line_value *= max(
            1, 2 - abs(root_center_x - (key_value + score_value) / 2) / root_width * 10
        )

    if next_number == 0:
        candidate_width = candidate_rect.bbox_width()
        line_value *= (
            max(wide_accumulator, following_edge_max_width)
            / candidate_width
            * (max(preceding_max_width, next_gap) / candidate_width)
        )
        ...
        if min_value < root_center_x and max_value > root_center_x:
            ...
            line_value *= 1 + min(left_center_distance, value) / max(
                left_center_distance, value
            )
        ...
        if key_value < edge_top or score_value > edge_bot:
            ...
            if (
                key_value < edge_top
                and event_count >= 1
                and wide_accumulator < root_width / 4
            ) or (
                score_value > edge_bot
                and following_numbering_count >= 1
                and following_edge_max_width < root_width / 4
            ):
                line_value *= 9
        if (
            2 * width_value >= limit_number
