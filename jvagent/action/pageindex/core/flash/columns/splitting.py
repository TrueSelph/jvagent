...
def recursive_split(
    ...
    if current_rect.bbox_width() >= context.state_slot:
        collect_gutter_candidates(
            ...
            vertical_events,
            0,
            current_rect.bbox_width(),
            context.auxiliary_slot,
            split_candidates,
        )
    ...
    if len(split_candidates) <= 0:
        ...
        while gap_scan_index < len(horizontal_events) - 1:
            ...
            if (
                event_is_start
                or active_overlap_count > 2
                or width_value < reference_rect.left + reference_rect.bbox_width() / 6
            ):
                gap_scan_index += 1
                continue
            previous_gap_index = gap_indices[-1] if gap_indices else None
            if (
                previous_gap_index is not None
                and width_value
                < horizontal_events[previous_gap_index].position
                + reference_rect.bbox_width() / 10
            ):
        ...
        for index in range(len(gap_indices) + 1):
            pos = (
                gap_indices[index]
                if index < len(gap_indices)
                else len(horizontal_events)
            )
            for event_index in range(cursor, pos):
                ...
                horizontal_events[pos].position
                if pos < len(horizontal_events)
                else current_rect.right_edge()
            )
            out.append(
                Rect(
                    horizontal_events[cursor].position,
                    end,
                    current_rect.top,
                    current_rect.bottom_edge(),
                )
            )
            cursor = pos + 1
        return out
    ...
    if best.direction == 0:
        ...
        upper = recursive_split(
            context,
            upper_horizontal_events,
            upper_vertical_events,
            reference_rect,
            Rect(upper_left, split_max, current_rect.top, best.end),
            depth + 1,
            column_offset,
        )
    ...
    left = recursive_split(
        ...
        left_vert,
        reference_rect,
        Rect(current_rect.left, best.start, split_max, left_bottom),
        depth + 1,
        column_offset,
    )
...
def detect_columns(column: ColumnDetectionContext) -> list[Rect]:
    ...
    horizontal_events.sort(
        key=lambda split_event: (
            split_event.position,
            0 if split_event.is_start else 1,
            split_event.line.bbox_width(),
        )
    )
