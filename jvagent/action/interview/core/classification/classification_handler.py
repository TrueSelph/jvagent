...
class ClassificationResult:
    ...
    intent: (
        str  # "CANCELLATION", "CONFIRMATION", "UPDATE", "DECLINE", "SUBMISSION", "NONE"
    )
    ...
    extracted_data: Optional[Dict[str, Any]] = (
        None  # Extracted responses for "SUBMISSION" intent
    )

    # Metadata for tracking extraction source
    from_data_input_field: bool = (
        False  # True if data_input_field contributed to this result
    )
...
class ClassificationHandler:
    ...
    def _extract_field_values(
        ...
        for item in raw_list:
            ...
            if (
                allowed_field_names is not None
                and field_name not in allowed_field_names
            ):
                logger.debug(
            ...
            if isinstance(val, str) and (
                not val.strip() or val.strip().upper() == "N/A"
            ):
                continue
        ...
        if session.question_graph:
            ...
            if first_question_name:
                try:
                    ...
                    logger.debug(
                        "Failed to get first question node for path walker in extract_data_input_values"
                    )
    ...
    async def _get_context_data_note(
        ...
        if provider_name and session:
            try:
                ...
                logger.debug(
                    f"Could not fetch context data from provider '{provider_name}': {e}"
                )
            ...
            if isinstance(value, list) and value:
                ...
                if len(value) <= compact_threshold:
                    ...
                    context_notes.append(
                        f"{key.replace('_', ' ')}: {len(value)} {options_text}"
                    )
    ...
    async def build_classification_context(
        ...
        if session.question_graph:
            ...
            if first_question_name:
                try:
                    ...
                    logger.exception(
                        "Failed to get first question node for path walker"
                    )
        ...
        if first_node and visitor:
            ...
            active_questions = [
                q for q in session.question_graph if q.get("name") in reachable_names
            ]
        else:
        ...
        for item in active_questions:
            ...
            other_constraints = {
                ...
                if k
                not in ("description", "data_input_field", "extraction_mode", "options")
            }
            constraint_strs = [f"{k}: {v}" for k, v in other_constraints.items()]
            constraints_display = (
                ", ".join(constraint_strs) if constraint_strs else "(none)"
            )
            ...
            if extraction_mode == "select":
                ...
                if static_options and isinstance(static_options, list):
                    options_str = ", ".join(
                        str(opt) for opt in static_options[:10]
                    )  # Limit to first 10
                    ...
                    if provider_name:
                        try:
                            ...
                            func = get_input_context_provider(
                                session.interview_type, provider_name
                            )
                            if func:
                                dynamic_context = (
                                ...
                                if dynamic_context and isinstance(
            ...
            context_data_note = await self._get_context_data_note(
        ...
        if first_node:
            ...
            unanswered = session.get_unanswered_questions()
        current_question = (
            unanswered[0] if unanswered else "None (all questions answered)"
        )
    ...
    async def classify_and_extract(
        ...
        if interpretation_available:
            ...
            if (
                utterance
                and utterance.strip()
                and utterance.strip() != interaction.interpretation.strip()
            ):
        ...
        try:
            ...
            context = await self.build_classification_context(
                session, excluded_fields=excluded_fields, visitor=visitor
            )
            ...
            if not model_action:
                logger.warning(
                    f"{self.action.get_class_name()}: Could not get model action for unified classification"
                )
                return ClassificationResult(intent=Intent.NONE)
            ...
            primary_text = (
                ...
                else (interaction.interpretation if interpretation_available else "")
            )
            ...
            if not result:
                ...
                if data_input_values:
                    return self._build_result_from_data_inputs(
            ...
            try:
                ...
                logger.warning(
                    f"{self.action.get_class_name()}: Invalid intent value '{intent_str}', defaulting to NONE"
                )
                intent = Intent.NONE
                    ...
                    for item in raw_list:
                        if isinstance(item, dict) and len(item) == 1:
                            ...
                            if isinstance(v, str) and v.strip().upper() == "N/A":
                                fname = (
                                    fname.strip() if isinstance(fname, str) else None
                                )
                                if fname and (
                                    not allowed_field_names
                                    or fname in allowed_field_names
                                ):
            ...
            logger.error(
                f"{self.action.get_class_name()}: Failed to parse unified classification JSON: {e}",
                exc_info=True,
            )
