...
class QuestionNodeCache:
    ...
    async def get_cached_node(
        self, question_name: str, fetch_func: callable
...
class BranchCache:
    ...
    def invalidate_from(self, question_name: str, question_graph: list) -> None:
        ...
        self.session.context[CACHE_KEY_BRANCH_CACHE] = cache
        logger.debug(
            f"Branch cache invalidated from '{question_name}' onward ({len(names) - start_idx} entries)"
        )
    ...
    def record_pruned_response(
        self, question_name: str, old_value: Any, reason: str
    ) -> None:
        ...
        try:
            dependency_snapshot = (
                dict(self.session.responses)
                if isinstance(self.session.responses, dict)
                else {}
            )
        except Exception:
            dependency_snapshot = {}
        ...
        pruned[question_name] = {
            ...
            "pruned_at": (
                ...
                if hasattr(self.session, "last_modified")
                else None
            ),
