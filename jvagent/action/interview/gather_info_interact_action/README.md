...
├── gather_info_interact_action.py   # Main action implementation (Interact Action class)
...
- **`gather_info_interact_action.py`**: Contains the `GatherInfoInteractAction` class with business logic, lifecycle hooks, and configuration properties
...
```yaml
id: gather_info_interact_action
name: Gather Info Interact Action
version: 1.0.0
enabled: true
config:
  timeout: 30
...
2. Modify `gather_info_interact_action.py` to implement your logic
...
2. **`endpoints.py`** defines all HTTP endpoints:
   ...
   async def my_endpoint(action_id: str):
       action = await GatherInfoInteractAction.get(action_id)
       # ... endpoint logic
   ```
