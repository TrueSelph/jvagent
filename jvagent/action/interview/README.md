# Interview Action (`jvagent/interview`)
...
> **Important:** Reference packages live under `interview/examples/` (not auto-discovered). Live skills go in the app action overlay `skills/` path.
...
Existing tests under `tests/action/interview/`:
...
- [ ] `requires-actions` lists all dependencies (`InterviewAction`, `ZoonAPIAction`, etc.) — **gate only**; lifecycle hooks bind via `extends: action:jvagent/interview` (or sole lifecycle-capable required Action), not `agent.yaml` order
