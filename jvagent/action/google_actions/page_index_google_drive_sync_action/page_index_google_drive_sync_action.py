...
class PageIndexGoogleDriveSyncAction(GoogleAction):
    ...
    async def ingest_documents_from_google_drive(self, google_drive_folders:list=[], remove_deleted_documents:bool=False) -> dict:
        ...
        page_index_action = await self.get_action(self.page_index_action)
        if not google_drive_action:
            logger.warning("No GoogleDriveAction found! Unable to retrieve documents from Google Drive")
            return
        elif not page_index_action:
            logger.warning(f"No {self.page_index_action} found! Unable to ingest documents from Google Drive")
            return
        
        if not google_drive_folders:
            google_drive_folders = self.google_drive_folders
        elif not self.google_drive_folders and not google_drive_folders:
            return
        ...
        for google_drive_folder in google_drive_folders:
            ...
            files = await google_drive_action.list_files(with_link=True, folder_id=google_drive_folder_id)
            metadata=google_drive_folder.get("metadata", {})
            old_folder_result = await self.node(node="GoogleDriveDocuments", folder_id=google_drive_folder_id)

            if old_folder_result:
                file_result = google_drive_action.compare_files(old_files=old_folder_result.files, new_files=files)
                old_folder_result.files = files
                old_folder_result.metadata = metadata
                await old_folder_result.save()
            else:
                file_result = google_drive_action.compare_files(old_files=[], new_files=files)
                google_drive_documents_node = await GoogleDriveDocuments.create(folder_id=google_drive_folder_id, files=files, metadata=metadata)
            ...
            logger.warning(f"page_index_action.node_summary: {page_index_action.node_summary}")
            ...
            for modified_result in file_result['modified']:
                ...
                # Delete old document
                await delete_document(old_file.get("name", ""), collection_name=self.agent_id)
        ...
        return {"status": "completed", "message": message, "documents_ingested": document_ingested}
