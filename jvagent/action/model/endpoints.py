...
async def query_model_action(
    ...
    Supports both synchronous and streaming responses, and both text-only
    and multimodal (text + images) queries.
    ...
    Returns:
        For sync: Dictionary with response and metadata
        For stream: StreamingResponse with SSE events
...
async def list_model_action_templates(action_id: str) -> Dict[str, Any]:
    """List available templates for a model action.

    Args:
        action_id: ID of the model action

    Returns:
        Dictionary with list of template names
...
async def render_model_action_template(
    ...
    Args:
        ...
        variables: Template variables as dictionary
