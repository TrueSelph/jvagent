...
class BrainTriggerAction(InteractAction):
    ...
    async def execute(self, visitor: "InteractWalker") -> None:
        ...
        await self._check_proactive_triggers(visitor, interaction, conversation)
        ...
        for task in pending_tasks:
            ...
            if not should_trigger and trigger_condition not in ("none", ""):
                ...
                if trigger_condition in utterance_lower:
                    should_trigger = True
                    logger.debug(f"BrainTriggerAction: Keyword trigger '{trigger_condition}' matched")
            ...
            if should_trigger:
                directive = (
                    f"PROACTIVE FOLLOW-UP: {task.get('description', '')}\n"
                    f"CONTEXT: {metadata.get('context', '')}"
                )
                await visitor.add_directive(directive)
                await conversation.update_task(status="completed", task_id=task.get("task_id"))
                triggered_count += 1
                logger.info(f"BrainTriggerAction: Triggered proactive task: {task.get('description')}")

        if triggered_count > 0:
            await interaction.save()
            logger.info(f"BrainTriggerAction: Fired {triggered_count} proactive task(s)")
