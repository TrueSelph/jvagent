...
class ElevenLabsTTSModule(TTSModule):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "eleven_turbo_v2", voice: str = "Sarah", action=None, **kwargs):
    ...
    async def invoke(self, text: str, as_base64: bool = False, as_url: bool = True) -> Optional[Union[str, bytes]]:
            ...
            try:
                ...
                if asyncio.iscoroutinefunction(self.action.save_file):
                    ...
                    success = loop.run_until_complete(
                        self.action.save_file(storage_path, audio, metadata={"type": "tts_audio"})
                    )
                else:
                    success = self.action.save_file(storage_path, audio, metadata={"type": "tts_audio"})
                
                if success:
                    # Get URL using action's get_file_url method
                    if asyncio.iscoroutinefunction(self.action.get_file_url):
                        url = loop.run_until_complete(self.action.get_file_url(storage_path))
                    else:
