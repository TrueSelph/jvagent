...
class WhatsAppAction(Action):
    ...
    async def on_reload(self) -> None:
        ...
        try:
            ...
            if isinstance(result, dict) and result.get("ok", True) and result.get("status") != "ERROR":
                self._session_registered = True
            else:
                error_msg = result.get("error") or result.get("message", "Unknown error") if isinstance(result, dict) else "Unknown error"
                logger.error(f"Session re-registration failed during reload: {error_msg}")
        except Exception as e:
            logger.error(f"Error re-registering session during reload: {e}", exc_info=True)
    ...
    async def on_startup(self) -> None:
        ...
        try:
            timeout_str = os.environ.get("WHATSAPP_SESSION_REGISTER_TIMEOUT_SECONDS", "120")
    ...
    def api(self) -> Union[WPPConnectAPI, WWebJSAPI, UltraMsgAPI]:
        ...
        if not self.is_configured():
            raise ValidationError(f"WhatsApp action is not configured: {'; '.join(self._config_issues())}")
    ...
    async def get_webhook_url(self, allowed_ip: Optional[str] = None, regenerate: bool = False) -> str:
        ...
        if not self.base_url or not self.base_url.strip():
            raise ValidationError("base_url (APP_BASE_URL) is required for webhook URL generation")
        if not self.base_url.startswith(("http://", "https://")):
            raise ValidationError(f"base_url must be a valid HTTP/HTTPS URL, got: {self.base_url}")
        ...
        try:
            ...
            if not regenerate and self.webhook_url and "?api_key=" in self.webhook_url and self.webhook_url.startswith(expected_url_base):
    ...
    async def set_recording_status(
        ...
        if not self.is_configured():
            return
        try:
            await self.api().set_recording_status(phone=phone, value=value, is_group=is_group, duration=duration)
        except Exception as e:
    ...
    async def register_session(self) -> Dict[str, Any]:
        ...
        if not self.is_configured():
            ...
            logger.debug(f"WhatsApp action not configured, cannot register session. Missing: {'; '.join(issues)}")
            return {
                "status": "skipped",
        ...
        try:
            ...
            if result.get("status") == "ERROR" or not result.get("ok", True):
                ...
                logger.warning(f"Session registration failed for '{self.session}': {error_msg}")
                return result
    ...
    async def healthcheck(self) -> Union[bool, Dict[str, Any]]:
        ...
        errors = []
        if not self.provider or self.provider not in ["wppconnect", "wwebjs", "ultramsg"]:
