...
- `WHATSAPP_RELOAD_WEBHOOK_SUBSCRIBE=false` — skip override on action reload (default). Set `true` only to force re-register on reload.
...
| 2. App auth middleware | **`POST …/_internal/deferred` must bypass `AuthenticationMiddleware`** (no JWT on LWA pass-through). jvspatial **`PathMatcher` always exempts** `/_internal/deferred` so custom **`JVSPATIAL_AUTH_EXEMPT_PATHS`** cannot accidentally drop it. If you still see **`[401] path=/api/_internal/deferred has_auth_header=False`**, upgrade jvspatial to a version with that fix. Deferred-route auth: **LWA loopback** (`127.0.0.1`) is always allowed; **non-loopback** callers need **`JVSPATIAL_DEFERRED_INVOKE_SECRET`** (or get 401 when unset). If logs say secret unset and peer is not loopback, you are hitting the public URL path—not LWA self-invoke. | No **401** from `auth_middleware` on `/_internal/deferred`; deferred self-invoke from `127.0.0.1` succeeds without a secret header. |
