"""AccessControl for the SkillExecutive (ADR-0012 inv. 6).
...
async def _resolve_access_control(agent: Any) -> Optional[Any]:
    ...
    try:
        ...
        logger.debug("skill_executive.access: failed to fetch AC: %s", exc)
        return None
    if ac is None:
        return None
    try:
...
async def is_tool_allowed(
    ...
    try:
        ...
        logger.warning(
            "skill_executive.access: has_action_access raised for %s — "
            "failing closed: %s",
            label,
            exc,
        )
