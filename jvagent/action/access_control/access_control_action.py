...
class AccessControlAction(Action):
    ...
    exceptions: List[str] = attribute(
        ...
        description="Actions exempt from permissions"
    )
    ...
    session_groups: Dict[str, List[str]] = attribute(
        ...
        description="Group name to session IDs mapping"
    )
    ...
    async def has_action_access(self, session_id: str, action_label: str = "all", channel: str = "default") -> bool:
    ...
    def _check_access(self, user_id: str, channel: str, resource: str) -> bool:
        ...
        channel_perms = self.permissions.get(channel, self.permissions.get("default", {}))
        resource_perms = channel_perms.get(resource, channel_perms.get("any", {}))
        
        # Check deny rules first
        for deny_rule in resource_perms.get("deny", []):
            if deny_rule.get("enabled", True) and self._matches_rule(user_id, deny_rule):
                return False
        
        # Check allow rules
        for allow_rule in resource_perms.get("allow", []):
            if allow_rule.get("enabled", True) and self._matches_rule(user_id, allow_rule):
                return True
    ...
    def _matches_rule(self, user_id: str, rule: Dict) -> bool:
        ...
        if rule_group:
            if rule_group in ["all", "any"]:
                return True
            if rule_group in self.session_groups and user_id in self.session_groups[rule_group]:
                return True
