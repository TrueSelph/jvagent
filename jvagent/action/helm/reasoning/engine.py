...
class CockpitEngine:
    ...
    async def _build_system_prompt(self) -> str:
        ...
        if catalog and self.ctx.preloaded_skills:
            ...
            if filtered:
                try:
                    from jvagent.action.helm.reasoning.catalog.skill_catalog import (
                        SkillCatalog,
                    )
        ...
        if getattr(cfg, "preload_user_memory", True):
            try:
                from jvagent.action.helm.reasoning.tools.memory import (
        ...
        try:
            from jvagent.action.helm.reasoning.tools.clock import (
