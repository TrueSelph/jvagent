...
class AppDescriptor:
    ...
    def __init__(self, data: Dict[str, Any], path: Path):
        ...
        self.properties = {
            ...
            if k not in ["name", "description", "file_storage_provider", "file_storage_root_dir", "file_storage_enabled"]
        }
...
class AppLoader:
    ...
    async def _install_agents(
        ...
        for agent_ref in descriptor.agents:
            ...
            if "/" not in agent_ref:
                logger.warning(f"Invalid agent reference format (expected 'namespace/agent_name'): {agent_ref}")
                failed_count += 1
                continue
        ...
        if failed_count > 0:
            logger.warning(f"Agent installation: {installed_count} succeeded, {failed_count} failed")
    ...
    async def get_app_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            agents_list = await agents_manager.list_agents() if hasattr(agents_manager, 'list_agents') else []
