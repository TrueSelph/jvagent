...
def _get_config_value(key: str, env_var: str, default: Any, config_type: type = str) -> Any:
...
async def cleanup_expired_entries() -> bool:
    ...
    async with _cache_lock:
        expired_agents = [
            agent_id for agent_id, (_, cached_at) in _agent_cache.items()
    ...
    async with _action_cache_lock:
        expired_actions = [
            cache_key for cache_key, (_, cached_at) in _action_cache.items()
