...
class AgentLoader:
        ...
        try:
            ...
            if existing_agents:
                if not update_if_exists:
                    logger.debug(f"Agent '{descriptor.namespace}/{descriptor.name}' already exists (skipping)")
            ...
            if descriptor.actions:
                await self._install_actions(agent, descriptor, actions_manager, update_if_exists=update_if_exists)
    ...
    async def _install_actions(
        ...
        if failed_count > 0:
            logger.warning(f"Action registration: {registered_count} registered, {updated_count} updated, {failed_count} failed")
    ...
    async def _dedupe_agent_actions(self, agent: Agent, actions_manager: Actions) -> None:
        ...
        try:
            ...
            if removed > 0:
                ...
                agent_namespace = getattr(
                    agent, "namespace", getattr(agent, "name", "unknown")
                )
                logger.warning(
