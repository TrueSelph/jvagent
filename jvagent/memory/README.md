...
- **Uploaded files (ADR-0021 S4)**: every file in `visitor.data` (keys `image_urls`, `whatsapp_media`, `files`, `attachments`, `documents`) is ingested as **one consolidated** `source="upload"` artifact by the orchestrator's `_ingest_uploads` reflex (gated by `ingest_uploads`, default on). Each artifact is the single home for that file — its reference (bytes persisted to the caller's **per-user file storage**, referenced by `Artifact.path`; **never inline on the node**, so the graph stays lean) *plus* its content/understanding: text files decode into the queryable `data`; an uploaded **image** is enriched in place with a **per-image** VisionAction interpretation (file + interpretation in ONE artifact — not two, tagged `interpreted`/`vision`); other binaries carry a descriptor. The single-artifact-per-file shape is the extension point for document interpreters later (extraction/summary merged onto the same artifact). When a file-backed artifact is reaped, `_reap_artifacts_for` also deletes its stored bytes (`_delete_artifact_file`), so storage tracks the graph. The standalone `_vision_reflex` (separate `source="vision"` artifact) remains only as the fallback when `ingest_uploads` is off.
