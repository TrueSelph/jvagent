...
class TestGoogleOAuth2Flow(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_authorize_and_save_credentials(self, mock_save_file, mock_from_config):
        mock_flow = MagicMock()
    ...
    @patch('jvagent.action.google_actions.google_action.Credentials.from_authorized_user_info')
    @patch.object(GoogleAction, 'get_file', new_callable=AsyncMock)
    async def test_get_service_with_cached_token(self, mock_get_file, mock_from_info, mock_build):
    ...
    async def test_get_service_missing_token_raises_error(self, mock_get_file):
        ...
        with self.assertRaisesRegex(ValueError, "No valid OAuth2 credentials found. Please call the auth_url endpoint to authorize."):
            await self.action.get_service()
