"""Lifecycle tests for ThinkingInteractAction task management."""
...
def _mock_action():
    ...
    action.get_class_name = MagicMock(return_value="ThinkingInteractAction")
...
async def test_execute_completes_task_and_cleans_up():
    ...
    with patch(
        "jvagent.action.thinking.thinking_interact_action.ToolExecutor",
        return_value=tool_executor,
    ):
        await ThinkingInteractAction.execute(action, visitor)
