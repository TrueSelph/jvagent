...
def whatsapp_action(mock_agent):
    ...
    # Mock get_agent to return the mock agent
    action.get_agent = AsyncMock(return_value=mock_agent)
    action.save = AsyncMock()
...
async def test_get_webhook_url_generates_new_key(whatsapp_action, mock_agent):
    ...
    with patch("jvagent.action.whatsapp.whatsapp_action.get_or_create_system_user") as mock_user, \
         patch("jvagent.action.whatsapp.whatsapp_action.APIKeyService") as mock_service_class, \
         patch("jvagent.action.whatsapp.whatsapp_action.get_prime_database") as mock_db, \
         patch("jvagent.action.whatsapp.whatsapp_action.GraphContext") as mock_context_class:
        ...
        mock_service.generate_key = AsyncMock(return_value=(plaintext_key, mock_api_key))
        ...
        assert webhook_url.startswith("http://localhost:8000/api/whatsapp/interact/webhook/n.Agent.test123")
...
async def test_get_webhook_url_regenerates_on_request(whatsapp_action, mock_agent):
    ...
    with patch("jvagent.action.whatsapp.whatsapp_action.get_or_create_system_user") as mock_user, \
        ...
        mock_service.generate_key = AsyncMock(return_value=(new_plaintext_key, mock_new_key))
