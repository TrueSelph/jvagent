...
from jvagent.action.whatsapp.whatsapp_action import WhatsAppAction, TypingStateManager
from jvagent.action.whatsapp.endpoints import (
    MediaBatchManager,
...
class TestTypingStateManagerConcurrency:
    ...
    async def test_concurrent_typing_no_interference(self):
        ...
        async def set_and_verify(user_id: str):
            ...
            assert result is True, f"Expected first set_typing to return True for {user_id}"
            ...
            assert result2 is False, f"Expected duplicate set_typing to return False for {user_id}"
    ...
    async def test_cross_user_isolation(self):
        ...
        async def check_user_b():
            ...
            for _ in range(100):
                ...
                assert is_typing is False, "User B's state was corrupted by User A's operations"
...
class TestConversationLockManagerConcurrency:
    ...
    async def test_different_users_not_blocked(self, lock_manager):
        ...
        assert total_time < 0.2, f"Users were blocked - took {total_time}s instead of ~0.1s"
...
class TestStressConcurrency:
    ...
    async def test_lock_manager_under_contention(self):
        ...
        assert counter["value"] == 100, f"Lock contention issue: counter is {counter['value']}, expected 100"
...
class TestConnectionPoolStress:
    ...
    async def test_concurrent_session_requests(self):
        ...
        assert len(unique_sessions) == 1, f"Expected 1 shared session, got {len(unique_sessions)}"
    ...
    async def test_multiple_hosts_isolation(self):
        ...
        assert id(session1) != id(session2), "Different hosts should have different sessions"
    ...
    async def test_session_recreation_after_close(self):
        ...
        assert session1_id != session2_id, "Closed session should be replaced with new one"
    ...
    async def test_multiple_background_tasks_independent(self):
        ...
        for i in range(10):
            if i % 3 == 0:
                ...
                tasks.append(create_background_task(success_task(i), name=f"success_{i}"))
        
        # Wait for all tasks
        await asyncio.gather(*tasks, return_exceptions=True)
        
        # Successful tasks should complete despite failures
        assert results["success"] >= 6, f"Expected at least 6 successes, got {results['success']}"
        assert results["failure"] >= 3, f"Expected at least 3 failures, got {results['failure']}"
