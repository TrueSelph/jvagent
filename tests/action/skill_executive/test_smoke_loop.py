"""Phase 3 — end-to-end SkillExecutive loop + continuation smoke (ADR-0012).
...
async def test_trivial_turn_uses_reply_tool(
    make_skill_executive, make_visitor, publish_log, monkeypatch
):
...
async def test_no_emission_falls_back_to_clarify(
    make_skill_executive, make_visitor, publish_log
):
    ex = make_skill_executive(actions=[], decisions=[{"action": "final", "answer": ""}])
...
async def test_first_entry_into_ia_flow_via_tool(
    make_skill_executive, make_visitor, publish_log, flow_stub_cls
):
...
async def test_active_flow_offtopic_routed_elsewhere_not_into_flow(
    make_skill_executive, make_visitor, publish_log, monkeypatch, flow_stub_cls
):
