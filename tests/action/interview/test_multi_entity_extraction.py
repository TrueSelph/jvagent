    ...
    return [
        {
            ...
            "constraints": {"description": "The user's full name", "type": "string"},
            "required": True,
        },
        {
...
async def mock_interview_action(sample_question_index):
    ...
    action = MagicMock()
    action.question_graph = sample_question_index
    ...
    action._sort_by_question_order = MagicMock(
        side_effect=lambda fields, session: sorted(fields)
    )
...
async def mock_session(sample_question_index, test_db):
    ...
    session = await InterviewSession.create(
        ...
        question_graph=sample_question_index,
...
@pytest.mark.skip(
    reason="_process_responses_to_questions removed; response processing in execute()"
)
class TestMultiEntityExtraction:
    ...
    async def test_all_valid_extractions_stored(
        ...
        # Setup mocks
        mock_interview_action._get_question_node = AsyncMock(
            return_value=mock_question_node
        )
        ...
        # Verify target_node is cleared (no active question)
        assert mock_session.target_node is None
    ...
    async def test_partial_valid_extractions(
        ...
        # Verify target_node points to first invalid field
        assert mock_session.target_node == "available_times"
        ...
        assert (
            "This field is required" in call_args[0][1]
            or "available_times" in call_args[0][1]
        )
    ...
    async def test_conditional_edge_skips_unreachable_questions(
        ...
        mock_session.question_graph = conditional_index
        ...
        interview_walker.process_and_validate = AsyncMock(
            side_effect=[
                ...
                (
                    ...
                    "Got it. Let me clarify the specific time.",
                ),
                ("eldon@mail.com", ValidationStatus.VALID, None),
            ]
        )
    ...
    async def test_first_invalid_tracked_correctly(
        ...
        assert (
            "required" in str(call_args[0][1]).lower()
            or "available_times" in str(call_args[0][1]).lower()
        )
