...
def _user_text(directive: str) -> str:
    ...
    body = directive[len("Tell the user:"):] if directive.lower().startswith("tell the user:") else directive
...
def test_terse_fragment_gets_question_appended():
    d = validation_guidance_directive("Invalid value", question_text="What is the price?")
    user = _user_text(d)
    assert user == "Invalid value What is the price?"


def test_question_ending_error_is_self_contained():
    d = validation_guidance_directive("What currency?", question_text="What currency is the price in?")
    user = _user_text(d)
    assert user == "What currency?"


def test_prefixed_error_strips_prefix_and_skips_question():
    d = validation_guidance_directive("Tell the user: Pick a valid slot", question_text="What times?")
    user = _user_text(d)
