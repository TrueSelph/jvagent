...
class TestConditionalBranching:
    ...
    async def test_get_reachable_unanswered_questions_with_branches(self, test_db):
        ...
        user_type_node = await QuestionNode.create(label="user_type", question_config=question_graph[0])
        premium_node = await QuestionNode.create(label="premium_features", question_config=question_graph[1])
        standard_node = await QuestionNode.create(label="standard_setup", question_config=question_graph[2])
        contact_node = await QuestionNode.create(label="contact_info", question_config=question_graph[3])
        
        # Create edges for conditional branching
        await QuestionEdge.connect(
            user_type_node, premium_node,
            condition={"op": "equals", "value": "premium"},
            branch_index=0,
            is_default=False
        )
        await QuestionEdge.connect(
            user_type_node, standard_node,
            condition={"op": "equals", "value": "standard"},
            branch_index=1,
            is_default=False
        )
        await QuestionEdge.connect(
            user_type_node, contact_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
        await QuestionEdge.connect(
            premium_node, contact_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
        await QuestionEdge.connect(
            standard_node, contact_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
    ...
    @pytest.mark.asyncio
    async def test_reachable_unanswered_prevents_branch_function_premature_execution(self, test_db):
        ...
        # Create question nodes
        dependency_node = await QuestionNode.create(label="dependency_question", question_config=question_graph[0])
        dependent_node = await QuestionNode.create(label="dependent_question", question_config=question_graph[1])
        other_node = await QuestionNode.create(label="other_question", question_config=question_graph[2])
        final_node = await QuestionNode.create(label="final_question", question_config=question_graph[3])
        ...
        await QuestionEdge.connect(
            dependency_node, other_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
        await QuestionEdge.connect(
            dependent_node, final_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
        await QuestionEdge.connect(
            other_node, final_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
    ...
    async def test_walker_stops_at_branches_without_cache(self, test_db):
        ...
        question_graph = [
            ...
            },
            {
                ...
                "required": False,
                "branches": [
                    {"condition": {"function": "detect_sensitive"}, "target": "is_sensitive"}
                ],
            ...
            },
            {
                ...
                "branches": [
                    ...
                    {"condition": {"op": "equals", "value": "no"}, "target": "reporting_on_behalf"}
                ]
            },
            {
        ...
        # Create question nodes
        desc_node = await QuestionNode.create(label="incident_description", question_config=question_graph[0])
        location_node = await QuestionNode.create(label="incident_location", question_config=question_graph[1])
        media_node = await QuestionNode.create(label="incident_media", question_config=question_graph[2])
        sensitive_node = await QuestionNode.create(label="is_sensitive", question_config=question_graph[3])
        behalf_node = await QuestionNode.create(label="reporting_on_behalf", question_config=question_graph[4])
        
        # Create edges (sequential with conditional branches)
        await QuestionEdge.connect(
            desc_node, location_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
        await QuestionEdge.connect(
            location_node, media_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
        await QuestionEdge.connect(
            media_node, sensitive_node,
            condition={"function": "detect_sensitive"},
            branch_index=0,
            is_default=False
        )
        await QuestionEdge.connect(
            media_node, behalf_node,
            condition=None,
            branch_index=-1,
            is_default=True
        )
        ...
        assert "incident_location" in reachable, \
        ...
        assert "incident_media" in reachable, \
        ...
        assert "is_sensitive" not in reachable, \
            "Should stop at incident_media (no cache) - is_sensitive should not be reachable"
        assert "reporting_on_behalf" not in reachable, \
    ...
    async def test_walker_continues_past_answered_nodes_with_branches(self, test_db):
        ...
        # Create nodes
        q1_node = await QuestionNode.create(label="q1", question_config=question_graph[0])
        q2_node = await QuestionNode.create(label="q2", question_config=question_graph[1])
        q3_node = await QuestionNode.create(label="q3", question_config=question_graph[2])
        ...
        assert "q2" in reachable, "Should continue past answered q1 even though it has branches"
