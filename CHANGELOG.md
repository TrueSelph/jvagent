...
- **Bridge + Helm and Cockpit patterns fully removed; the Executive + Centers pattern is now the canonical orchestrator.** The `jvagent/action/bridge/` and `jvagent/action/helm/` packages (and the earlier `jvagent/action/cockpit/` package) are deleted, along with their tests, the `bridge.yaml` scaffold profile, the `bridge_agent` / `example_agent` / `proposal_agent` / `resolv_demo` examples, the helm-era `SkillCatalog` and its `/skills/reload` HTTP endpoint, the `docs/BRIDGE.md` / `docs/COCKPIT.md` references, and ADR-0002 / ADR-0007 / ADR-0008 / ADR-0009. The scaffolder default profile is now `executive`. New code lives in `jvagent/action/executive/` (`ExecutiveInteractAction` + Skills/IA/Persona centers). Skill-hub install/remove scripts no longer hot-load into a running session (skills are picked up on the next interaction). The `Manifest` schema gained a `can_interrupt` field so a turn-locking action can own its own cancellation. See [`docs/EXECUTIVE.md`](docs/EXECUTIVE.md) and [`.planning/adr/0010-executive-centers-architecture.md`](.planning/adr/0010-executive-centers-architecture.md).
