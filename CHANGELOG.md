...
- **Wave 6 Refactor**: Extracted `_process_model_decision` from `_run_loop` in `jvagent/action/orchestrator/loop.py` (handles garbled decisions, normalization, final action deflections, chain/plan guards — 87-line cohesive block) and `_initialize_logging_database` from `create_server_from_config` in `jvagent/cli/server_config.py` (handles log DB config resolution, env var setting, and initialization — 131-line cohesive block). Both extractions are behavior-preserving and tested via existing orchestrator and CLI test suites. Documented in ADR-0035.
