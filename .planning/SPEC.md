...
>
> For terminology, see [`GLOSSARY.md`](GLOSSARY.md). For diagrams, see [`architecture.md`](architecture.md). For the underlying graph framework, see [`jvspatial-integration.md`](jvspatial-integration.md).
...
It does **not** specify: choice of model provider, transport protocol details (HTTP semantics are jvspatial's), storage backend selection, deployment topology, or the cockpit's prompt design (see [`../docs/COCKPIT.md`](../docs/COCKPIT.md)).
...
### 3.3 Cockpit walker-revisit pattern

`CockpitInteractAction` ([`cockpit/cockpit_interact_action.py:79`](../jvagent/action/cockpit/cockpit_interact_action.py)) implements the **walker-revisit pattern**:

- Each call to `execute(walker)` performs **exactly one** model call.
- If the model returns tool calls: the engine dispatches them, persists state on `walker._skill_state`, and re-adds itself to the walk path via `walker.prepend([self])`. The walker visits the cockpit again, restoring state and running the next step.
- If the model produces text with no tool calls: response is delivered and the cockpit run terminates.
- Termination is also forced by `max_iterations` (default 25, [`cockpit_interact_action.py:105`](../jvagent/action/cockpit/cockpit_interact_action.py)), `max_duration_seconds` (default 300.0), and a stuck-detection heuristic (Jaccard similarity ≥ `stuck_intent_jaccard_threshold` over the recent tool-call window).
...
- `get_tools() -> List[Tool]` ([`base.py:192`](../jvagent/action/base.py)) — every `Action` MAY expose tools to the Cockpit. Each tool wraps a callable with a JSON Schema for arguments. The cockpit registers these with an `action__` prefix in its tool registry.
...
See [`adr/0004-namespace-isolation.md`](adr/0004-namespace-isolation.md) and [`action-authoring.md`](action-authoring.md) for the package layout.
...
Rationale: [`adr/0003-interaction-limit-pruning.md`](adr/0003-interaction-limit-pruning.md). User-facing detail: [`memory-and-pruning.md`](memory-and-pruning.md).
...
2. **Environment variable** (resolved via `jvspatial.env.env`; see [`configuration-keys.md`](configuration-keys.md)).
...
See [`observability.md`](observability.md) for the unified index over `docs/logging.md`, `docs/error-logging.md`, `docs/interaction-logging.md`.
...
4. `InteractAction.execute()` is called by the walker; it MUST NOT recursively invoke the walker on the same action without explicit `visitor.prepend([self])` semantics (cockpit-style walker-revisit) or stack overflow / infinite-walk protection (`max_visits_per_node=100` in jvspatial walker) will trip.
...
- [`adr/0002-walker-revisit-cockpit.md`](adr/0002-walker-revisit-cockpit.md)
...
- The cockpit's prompt strategy → [`../docs/COCKPIT.md`](../docs/COCKPIT.md) and `jvagent/action/cockpit/prompts.py`.
