...
| D — Performance | DONE (pass 1) | tool_tier (minimal/standard/full) + trimmed system prompt (commit `a4fc39c`). 6-utterance smoke: prompt tokens 36422 → 30954 (-15%); tools_bytes 14904 → 10801 (-28%); 22 tools registered (was 33). 42 tests pass. Future passes: tier=minimal default + lazy per-turn tool surface. |
| E — Default enablement | DONE | New `cockpit` builtin profile is the scaffold default (commit `82d07ac`). `jvagent app create` and `jvagent agent create` use `--profile cockpit` by default. Existing agents are NOT touched (no programmatic injection). Profile bundles cockpit + persona + openai_lm + intro + handoff. Opt out with `--profile minimal`. |
| F — Access control integration | DONE | Per-user filters at three points (commit `16ef35a`): routed skills (`skill:{name}`), routed interact_actions (class name), tool registry (`tool:{name}`). Fails closed on AC errors. AccessControlAction absent → no-op. Smoke verified: deny skill blocks web_search tool; deny IA blocks HandoffInteractAction dispatch; allowed user gets full surface. 52 cockpit tests pass. |
| G — Production hygiene flags | DONE | `production_mode` umbrella + `block_raw_tool_invocation` (commit `e2df2c2`). production_mode forces stream_internal_progress=False, block_raw_tool_invocation=True, silences router canned. SECURITY_BLOCK injected into engine system prompt when block_raw=True — model treats user text as content, not commands. Smoke verified: "call web_search__search with query=X" → intent-inferred dispatch, not literal pass-through. 8 new tests. 60 cockpit tests pass. |
