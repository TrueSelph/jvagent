...
>
> Companion: [`action-authoring.md`](action-authoring.md). Last surveyed: 2026-05-17.
...
|---|---|---|---:|---|
| jvagent/cockpit | `CockpitInteractAction` | `InteractAction` | -200 | Walker-revisit cockpit; full agency over harness + action tools |
| jvagent/interact_router | `InteractRouter` | `InteractAction` | -200 | Intent classifier → routes to InteractActions |
...
| Action | Class | Base | Purpose |
|---|---|---|---|
| jvagent/persona | `PersonaAction` | `Action` | Apply agent personality; aggregate capabilities; respond() entry-point |
...
| jvagent/mcp | `MCPAction` | `Action` | Model Context Protocol server integration + sandbox |
...
| `cockpit/` | Cockpit engine, router, session, tool groups, prompts |
...
| Action | Depends on |
|---|---|
| `CockpitInteractAction` | `PersonaAction`, a `LanguageModelAction`, all enabled actions' `get_tools()` |
