...
Cross-link: [`action-authoring.md`](../action-authoring.md) for the full contract.
...
| Will it expose a tool to the cockpit (model-callable)? | Override `get_tools()` |
...
For Slack: it's a channel adapter — subclass `Action`, register with the `ResponseBus`, define `get_tools()` for cockpit-driven sending.
...
Edit [`.planning/actions-catalog.md`](../actions-catalog.md) — add a row under §1.4 (Messaging / broadcast).
...
- [ ] `get_tools()` exposes cockpit tools (if applicable)
...
- [ ] Catalog updated in [`actions-catalog.md`](../actions-catalog.md)
