...
| G — PersonaHelm | DONE | `jvagent/action/helm/persona/persona_helm.py` wraps `PersonaAction`; handoff_state schema accepts `text` / `directive` / `directives` / `history_limit` / `use_history`; injects "Tell the user: …" directive when draft text is present; falls back gracefully if PersonaAction unavailable. `response_deliver_via_persona` alias deferred. |
