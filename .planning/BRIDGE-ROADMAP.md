...
| C — ReasoningHelm parity | AWAITING REAL-LM RUN | C-0..C-6 shipped (commits `0718b3c`..`a6622e9`). Bridge + ReasoningHelm composition lives under `jvagent/action/helm/reasoning/`; `bridge_agent` example committed at `7a3713a`; smoke harness at `tests/action/bridge/smoke_bridge.py` (this commit). All 261 in-tree tests pass; cockpit (189) untouched. Final exit criterion — ≤5% drift on the 6-utterance suite vs baseline `7d95904` — requires a real-LM run with operator API keys. Run via `.venv/bin/python tests/action/bridge/smoke_bridge.py`; parity report archived under `tests/action/bridge/baselines/<sha>.json`. |
