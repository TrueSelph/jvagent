...
### Skill
Markdown-first standard operating procedure (with optional referenced tools) the Orchestrator can load to instruct the model. A skill is *judgment over capability* — it coordinates tools the agent already has, referencing action tools by their `namespace__tool` name. Discovered and exposed via `find_skill` / `use_skill` ([`jvagent/action/orchestrator/skills.py`](../jvagent/action/orchestrator/skills.py)). Distinct from `Action`s. See [`adr/0011-skills-two-kinds.md`](adr/0011-skills-two-kinds.md).
