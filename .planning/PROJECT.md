...
The model is the pilot. Tools are the controls. Skills are the flight plan. ([source](../docs/COCKPIT.md))
...
- The cockpit's walker-revisit pattern (one model call per visit, state on `visitor._skill_state`) gives the walker visibility into every iteration — stream commits, access control, and recording happen per step, not in a hidden inner loop.
...
| **Walker-revisit cockpit** over internal loop | Walker sees every model call (access control, streaming, recording fire per step) but adds graph-traversal overhead per iteration. See [`adr/0002`](adr/0002-walker-revisit-cockpit.md). |
...
| **Cockpit grants the model full agency** | Maximum flexibility for agent authors and skill writers; harder to predict cost/latency without `max_iterations` + stuck detection. |
...
This repo is `jvagent` only. The graph framework is at `../jvspatial` (sibling dir, pip-installed). See [`jvspatial-integration.md`](jvspatial-integration.md) for the boundary.

---

## Roadmap

In-flight planning lives at [`COCKPIT-ROADMAP.md`](COCKPIT-ROADMAP.md) and [`codebase/`](codebase/). When this project adopts the GSD workflow, roadmaps move to [`ROADMAP.md`](ROADMAP.md) under `.planning/`.
...
- A fresh AI agent dropped into a subsystem (`jvagent/core/`, `jvagent/action/cockpit/`, etc.) can do correct local work by reading the local `CLAUDE.md` alone.
