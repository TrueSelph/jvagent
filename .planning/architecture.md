...
sequenceDiagram
    ...
    loop For each top-level InteractAction in weight order
        ...
        alt run_in_background == True
            ...
            IA->>W: optionally visitor.visit(child) or visitor.prepend([self])  (cockpit)
        end
    end
...
## 4. Cockpit walker-revisit pattern

```mermaid
sequenceDiagram
    autonumber
    participant W as InteractWalker
    participant CIA as CockpitInteractAction
    participant CR as CockpitRouter (Phase 1)
    participant CE as CockpitEngine (Phase 2)
    participant M as LanguageModel
    participant T as Tool dispatcher
    participant RB as ResponseBus

    W->>CIA: execute(walker)  [visit #1]
    CIA->>CR: classify(utterance)
    CR-->>CIA: posture=RESPOND, skills=[skillA, skillB]
    CIA->>CE: initialize()  (build system prompt + assemble tools)
    CE->>M: model.query(messages, tools)
    M-->>CE: tool_calls=[t1, t2]
    CE->>T: dispatch(t1, t2)
    T-->>CE: results
    CE->>W: visitor._skill_state = state
    CE->>W: visitor.prepend([self])  (revisit me)

    W->>CIA: execute(walker)  [visit #2]
    CIA->>CE: step()  (restore state, run next model call)
    CE->>M: model.query(messages, tools)
    M-->>CE: text only (no tool calls)
    CE->>RB: deliver final response
    CE-->>W: done (no further visits)
```

**Why revisit instead of looping inside `execute()`?** Every model call passes through the walker, so per-step concerns — streaming flush, access control, action recording — happen naturally. See [`adr/0002-walker-revisit-cockpit.md`](adr/0002-walker-revisit-cockpit.md).

References:
- `action/cockpit/cockpit_interact_action.py:79-200` — class def
- `action/cockpit/engine.py` — engine
- `action/cockpit/routing/router.py` — router
- `action/cockpit/session.py` — `CockpitSession` + state plumbing
...
| If you want to... | Read |
|---|---|
| Build an action | [`action-authoring.md`](action-authoring.md) |
| Understand the cockpit deeply | [`../docs/COCKPIT.md`](../docs/COCKPIT.md) + cockpit `CLAUDE.md` |
| Tune logging | [`observability.md`](observability.md) |
| Run locally | [`runbooks/local-dev.md`](runbooks/local-dev.md) |
| See every action | [`actions-catalog.md`](actions-catalog.md) |
| Understand jvspatial | [`jvspatial-integration.md`](jvspatial-integration.md) |
