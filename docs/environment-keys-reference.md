...
- **jvconnect proxy** (required for `provider: meta`): set `JVCONNECT_URL`, `JVCONNECT_API_KEY`, and `WHATSAPP_PHONE_NUMBER_ID`. Do **not** set `WHATSAPP_ACCESS_TOKEN` / `WHATSAPP_APP_SECRET`. After startup webhook registration, `JVCONNECT_WEBHOOK_SECRET` is persisted on the action (or set from jvconnect’s register response). Optional: `WHATSAPP_PROXY_URL` alias for `JVCONNECT_URL`, `WHATSAPP_WABA_ID` for scoped admin ops. jvconnect routes live under `/api/v1/meta/whatsapp/*` (room for `/api/v1/meta/messenger/*` and `/instagram/*` later).
