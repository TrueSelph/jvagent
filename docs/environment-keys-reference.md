...
- **jvconnect proxy** (required for `provider: meta`): set `JVCONNECT_URL` and `JVCONNECT_API_KEY` only (create a phone-bound key in jvconnect API Credentials). Do **not** set `WHATSAPP_ACCESS_TOKEN` / `WHATSAPP_APP_SECRET`. Startup and reload always POST jvconnect `webhook/register`; jvconnect short-circuits same agent `callback_url` (returns existing `webhook_secret`, no Meta Graph). Secret is persisted on the action (optional env backup: `JVCONNECT_WEBHOOK_SECRET`). Optional: `WHATSAPP_PROXY_URL` alias for `JVCONNECT_URL`. Routes under `/api/v1/meta/whatsapp/*` (room for messenger/instagram later).
...
- `WHATSAPP_STARTUP_WEBHOOK_REGISTER_TIMEOUT_SECONDS` — fail-fast bound for startup Meta register (default `15` on serverless, `60` otherwise)
- `WHATSAPP_WEBHOOK_REGISTER_DELAY_SECONDS` — optional seconds before Meta/jvconnect register on startup (default `0`; meta provider only)
